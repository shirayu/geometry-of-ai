...
> [!TIP]
> 読了後に、[理解度チェック](./appendix.4/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#appendix-4-空間の「物差し」再考-2点間から情報の密度まで)でも確認できる。

## 本Appendixの位置づけ

本Appendixは、講義本編で個別に登場した「近さ」や「隔たり」の測り方を、**「物差し（計量）」という一つの視点で束ね直す**発展的内容である。内積・コサイン類似度・双曲距離という**対称的な尺度**、KLダイバージェンスという**非対称な尺度**、そしてフィッシャー情報行列という**空間の感度を測る計量**の3つを扱う。

前提として、[第4回](04.md)（Softmaxと情報幾何学）、[第6回](06.md)（Attentionの内積計算）、[第7回](07.md)（vMF分布と集中度 $\kappa$ ）の内容に触れる。特に[第7回](07.md)で導入した「点（ $\kappa = \infty$ ）と分布（ $\kappa$ 有限）」の区別は、本Appendixが統計多様体の「点」を論じる出発点になる。[第12回](12.md)（双曲幾何学）、[第13回](13.md)（MoE）にも触れるが、未読でも読み進められるよう都度補足する。

本編で断片的に登場した「物差し」を並べ直すことで、Attentionの内積、Softmaxの背後にあるKLダイバージェンス、自然勾配法が使うフィッシャー情報行列が、実は同じ問い（何をどう測るか）への異なる答えであることが見えてくる。
...
- **「点」という言葉の多義性について**：本講義では、文脈に応じて「点」の意味が変わる。
    - **[第7回](07.md)まで（表現空間）**：データの位置を表すベクトル $\mathbf{h}$ 。不確実性を持たないため、分布（霧）と対比された。従来の点埋め込みは $\kappa = \infty$ （確信度無限大）の極限状態と解釈できる。
    - **今回（統計多様体）**：分布のパラメータ $\theta = (\boldsymbol{\mu}, \kappa)$ 。この空間上の1点は、[第7回](07.md)で扱った**分布（霧）全体**を代表している。「点 $\theta$ を動かす」ことは、「霧の形や位置を変える」ことと同義である。

  つまり、**表現空間での「分布（レベル2）」は、パラメータ空間では「点（レベル3）」として扱われる**。これは視座の転換であり、矛盾ではない。[第7回](07.md)が「ズームイン（点を拡大して霧を見る）」なら、今回は「ズームアウト（霧を操るコントローラーの座標を見る）」である。
...
小学校で習う算数では、2点間の距離は定規で測る。定規は一つあれば十分、というのが素朴な感覚だろう。しかし、その「定規」自体が、どのような空間にいるかによって変わる。平らな紙の上では直線距離（ユークリッド距離）が自然だが、地球儀の上では大円に沿った測地距離が適切だ。

深層学習における表現空間も同様である。むしろ事情はもっと込み入っている。ベクトルの「近さ」を測る方法は一つではない：
...
物差しを選ぶ前に、そもそも何を測ろうとしているのかを確かめておきたい。情報幾何学において、**「点」は単なる数値（スカラ）や座標ではない**。
...
| **レベル2：表現ベクトル** | $\mathbf{h} \in \mathbb{R}^d$ | **[第7回](07.md)で拡張した対象**。<br>従来の点埋め込みは、分布埋め込みの視点では $\kappa \to \infty$ （確信度無限大）の極限に相当すると見なせる。<br>[第7回](07.md)では、これを有限の $\kappa$ を持つ分布に拡張した。 | 望遠鏡で見る前の星<br>（点 or 霧） | 点： $\kappa = \infty$ <br>分布： $\kappa$ 有限 |
...
**本Appendixが焦点を当てるのは第3の階層である。** 統計多様体の各点は、ある確率分布に対応し、 $\theta$ はその点を指定する **座標（パラメータ）** である。つまり：

- **一般的な直感：** 点 = 位置を表すスカラや座標
- **[第7回](07.md)（表現空間）：** 点（ $\kappa = \infty$ ） vs 分布（ $\kappa$ 有限）の対比
- **今回（統計多様体）：** 点 $\theta$ = ある確率分布を代表し、その背後に[第7回](07.md)で扱った「霧」全体が広がっている

たとえば、vMF分布族は $(\boldsymbol{\mu}, \kappa)$ という2つのパラメータで指定される多様体を成す。各点（パラメータ $(\boldsymbol{\mu}, \kappa)$ ）は一つのvMF分布に対応し、その背後には**球面上のすべての可能な $\mathbf{h}$ に対する確率密度という関数的な構造**が広がっている。

[第7回](07.md)では「点埋め込み（ $\kappa = \infty$ ）を分布埋め込み（ $\kappa$ 有限）に拡張する」ことで解像度を上げた。今回は、その**分布を操るパラメータ空間**に視座を移す。「霧の形（ $\kappa$ ）」や「霧の位置（ $\boldsymbol{\mu}$ ）」を変えることは、パラメータ空間上で点 $\theta$ を動かすことに対応する。
...
>
> [第7回](07.md)では、この「点」を望遠鏡で拡大し、「星雲（分布）」として観察した。霧の濃さ（ $\kappa$ ）や中心の位置（ $\boldsymbol{\mu}$ ）という、より詳細な構造が見えるようになった。
>
...
本Appendixでは、これら3つの視点から「物差し」を再考し、情報幾何学の中心概念を、数式を最小限に抑えつつ直感的に理解することを目指す。
...
> [!NOTE]
> **[第6回](06.md)との接続**：標準的なAttentionは内積を使用し、Softmaxで正規化する。これは「どのKeyがQueryと高い内積を持つか」を測定し、その値に応じて情報を集約する操作である。内積はベクトルの大きさも考慮するため、純粋な「方向の類似度」とは異なる。一部のモデル（例：nGPTなど）では明示的にL2正規化を行い、コサイン類似度ベースのAttentionを実装している。

### 双曲距離：階層構造の深さを測る

ユークリッド空間や球面空間とは異なる計量として、**双曲距離（hyperbolic distance）** がある。これは負の曲率を持つ空間での距離である。

[第12回](12.md)で詳しく扱ったが、双曲空間の重要な特徴は：
...
| **球面** | 正 | 中心への回帰性 | 方向の多様性 | 正規化埋め込み（[第3回](03.md)） |
| **双曲** | 負 | 外に向かって拡張 | 階層・木構造 | Poincaré Embedding（[第12回](12.md)） |
...
> [!NOTE]
> **Attentionと内積**：[第6回](06.md)で扱う標準的なAttention（scaled dot-product attention）は、内積を基本とする。内積はベクトルの大きさと方向の両方を反映するため、純粋な「方向の類似度」（コサイン類似度）とは異なる。一部のモデル（nGPTなど）では明示的にL2正規化を行い、コサイン類似度ベースのAttentionを実装している。

## 非対称な尺度（ダイバージェンス）：学習の駆動力

### 距離では学習の方向が見えない

内積もコサイン類似度も双曲距離も、AとBの隔たりはBとAの隔たりと等しい。この対称性は、配置を決めるだけなら十分だった。しかし、**どちらからどちらに向かうべきか**という方向性は、この枠組みからは出てこない。
...
| 尺度 | 対称性 | 用途 | 講義での位置 |
| --- | --- | --- | --- |
| **距離（内積・コサイン）** | 対称 | 配置の決定 | [第6回](06.md)（Attention） |
| **双曲距離** | 対称 | 階層構造の表現 | [第12回](12.md) |
| **KLダイバージェンス** | 非対称 | 学習の駆動 | [第4回](04.md)（Softmax） |

> [!IMPORTANT]
> **[第4回](04.md)との接続**：[第4回](04.md)で扱ったSoftmax/Cross-Entropyの幾何学的意味は、まさにこの非対称なKLダイバージェンスにある。モデルの分布を真の分布に「引き寄せる」力が、学習の勾配となる。対称的な距離では、この引力の方向が定義できない。

## 計量としてのフィッシャー情報：空間の感度を測る

### 「局所と大局」の比喩とその限界

配置を決める定規と、学習を駆動する定規は手に入った。残るは、空間そのものの「硬さ」を測る定規である。その前に、一つ確認しておきたいことがある。
...
#### [第7回](07.md)との接続：vMF分布と $\kappa$ による直感

[第7回](07.md)で導入した $\kappa$ （集中度）を思い出してほしい。vMF分布のフィッシャー情報行列は、この $\kappa$ に強く依存する。

**なぜ「曲がっている」と言えるのか？**

- $\kappa$ が大きい（高確信度）場合：分布は鋭く尖っている。このとき、中心方向 $\boldsymbol{\mu}$ を少しでもズラすと、確率密度は劇的に変化する。つまり**感度（フィッシャー情報量）が高い**。パラメータ空間のこの領域は「ジャングル」であり、慎重な更新が必要な、情報密度の高い場所である。
  
- $\kappa$ が小さい（低確信度）場合：分布はボヤけている。中心 $\boldsymbol{\mu}$ を多少ズラしても、確率密度はあまり変わらない。つまり**感度（フィッシャー情報量）が低い**。パラメータ空間のこの領域は「砂漠」であり、大胆な更新が可能な、情報密度の低い場所である。
...
つまり、集中度 $\kappa$ が大きいほど、平均方向 $\boldsymbol{\mu}$ への感度が高くなる。[第7回](07.md)で「解像度を上げる（ $\kappa$ を無限大から有限に）」ことで得た不確実性の情報は、パラメータ空間の幾何（フィッシャー計量）に直接反映されている。

> [!NOTE]
> **表現空間とパラメータ空間の対応**：
>
> - [第7回](07.md)（表現空間）：「 $\kappa$ が小さい → 霧が濃い → 不確実性が高い」
...
しかし、学習の軌道は本来、**パラメータの表し方に左右されるべきではない**。自然勾配法は、フィッシャー情報行列を使って勾配を補正することで、**微小ステップの極限において、どのようなパラメータ化を選んでも確率分布の空間での同じ方向に進む**ことを実現する。
...
| **双曲距離** | 負曲率空間での距離 | 階層構造の表現（[第12回](12.md)） |
...
| --- | --- | --- | --- | --- |
| **表層** | データ | $x$ | 分布からサンプリングされた一点（雨粒） | [第0回](00.md) |
| **中層** | 情報量とエントロピー | $-\log p(x)$ 、 $H(P)$ | 一点の珍しさ（確実性からの遠さ）と、分布全体の広がり（霧の体積） | [第4回](04.md)、[第7回](07.md) |
...
### [第7回](07.md)との統合：「点」の二重性

[第7回](07.md)では「点埋め込み（ $\kappa = \infty$ ）から分布埋め込み（ $\kappa$ 有限）へ」という**ズームイン**を行った。本Appendixでは、その分布を操るパラメータ空間という**ズームアウト**を行った。

両者を統合すると、以下の二重の視点が得られる：

| 視点 | 「点」とは何か | 変化とは何か | 測り方 |
| --- | --- | --- | --- |
| **[第7回](07.md)（表現空間）** | ベクトル $\mathbf{h}$ 。 $\kappa = \infty$ なら点、 $\kappa$ 有限なら分布（霧）。 | 星の位置や霧の濃さが変わる | コサイン類似度、測地距離 |
...
内積、ダイバージェンス、フィッシャー情報。三つの定規を手にした今、最初の問い「物差しとは何か」に戻ってみる。**「測る」ことは「理解する」ことの第一歩である。そして「何を測るか」を知ることは、さらに根本的である。**
...
最後の問いは、情報幾何学の核心である。パラメータという「座標」の背後に、確率分布という豊かな構造が広がっている。位置という「点」の先に、情報という「形」が存在する。この多層的な視点が、AIの表現を読み解く土台になる。

### 講義本編との接続

本Appendixで扱った概念は、以下の回と密接に関連している：

- **[第4回](04.md)（Softmax/情報幾何）**：KLダイバージェンス、自然勾配法の応用
- **[第6回](06.md)（Attention）**：内積（scaled dot-product）による類似度計算。標準的なAttentionは内積を使用し、ベクトルの大きさも考慮する。正規化Attentionではコサイン類似度に相当。
- **[第7回](07.md)（不確実性の復権）**：本Appendixと最も強く結びついている。[第7回](07.md)が「表現空間での解像度向上（点→分布）」なら、本Appendixは「パラメータ空間での幾何学（分布を操る座標系）」。vMF分布の $\kappa$ がフィッシャー情報行列に与える影響を具体例として扱った。
- **[第12回](12.md)（双曲幾何）**：双曲距離による階層構造の表現
- **[第13回](13.md)（高次元/MoE）**：計算資源の動的配分（Fisher情報との**比喩的対応**として理解可能）
...
本Appendixではリーマン計量（フィッシャー情報行列）を中心に扱った。物差しの話はこれで尽きた、と思いたいところだが、リーマン幾何学はあくまで「物差しの一族」の中核であって、全体ではない。ここでは、近年の深層学習研究で存在感を増しつつある2つの幾何学的構造に簡潔に触れておく。いずれも発展的な話題であり、本講義で前提とはしないが、「リーマン計量では足りなくなる場面」がどこにあるかを知っておくことは、視野を広げる助けになるだろう。
