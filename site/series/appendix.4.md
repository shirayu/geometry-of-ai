...
前提として、[第4回](04.md)（分類を確率で読む）、[第6回](06.md)（Attentionの内積計算）、[第7回](07.md)（点から分布へ）の内容に触れる。特に[第7回](07.md)で導入した「点（ $\kappa = \infty$ ）と分布（ $\kappa$ 有限）」の区別は、本Appendixが統計多様体の「点」を論じる出発点になる。[第12回](12.md)（双曲幾何学）、[第13回](13.md)（MoE）にも触れるが、未読でも読み進められるよう都度補足する。
...
本Appendixでは、これら3つの視点から「物差し」を再考し、情報幾何学の中心概念を、数式を最小限に抑えつつ直感的に理解することを目指す。物理的な「長さ」という直感から、情報的な「量」へと視野を広げることがその目標である。距離を測ることは、単に「どれだけ離れているか」を知るだけでなく、「どう変化するか」「どこが重要か」を知ることでもある。
...
| [第7回](07.md)（点から分布へ） | 本Appendixと最も強く結びついている。[第7回](07.md)が「表現空間での解像度向上（点→分布）」なら、本Appendixは「パラメータ空間での幾何学（分布を操る座標系）」。vMF分布の $\kappa$ がフィッシャー情報行列に与える影響を具体例として扱った |
...
本講義は、**空間の形**（どこに配置されているか）を扱った。しかし、情報幾何学のもう一つの顔は、**情報の流れ**（どう移動するか）である。
