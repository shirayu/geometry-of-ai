...
| キーワード | 説明 |
| --- | --- |
| **確率分布の空間** | <a id="kw-prob-distribution-space"></a>個々の確率分布を「点」として扱う空間（分布の集合）。 $K$ カテゴリでは確率単体 $\Delta^{K-1}$ の内部、パラメトリックモデルでは統計多様体がその例。「確率分布」はこの空間の1点であり、空間そのものとは区別される（[第0回](00.md)で導入、[第4回](04.md)で詳述）。 |
| **情報幾何学** | 確率分布の空間を多様体として扱う学問。 |
| **再パラメータ化不変性** | 座標系の取り方に依存しない性質。 |
| **フィッシャー情報行列** | <a id="kw-fisher-info"></a>確率分布の「変化しやすさ」を反映したリーマン計量。パラメータ $\theta$ は通常、分布を指定する少数の変数（[第4回](04.md)・[Appendix 4](appendix.4.md)・[Appendix 5](appendix.5.md)）を指すが、[Appendix 6](appendix.6.md)ではネットワークの重み全体に読み替えて扱う。 |
| **自然勾配** (Natural Gradient) | <a id="kw-natural-gradient"></a>フィッシャー情報行列を用いて、確率分布の空間での等しい変化量を目指す勾配補正手法。 |
| **統計多様体** (Statistical Manifold) | <a id="kw-statistical-manifold"></a>確率多様体にFisher-Rao計量と双対な接続（e-接続・m-接続）が装備された多様体。点は分布そのものに対応し、情報幾何学の主舞台となる（[第4回](04.md)で詳述）。 |
...
## [第1回: かつての地図: 平らな世界で戦っていた私たち](01.md)
...
| **線形** | 加法性と同次性を満たす写像の性質（ $f(a\mathbf{x}+b\mathbf{y}) = af(\mathbf{x}) + bf(\mathbf{y})$ ）。直線を直線に、平面を平面に移す。 |
| **非線形** | 線形でないこと。比例と足し合わせ（直線や平面）では記述できない構造や変換を指す。 |
...
| **固有顔 (Eigenfaces)** | 顔画像をPCAで低次元表現し、比較や認識を行う画像処理への応用例。 |
| **中心化** | 各次元の平均を引き、重心を原点に移す前処理。PCAの標準手順。 |
| **共分散行列** | 変数間の分散と共分散をまとめた行列。PCAでは固有値分解の対象となる。 |
...
| **スイスロール** | 曲がった多様体の典型例として用いられる人工データ。 |
...
| キーワード | 説明 |
| --- | --- |
| **マージン最大化** | 二つのクラスを隔てる境界と、最も近いデータ点（サポートベクター）との距離を最大化するSVMの中心アイデア。 |
...
| キーワード | 説明 |
| --- | --- |
| **曲がった多様体** | スイスロールのように、ユークリッド空間内で巻いたり曲がったりしているデータ本来の形。 |
| **ノルムの呪い** | ベクトルの「長さ」と「方向」が区別されず、意味や確信度が混同されていた問題。 |
| **順序の喪失** | BoWのように、言語が持つ「時間の流れや文脈」を捨ててしまっていた点。 |
| **コサイン類似度** | <a id="kw-cosine-sim"></a>ベクトルのなす角に基づく類似度尺度。角度の重要性の文脈で登場。 |

## [第2回: ノルムの呪い: 意味と不確実性の未分化](02.md)
...
| **確信度の偽装** | Softmaxの出力（確率の尖り方）が、実際の確信度ではなく、単なるロジットのスケール（ノルム）に左右される問題。 |
| **Layer Normalization** | <a id="kw-layer-norm"></a>各トークンの特徴方向について平均と分散（またはRMS）を正規化し、活性のスケール変動を抑える。結果として表現のスケール（ノルムの暴走）を緩和し得るが、L2ノルムを厳密に一定値に固定するわけではない。（ただし、RMSNormでは各ベクトルの方向のみを保持し、スケールを一定化する側面が強い） |
...
## [第3回: プラネタリウムの建設: 球面へのパラダイムシフト](03.md)
...
## [第4回: 分類を確率で読む: Softmaxと情報幾何学](04.md)
...
| **確率多様体** (Probability Manifold) | 確率分布の集合を滑らかな多様体として捉えたもの。本文では $\text{int}(\Delta^{K-1})$ がその典型例。座標・接線空間・滑らかさのみを持つ「素の多様体」。 |
| **統計多様体** | → [［再掲・第0回］](#kw-statistical-manifold)。確率多様体にFisher-Rao計量と双対接続を装備したもの。確率多様体が「用紙」なら統計多様体は「縮尺と方位が書かれた地図」。 |
...
| **情報量** (Self-information / Surprisal) | <a id="kw-self-info"></a> $-\log p(x)$ で定義される、ある出来事の「珍しさ」の尺度。確率1で0、確率0で無限大。クロスエントロピー損失の正体であり、エントロピーはその期待値（[第7回](07.md)）。 |
...
| **分配関数** (Log-Partition) | <a id="kw-log-partition"></a>すべての候補を正規化する項。SoftmaxやCRFで $\log \sum \exp$ として現れる。 |
| **自然勾配** | → [［再掲・第4回］](#kw-natural-gradient) |
| **フィッシャー情報行列** | → [［再掲・第4回］](#kw-fisher-info) |
| **Chentsovの定理** | 統計的な粗視化（マルコフ写像）で距離が増えないという条件を満たす、確率分布の空間のリーマン計量は、定数倍を除いてFisher計量に限るという一意性定理。Fisher計量の選択が恣意でないことの根拠。 |
...
| **最大マージン原理** | 決定境界とデータの最小距離を最大化し、汎化性能を高めるSVMの中心思想。 |
...
## [第6回: Transformerという測量士: 動的な接続](06.md)
...
| **Procrustes整列** | 対応点ペアを用いて、2つの埋め込み空間の回転や反転を最適化する整列手法。 |
| **ハルシネーション** | 低確信度領域で、根拠の薄い出力を確信的に生成してしまう現象（無知の捏造）。「確信した嘘」はAppendix.2で扱う。 |
...
| **ベクトル場** | <a id="kw-vector-field"></a>各点に対して「次にどの方向に動くべきか」を指示する関数。ベクトル場として見たときのニューラルネットワークの役割。 |
...
| **Batch Normalization** | ミニバッチ内の平均と分散で正規化する手法。 |
...
| **確率微分方程式 (SDE)** | ドリフト項と拡散項を持つ確率過程の記述。拡散モデルの順過程と逆過程の基礎。 |
...
| キーワード | 説明 |
| --- | --- |
| **学習の熱力学理論** | 学習をパラメータ分布の時間発展として捉え、損失、更新、散逸を熱力学的量として再解釈する枠組み。 |
| **有限時間輸送** | 学習を、初期分布から学習済み分布への有限時間での輸送問題として定式化する視点。 |
| **認識論的コスト (Epistemic Costs)** | 外部データから情報を獲得して不確実性を減らす際に必要となる、理論的な物理コスト。 |
| **ランダウアーの原理** | 情報の消去に最低限のエネルギーコストが伴うという原理。学習における忘却コストの解釈に対応する。 |
| **エントロピー生成（散逸）** | 準静的でない学習更新により生じる不可逆な損失。理想的な可逆過程からの乖離を表す。 |
| **熱力学的仕事** | パラメータ分布を強制的に変形させるために必要な仕事。損失関数をその上界として解釈する。 |
| **学習の不可逆性（時間の矢）** | 収束した学習状態が自然には初期ランダム状態へ戻らないという、学習過程の方向性。 |
| **Fokker-Planck方程式** | 確率分布の時間発展を記述する方程式。生成（ $\mathbf{x}$ 空間）と学習（ $\boldsymbol{\theta}$ 空間）の類似構造を与える。 |
...
| **One-shot推論** | 中間ステップを介さず、入力から出力へ直接ジャンプする推論モード。単純でパターン認識的な問題に向く。 |
...
| **Self-reflection / Verifier** | 生成結果を検証し、必要に応じて修正する検証と再生成の枠組み。 |
...
## [第11回: 異なる空間をつなぐ: マルチモーダル表現の幾何学](11.md)
...
## [第13回: 高次元の深淵: 幾何学的な恐怖と祝福](13.md)

高次元空間に特有の集中現象と、それを活用または回避する設計指針。
...
## [第14回: トポロジーという顕微鏡: 穴とループの発見](14.md)
...
| **持続的ホモロジー** | データのスケールを変化させながら、そこに出現し消失する「穴（不変量）」を追跡し、データの真の構造を探る手法。 |
...
## [第15回: 次の空間を設計する: 幾何学的帰納バイアスと未解決問題](15.md)

未解決問題と未来の設計課題を整理し、次の座標系を構想する最終回。

| キーワード | 説明 |
| --- | --- |
| **統一多様体** | 複数モダリティを単一の空間に埋め、意味的な共通表現を形成できるかという仮説的な問い。 |
| **モダリティ・ブリッジ** | 異なる曲率や次元の空間同士を翻訳し接続する発想。 |
...
| **低次元射影の限界** | 高次元多様体の構造を保った可視化が難しいという課題。 |

## [Appendix 1: 量子化の幾何学: 重みの離散化が多様体に何をするのか](appendix.1.md)
...
## [Appendix 2: 多様体の純度問題: 幾何学がまだ解けていない課題](appendix.2.md)
...
| **形式の安定と内容の真偽** | 正規化（球面化）は計算形式を安定させるが、その方向が「正しい」かどうか（真偽）には中立であるという原理的限界。 |
| **「正しさ」の3類型** | 論理整合性（推論規則）、事実整合性（世界知識）、規範整合性（価値判断）の区別。 |
| **影響度** | 特定のサンプルを除去または変更した際の、検証誤差や推論安定性の変化量。 |
...
| **Memory Wall** | メモリ容量や帯域の制約が計算性能のボトルネックになる問題。 |
...
## [Appendix 4: 空間の「物差し」再考: 2点間から情報の密度まで](appendix.4.md)
...
## [Appendix 5: 情報幾何学における双対構造: 2種類のまっすぐ](appendix.5.md)
...
| **計量整合性** (Metric Compatibility) | 平行移動で内積（長さと角度）が保存される性質。一般に $\alpha \neq 0$ では失われる。 |
...
## [Appendix 6: 特異点の幾何学: AIはなぜ汎化するのか](appendix.6.md)
...
| **同定不能性** (Non-identifiability) | 異なるパラメータが同一の分布（関数）を表してしまう性質。 |
...
| **ブローアップ** (Blow-up) | 特異点を方向ごとに展開して交差や退化をほどく代数幾何の基本操作。 |
