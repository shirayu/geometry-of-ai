...
「重みを極端に離散化すると多様体の形状にどう影響するか」という問いは、2024年以降の1-bit/2-bit LLM研究の広がりとともに、実務・理論の両面で重要性を増している。講義本編では第4回補論「離散と連続の界面」でSTEによる勾配近似を扱い、第13回「スパース性の幾何学」でパラメータの疎性を扱った。本Appendixはその延長線上にあるが、扱う離散化の程度が桁違いに大きい。連続空間を格子に切り詰めるという操作が、多様体という幾何学的対象に何をするのか。崩れ方の不均一さまで含めて追っていく。
...
三つとも、答えは「場合による」で止まっている。位相的性質が保たれるかどうかが層やタスクに依存するという一点だけは、はっきりしている。逆に言えば、量子化の影響は多様体上で一様には出ない。ならば、多様体の形が最初から限定されている場合はどうか。

### 球面上の量子化（nGPTとの接続）

第3回で扱ったnGPTでは、表現は球面上に制約されていた。自由な高次元空間ではなく、ノルム1という束縛の下にある多様体である。この制約が、量子化の挙動をどう変えるだろうか。
...
forwardとbackwardで別の空間を使うという、この一見都合のよいごまかしが実際に学習を回せてしまう。離散と連続の境界がどこまで妥協を許すかは、第4回補論で見た通りである。
