...
**パーシステントホモロジー（Persistent Homology）** は、この問いに対する洗練された回答を与える。「どのスケールか」を一つに決めるのではなく、スケールそのものを動かしてしまう。そのアイデアは：
...
深層学習モデルは、入力データを高次元の表現空間に写像する。この表現空間のトポロジー的構造と、モデルの性能や振る舞いの間に **相関が観察される** ことがある。トポロジーが性能を左右しているのだと言い切りたくなるが、そう単純には結論できない。これが因果関係（トポロジーが性能を決める）なのか、単なる相関なのか、あるいは共通の原因による見かけの関係なのかは、多くの場合まだ明らかではない。
