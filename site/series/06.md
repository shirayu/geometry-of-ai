...
> [!IMPORTANT]
> **標準TransformerとL2正規化の違い：** 標準Transformerでは、LayerNormやRMSNormが使われるが、これらは **平均と分散の正規化** であり、ベクトルを単位ノルムにする **L2正規化とは異なる**。したがって、 $\mathbf{Q}^\top \mathbf{K}$ は純粋な角度情報（ $\cos\theta$ ）ではなく、**方向と大きさの両方の影響を受ける**（ただし高次元空間ではLayerNorm後のベクトルのノルムが確率的にある範囲に集中する傾向があり、「実質的には角度に近いものを見ている」という近似的解釈も現場の直感としては役立つ）。この点は注意事項で述べた通りであり、以下ではこの区別を前提に、いつ厳密な角度計算になるかを見ていく。
