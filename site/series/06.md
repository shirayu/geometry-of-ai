...
本回では、このAttention機構を幾何学的に再解釈する。特に、正規化の有無が解釈にどう影響するかを明確にしながら、Query-Key-Value機構の働きを捉え直す。

## Transformerの全体像

本節では、後続の議論で登場するTransformerの各構成要素を整理する。各要素が本講義のどこで幾何学的に再解釈されるかも合わせて示す。
...
ここまで、Attentionを「測量士」として捉えてきた。測量士のメタファーは、「入力に応じて動的に計量を測り直す」というAttentionの骨子を直感的に伝える。ここからは、この測量という営みを、Query-Key-Valueの対応がより鮮明になる**天体観測**として描き直す。測量士が基準点を測るように、観測者は星を観測する——同じ「動的な観測」を、より具体的なQKVの言葉に翻訳するのが本節である。
...
実際の高次元空間では、ベクトルを2次元ずつのペアに分け、ペアごとに **回転させるスピード（周波数 $\theta_i$ ）** を変える。
...
標準TransformerのAttentionは、厳密には「角度を計算している」わけではない。LayerNormやRMSNormは平均と分散の正規化であり、L2正規化とは異なる。したがって、内積には方向と大きさの両方が影響する。
