...
第1回で見たように、古典的な機械学習手法は「固定された特徴空間」を前提としていた。PCAは固定された主成分軸を、SVMは固定されたカーネル関数を使う。入力が何であれ、空間の構造は変わらない。同じ地図を、誰に対しても広げて見せるようなものだった。
...
Attentionが「動的」であることの意味は、何と比べるかによって輪郭が変わる。比較対象として都合がいいのは、第1回で触れたカーネル法である。もう少し詳しく振り返っておこう。
...
回転にせよ正規化にせよ、ここまで見てきた工夫はQueryとKeyの側の話だった。しかし、内積を確率に変える最後の一段にも、調整すべき余地が残っている。第4回で見たように、Softmaxには温度パラメータがある。Attentionの文脈では、スケーリング係数 $\sqrt{d_k}$ が分布の鋭さに影響する。
