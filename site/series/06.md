...
> [!TIP]
> 読了後に、[理解度チェック](./06/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#第6回-transformerという測量士-動的な接続)でも確認できる。

## 注意事項

- 標準Transformerの $Q, K$ は単位ノルム化されていない。LayerNorm/RMSNormは平均と分散の正規化であり、 $L_2$ 正規化（単位ノルム化）とは別物である。
...
[第1回](01.md)で見たように、古典的な機械学習手法は「固定された特徴空間」を前提としていた。PCAは固定された主成分軸を、SVMは固定されたカーネル関数を使う。入力が何であれ、空間の構造は変わらない。同じ地図を、誰に対しても広げて見せるようなものだった。

Transformerは、この前提を根本から覆した。

Attention機構は、入力に応じて「どこを見るか」を動的に決定する。地図を固定して読むのではなく、その場その場で測量をやり直す。入力が変われば、測量結果も変わり、空間の見え方自体が変化する。同じ単語列でも、前後の文脈が違えば、測り直された空間はまったく別の形になる。

では、この「測り直し」は何を測っているのか。Query、Key、Valueという三つ組みは、しばしば検索の比喩で説明される。だが、その比喩がどこまで正確なのかは、正規化の有無という一点にかかっている。本回では、このAttention機構を幾何学的に再解釈しながら、その一点を明確にしていく。

## Transformerの全体像

Attentionの幾何学に踏み込む前に、Attentionが埋め込まれている構造そのものを見ておきたい。Attention単体を切り出して語ると、それが担っている役割の輪郭を見失いやすい。
...
| --- | --- | --- | --- |
| **トークン埋め込み** | 離散トークンを $d$ 次元ベクトルに変換 | 「記号を空間に配置する」 | [第2回](02.md)（ノルム）、[第3回](03.md)（球面） |
| **位置埋め込み** | トークンの順序情報を付与 | 「星に座標を与える」 | 第6回（RoPE：本回） |
| **Multi-Head Attention** | 入力に応じてトークン間の関係を動的に計算 | 「望遠鏡で星空を測量する」 | 第6回（本回）、[第4回](04.md)（Softmax：情報幾何学） |
| **FFN（Feed-Forward Network）** | 各トークンの表現を非線形に変換 | 「観測データを処理する」 | [第8回](08.md)（ベクトル場） |
| **残差接続** | 入力をショートカットで加算し、勾配の流れを安定させる | 「元の情報を保持しながら更新する」 | [第8回](08.md)（Neural ODE） |
| **正規化（LayerNorm等）** | 表現のスケールを安定させる | 「計器のキャリブレーション」 | [第3回](03.md)（nGPT） |
| **出力層** | タスクに応じた最終変換（分類ならSoftmax等） | 「測量結果を報告する」 | [第5回](05.md)（ArcFace） |

> [!IMPORTANT]
> **AttentionとFFNの分業：** Attentionは「トークン間の情報交換（どの情報を集めるか）」を担い、FFNは「各トークンの表現変換（集めた情報をどう加工するか）」を担う。この分業構造がTransformerの基本設計である。[第3回](03.md)で見たnGPT（[Loshchilov et al., 2025](#ref-loshchilov2025)）は、この両方の出力に球面制約を課す設計だった。また、近年のMoE（Mixture of Experts）ではFFN部分を動的に切り替える設計も登場している（[第13回](13.md)で触れる）。
...
Attentionが「動的」であることの意味は、何と比べるかによって輪郭が変わる。比較対象として都合がいいのは、[第1回](01.md)で触れたカーネル法である。もう少し詳しく振り返っておこう。
...
カーネル関数の形自体は、データを見る前に決まっている。これがカーネル法の根本的な制約であり、**特徴空間の構造が入力に依存しない**という一文に集約される。

RBFカーネルは常に「ユークリッド距離に基づく類似度」を計算する。入力がテキストであろうと画像であろうと、同じ関数形を使う。すべての入力に対して同じ「地図」を広げているようなものだ。
...
> [!NOTE]
> **カーネル法との関係（[Tsai et al., 2019](#ref-tsai2019)）：** 上の表は「カーネル法 vs Attention」という対立として整理しているが、これは著者による説明上の構図である。Tsai et al. (2019) の立場はむしろ逆で、「Attentionはカーネル平滑化（kernel smoothing）として定式化できる」、つまり**Attentionはカーネルの一種**という**包含関係**を主張している。本節の対立図式はカーネル法の固定性という制約を浮き彫りにするための導線であり、Tsai et al. の主張をそのまま反映したものではない。

## Attention機構：動的な測量システム

### Query-Key-Valueの直感

固定された地図がないなら、Attentionは何を手がかりに「どこを見るか」を決めているのか。手がかりを持たない測量士はいない。**測量**のメタファーで考えてみよう。

測量士は、未知の地形を測定するために、複数の基準点との距離や角度を測る。その測定結果を統合して、自分の位置や地形の構造を推定する。

> [!CAUTION]
> **測量士メタファーの賞味期限：** 測量士の測定は、あくまで個々の基準点との局所的な関係（距離・角度）の集まりである。Attentionが実際に行っているのは、全トークン間の関係を一度に、大域的に計算し直す操作であり、局所的な測定の積み重ねでは捉えきれない。この比喩は「複数の手がかりを統合して現在地を推定する」という直感を運ぶための道具であり、Attentionのグローバルな相互参照性そのものを説明するものではない。
...
測量士の比喩は直感を運んでくれるが、実際の計算がどこまでその比喩に忠実かは、まだ確かめていない。標準的なTransformerのSelf-Attentionは、以下のステップで計算される。
...
$\sqrt{d_k}$ はKeyとQueryの次元数の二乗根で、これによるスケーリングは、内積の分散を安定させるためのものである。

> [!IMPORTANT]
> **標準TransformerとL2正規化の違い：** 標準Transformerでは、LayerNormやRMSNormが使われるが、これらは **平均と分散の正規化** であり、ベクトルを単位ノルムにする **L2正規化とは異なる**。
> 学習可能なアフィン変換と $\varepsilon$ を無視すれば、LayerNorm直後のベクトルのノルムはおおむね $\sqrt{d}$ に固定される。
> しかし、その後の $W_Q$ と $W_K$ による射影はQueryとKeyのノルムを変えるため、 $\mathbf{Q}^\top \mathbf{K}$ は **方向と大きさの両方の影響を受ける**。
> 「実質的には角度に近い」という近似が有効なのは、射影後のQueryとKeyのノルムが実際に狭い範囲へ揃っている場合であり、LayerNormだけから一般には保証されない。
> 以下ではこの区別を前提に、いつ厳密な角度計算になるかを見ていく。

### 球面解釈が成り立つ条件

角度と大きさが混ざっているなら、「Attentionは角度を計算している」という言い方は、どこまで許されるのか。厳密に成り立つのは、実は特定の条件下だけである。
...
測量士は、基準点が誰であるかをあらかじめ知っている。だが、AttentionにおけるKeyは、測る前から「どの基準点が重要か」を教えてくれるわけではない。測量のメタファーだけでは、この「何を基準点として選ぶか」という一段階が見えにくくなる。

### なぜ「辞書」ではなく「天体観測」か

Attention機構の説明として、「Queryは検索クエリ、Keyはインデックス、Valueは内容」という**辞書メタファー**がよく使われる。これは初期のメモリネットワークや、エンジニアに馴染み深いKey-Valueストアからの類推であり、実装の対応関係だけを見れば間違ってはいない。

しかし、この比喩には限界がある。辞書は「AならB」という1対1の対応を前提とするが、Attentionは**連続的な類似度**に基づいて**すべてのValueを混合**する。「持った」を引けば「持った」の項が返ってくる、という発想では、Attentionが実際にやっていることの半分しか説明できない。
...
> [!NOTE]
> **ハルシネーションの幾何学的理解：** 辞書観では「検索ミス」としか理解できないハルシネーションも、天体観測観では「観測データの密度が低い領域で、Attentionが変な方向に歪み、存在しない星（意味）を観測してしまった」と理解できる。これは「空間の歪みをどう補正するか」という議論への入口を開く。

## Multi-head Attention：複数の視点

### なぜ複数のヘッドが必要か

一つの望遠鏡、一つのフィルターで観測できる星空は、一種類でしかない。Single-head Attentionは、一つの「見方」しか持たない。しかし、言語には複数の側面がある。文法的な関係、意味的な関係、照応関係など、異なる種類の関係を同時に捉える必要がある。
...
> [!NOTE]
> **LDAとの比較（比喩として）：** [第1回](01.md)で触れたLDA（Linear Discriminant Analysis）は、固定されたトピック空間への射影を行う。Multi-head Attentionは、入力ごとに変化する「見方」の集合を学習するという意味で、LDAの動的・連続版と見なせる。ただし、これは直感的な比喩であり、最適化問題として同一ではない。
...
QKVの積和だけを見ると、Attentionには奇妙な欠落がある。Self-Attentionは、入力の順序を区別しない。「犬が猫を追いかけた」と「猫が犬を追いかけた」は、単語の集合としては同じだが、意味は異なる。位置情報を何らかの形で導入する必要がある。

初期のTransformer（[Vaswani et al., 2017](#ref-vaswani2017)）は、正弦波による **絶対位置埋め込み** を使った。しかし、近年の大規模言語モデル（Llama, Qwen, Mistral等）では、**RoPE（Rotary Position Embedding）** が広く採用されている。

> [!NOTE]
> **位置埋め込みの多様性：** RoPEは近年の多くのLLMで採用されているが、唯一の選択肢ではない。ALiBi（[Press et al., 2022](#ref-press2022)）のように、Attention重みに位置依存のバイアスを加える方式の採用例もある。最適な位置埋め込みはタスクやモデル設計に依存する。

### RoPEの仕組み：回転による相対位置

RoPE（[Su et al., 2024](#ref-su2024)）のアイデアは、位置を**回転**として表現することである。
...
ここで、このペアの回転角 $\theta_i = 0.5$ （約28度）と仮定し、Queryベクトル $\mathbf{q} = (1, 0)$ の変化を追ってみる。
...
2次元の円周上の回転だけでは、まだRoPEの本領は見えない。実際の高次元空間では、ベクトルを2次元ずつのペアに分け、ペアごとに **回転させるスピード（周波数 $\theta_i$ ）** を変える。

例えば、 $d=128$ 次元のモデルで、2つの異なるペア（次元）の動きを比較してみる。

1. **「速い」回転のペア（低次数の次元）**: 各ペアの周波数 $\theta_i$ が大きく設定される。**役割**: 「隣の単語か、その隣か」といった、至近距離の微細な位置関係の変化に敏感に反応する。
2. **「遅い」回転のペア（高次数の次元）**: 各ペアの周波数 $\theta_i$ がごく小さく設定される。**役割**: 文全体を俯瞰し、長距離の大まかな位置関係を捉える。
...
回転で相対位置だけが残る、という数式の帰結は、天体観測に置き換えるとむしろ当たり前の話に見えてくる。
...
> [!NOTE]
> **対称性の幾何：** 2次元の回転全体の集合 $\text{SO}(2)$ は、**リー群（Lie Group）** と呼ばれる構造を持つ。リー群とは、回転などの「対称変換」の集合が、それ自体として滑らかな多様体になっているものを指す。 $\text{SO}(2)$ は円周 $S^1$ と同じ形をしており、RoPEの各次元ペアに適用される回転行列 $R_m$ は、この連続な回転群 $\text{SO}(2)$ から位置 $m$ に応じてサンプリングされた要素である。
>
...
>
> 本講義では、リー群の理論に深入りはしない。しかし、「変換自体が幾何学的な空間をなす」という視点は、RoPEの設計がなぜ数学的にうまくいくのかの理解を助ける。回転という操作が、単なる計算テクニックではなく、対称性という深い数学的構造に根ざしていることを知っておくだけで十分である。

## Softmaxの役割：連続から離散への橋渡し

### Attentionにおけるスケーリング

回転にせよ正規化にせよ、ここまで見てきた工夫はQueryとKeyの側の話だった。しかし、内積を確率に変える最後の一段にも、調整すべき余地が残っている。[第4回](04.md)で見たように、Softmaxには温度パラメータがある。Attentionの文脈では、スケーリング係数 $\sqrt{d_k}$ が分布の鋭さに影響する。

$$\text{softmax}\left(\frac{\mathbf{Q} \mathbf{K}^\top}{\sqrt{d_k}}\right)$$

$\sqrt{d_k}$ によるスケーリングは、**主に内積の分散を安定化するため**に導入された（[Vaswani et al., 2017](#ref-vaswani2017)）。 $d_k$ が大きいと、内積の値も大きくなりやすく、Softmaxが極端に鋭くなって勾配が消失する問題がある。 $\sqrt{d_k}$ で割ることで、次元に関わらず内積のスケールを揃える。
...
倍率を合わせたら、あとは実際に光を集める番である。Attention重みが計算されると、Valueの加重和として出力が生成される：
...
## まとめ：Attentionの幾何学的統一

測量士から始まり、天体観測、回転、確率へと話を広げてきた。ここで一度、扱った道具を並べ直しておく。
...
測量士から天体観測者へと比喩を重ねてきたが、その比喩がどこまで数式に裏打ちされているかは、正規化の有無で答えが変わる。**「どの幾何学か」は設計次第**である（詳細は注意事項と「球面解釈が成り立つ条件」を参照）。Cosine AttentionやnGPTのように明示的にL2正規化する設計では「 $Q^\top K = \cos\theta$ 」が厳密に成り立ち、標準設計はこの純粋形からのずれとして位置づけられる。

RoPEは、位置情報を「回転」として表現する設計であり、本講義のテーマ（角度、回転、球面）と深く整合する。絶対位置ではなく相対位置に基づくAttentionを可能にし、近年の多くのLLMで広く採用されている。正規化の有無をめぐる留保とは独立に、この設計だけは角度の幾何学に素直に乗る。
...
- <a id="ref-su2024"></a>Su, J., Lu, Y., Pan, S., Murtadha, A., Wen, B., & Liu, Y. (2024). RoFormer: Enhanced Transformer with Rotary Position Embedding. *Neurocomputing*, 568, 127063. arXiv: [2104.09864](https://arxiv.org/abs/2104.09864) (2021)
    - RoPEの原論文。回転による相対位置表現を提案。最初の提案は2021年（arXiv）、2024年にNeurocomputing誌に掲載。
- <a id="ref-press2022"></a>Press, O., Smith, N. A., & Lewis, M. (2022). Train Short, Test Long: Attention with Linear Biases Enables Input Length Extrapolation. *ICLR 2022*. arXiv: [2108.12409](https://arxiv.org/abs/2108.12409)
...
- <a id="ref-tsai2019"></a>Tsai, Y.-H. H., Bai, S., Yamada, M., & Morency, L.-P. (2019). Transformer Dissection: A Unified Understanding of Transformer's Attention via the Lens of Kernel. *EMNLP 2019*. arXiv: [1908.11775](https://arxiv.org/abs/1908.11775)
