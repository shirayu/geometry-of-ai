...
2. **「遅い」回転のペア（高次数の次元）**: 各ペアの周波数 $\theta_i$ が非常に小さく設定される。**役割**: 文全体を俯瞰し、長距離の大まかな位置関係を捉える。
...
>
> 本講義では、リー群の理論に深入りはしない。しかし、「変換自体が幾何学的な空間をなす」という視点は、RoPEの設計がなぜ数学的にうまくいくのかを理解する鍵となる。回転という操作が、単なる計算テクニックではなく、対称性という深い数学的構造に根ざしていることを知っておくだけで十分である。
