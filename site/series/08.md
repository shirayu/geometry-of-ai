...
> [!TIP]
> 読了後に、[理解度チェック](./08/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#第8回-時間の発見-一撃からの脱却)でも確認できる。
...
これまでの講義では、表現空間の「形」を議論してきた。球面、双曲空間、分布としての埋め込み。だが、これらはすべて**静的な描像**である。星空のスナップショットは撮れる。星の動きを追うことは、まだできない。

深層学習の歴史を振り返ると、興味深い転換点がある。初期の生成モデル（GAN、VAE）は、潜在空間から出力空間への**一撃の変換**を行っていた。魔法使いが呪文を唱えると、何もないところから画像が「召喚」される。中間過程は見えない。見せる必要すら、なかった。

ところが、近年の主流は異なる。拡散モデルは、ノイズから徐々に意味が立ち上がる**プロセス**を明示的に扱う。Chain of Thoughtは、推論の過程を**軌跡のように見える形で**言語化する。一撃で済んでいたはずのものが、なぜわざわざ何段階にも分解されるようになったのか。
...
本回では、この「時間の発見」を幾何学的に扱う。**空間の安定化**が**時間発展の制御**を可能にした、と解釈できるという洞察がその軸である。

## 古典的生成モデルの構造と限界

### GANの構造

**GAN（Generative Adversarial Network）**（[Goodfellow et al., 2014](#ref-goodfellow2014)）は、生成モデルの革命だった。その構造は驚くほどシンプルである。
...
**VAE（Variational Autoencoder）**（[Kingma & Welling, 2014](#ref-kingma2014)）は、確率的な枠組みを導入した。
...
これらのモデルは画期的だった。それでも、共通の限界を抱えていた。
...
一撃変換の限界、特に学習の不安定性は、なぜ生じるのか。幾何学の観点から理解できる。

#### ノルムの暴走

[第2回](02.md)で議論したように、正規化されていない空間では、ベクトルのノルムが学習中に暴走しやすい。
...
**残差接続**（[He et al., 2016a](#ref-he2016)）は、深層学習の安定性を劇的に改善した。

$$\mathbf{h}_{t+1} = \mathbf{h}_t + f(\mathbf{h}_t)$$

従来の理解では、これは「勾配消失問題の緩和」として説明される。スキップ接続により、勾配が直接浅い層に伝播できる。この説明は正しい。しかし、それだけではない。幾何学的に見ると、もう一つの顔が浮かび上がる。
...
> [!NOTE]
> **ベクトル場とベクトル値関数の違い：** $f$ はベクトルを返す関数なので「ベクトル値関数」と呼ぶこともできる。しかし「ベクトル場」と呼ぶとき、強調したいのは**空間のすべての点にベクトル（矢印）が対応している**という大域的な構造である。天気図の風向図や川の流速分布をイメージするとよい。「この地点に来たら、どの方向へ、どのくらいの速さで進むか」という指示書が、空間全体に敷き詰められている状態である。単なる写像（入力を一撃で出力に変換するテレポート）とは異なり、ベクトル場は**局所的な「押し出し方向」を累積することで変換を実現する**という点で異なる。
...
式の左側にはいつも $\mathbf{h}_t$ がそのまま残っている。この項は、いったい何をしているのか。

**記憶保持機構**：入力の情報を、変換を経ても保持する。 $f(\mathbf{h}_t)$ が何をしようと、 $\mathbf{h}_t$ の成分は残る。

**恒等写像への近さ**： $f(\mathbf{h}_t) \approx 0$ のとき、 $\mathbf{h} _{t+1} \approx \mathbf{h}_t$ となる。つまり、「何もしない」という選択肢が常にある。これにより、深い層でも浅い層の情報が失われにくい（[He et al., 2016b](#ref-he2016identity)）。
...
**Batch Normalization**（[Ioffe & Szegedy, 2015](#ref-ioffe2015)）：ミニバッチ内で平均と分散を正規化。

**Layer Normalization**（[Ba et al., 2016](#ref-ba2016)）：各サンプル内で正規化。Transformerで広く使用。

**RMSNorm**（[Zhang & Sennrich, 2019](#ref-zhang2019)）：平均を引かず、二乗平均平方根で割る。LLMで採用例多数。
...
> [!CAUTION]
> **LayerNormとL2正規化の違い：** [第6回](06.md)でも強調したが、LayerNorm/RMSNormは平均と分散の正規化であり、ベクトルを単位ノルムにするL2正規化とは異なる。LayerNormを通過したベクトルは、一般に単位ノルムではない。

## 概念整理：写像からベクトル場へ

同じ「関数 $f$ を使う」という点は共通していても、それをどう扱うかで、これまで登場したモデルは大きく二つの流派に分かれる。「写像」と「ベクトル場」である。両者とも関数 $f$ を用いるが、それをどう反復（離散の場合）または積分（連続の場合）してプロセスを定義するかが根本的に異なる。
...
- 直感イメージ：**「海流」**。入力 $\mathbf{x}$ は、その地点にある矢印に従って「少しずつ押し流される」。微小な変化を足し合わせ続ける（積分・反復する）ことで、初めて移動というプロセスが生じる。
    ...
    - ただし実際の海流とは異なり、この「流れ」 $f$ は質量保存やナビエ・ストークス方程式のような物理的制約に従うわけではない。学習によって自由に形作られる、単なる方向場である。
...
離散のステップ幅を、どこまで小さくできるのか。ResNetの残差接続を、時間ステップ $\Delta t = 1$ のオイラー法と見なすと：
...
**Neural ODE**（[Chen et al., 2018](#ref-chen2018)）は、この連続時間の枠組みを直接扱う。
...
$$
\begin{aligned}
h(0) &\overset{(1)}{\to} h(t) \overset{(2)}{\to} h(T)
\end{aligned}
$$

ここで、 $\overset{(1)}{\to}$ はベクトル場 $f(h,t,\theta)$ に従った時間発展、 $\overset{(2)}{\to}$ は ODE solver による積分を表す。
...
ResNetは「層」という固定された離散ステップで状態を変換する。シェフが決まった工程数で料理を仕上げるようなものだ。工程を増やしたければ、材料（パラメータ）を増やすしかない。融通は利かない。
...
### Neural ODEの跳躍：何が革命だったのか

Neural ODE（[Chen et al., 2018](#ref-chen2018)）は NeurIPS 2018 Best Papers の一つに選ばれ、ResNetを「微分方程式の離散化」として捉える見方を広く共有させた。
その革命の正体は、ニューラルネットワークと常微分方程式を同じ枠組みで扱えるようにした点にある。一見遠く見えるこの二つの分野が、実は同じ言葉で語れる。ここから得られる洞察を三つに整理する。
...
これは何を意味するか。**外部の高精度ソルバーをそのまま深層学習のフォワードパスとして使えるようになった**。精度保証、適応的なステップ幅、剛性への対応など、数値解析が長年かけて磨いてきた知見が、ニューラルネットワークの計算に直接流れ込んだ。ベクトル場が急峻な領域では細かく、滑らかな領域では粗く、この「難しいところだけ細かく計算する」という動作は、固定的な層の積み重ねでは実現できない。

#### 洞察3：勾配の計算もODEである（随伴法の本質）

最も衝撃的な洞察は、逆伝播の扱い方にある。

通常の深層学習では、逆伝播のために中間状態を保存しておく必要がある。100層のネットワークなら100個の状態を記憶しておく。Neural ODEは理論上これが不要になる。ここで、著者らは別の問いを立てた。**「勾配の計算もODEとして解けないか」**。

答えはイエスだった。損失 $L$ に対する勾配の時間発展は、「随伴状態（adjoint state）」 $\mathbf{a}(t) = \partial L / \partial \mathbf{h}(t)$ が満たすODEとして書ける：

$$\frac{d\mathbf{a}}{dt} = -\mathbf{a}(t)^{\top} \frac{\partial f(\mathbf{h}(t), t, \theta)}{\partial \mathbf{h}}$$

フォワードパスのODEを解くのと同じ仕組みで、バックワードパスも解ける。**「ODEを解く」という行為が、学習の全体をひとつの枠組みで扱えるようにした**のである。
...
$\overset{(1)}{\to}$ は「 $\frac{dh}{dt}=f(h,t,\theta)$ を満たす軌道を ODE solver で積分する」ことを表している。

$f$ はニューラルネットワークで表現され、 $\theta$ はその学習パラメータ。ODEソルバーが、 $t=0$ から $t=T$ まで状態を積分する。

### Neural ODEの表現力の限界とAugmented Neural ODEs

万能に見えたNeural ODEにも、超えられない制約がある。ODEの解は連続で、異なる初期条件から出発した軌道は交差しない（解の一意性）。これは、**トポロジー的に表現できない関数が存在する**ことを意味する。
...
**Augmented Neural ODEs**（[Dupont et al., 2019](#ref-dupont2019)）は、この制約を回避するために**状態空間の次元を拡張**する。入力を高次元空間に持ち上げることで、元の空間では交差するように見える軌道も、拡張された空間では交差しないように配置できる。
...
深層学習の言葉と、力学系の言葉は、どこまで重ねられるのか。力学系の言葉を借りると、表現空間は **位相空間（phase space）** に対応する。各点が「状態」を表し、時間発展は位相空間内の軌道として描かれる。

生成モデルの文脈では：

- **位相空間**：潜在空間 + 中間表現の空間
- **軌道**：生成プロセスにおける状態の遷移
- **アトラクター（比喩的対応）**：学習されたデータ分布。力学系の厳密なアトラクターの定義（軌道が漸近的に引き寄せられる不変集合）をそのまま満たすわけではなく、「軌道が最終的に落ち着く先」という直感を運ぶための対応づけである
...
2018年のNeural ODEの登場は、深層学習コミュニティで概念的な転換点として受け止められた。「深層ネットワークとODEが同じだった」という洞察は、その後の研究の方向性に大きな影響を与えた。一本の論文が、これほど広い地形を書き換えることは、そう多くない。

### 連続正規化フロー（FFJORD）

パラメータ共有と勾配のODE化だけでは終わらなかった。Neural ODEの発表から数か月後、**FFJORD**（[Grathwohl et al., 2019](#ref-grathwohl2019)）が登場した。

確率密度の連続時間変化を記述するには、ベクトル場 $f$ の **divergence（ヤコビアンのトレース $\mathrm{tr}(\partial f/\partial \mathbf{h})$ ）** を積分する必要がある。このトレースを厳密に求めるには、基底ベクトルを用いて対角成分を足し上げる必要があり、結果として $O(d)$ 回程度の自動微分（JVP/VJP）が必要になるため、高次元ではスケールしなかった。

FFJORDはここに**Hutchinson推定量**を組み合わせた。ランダムベクトルを使ったトレース推定により、divergenceを $O(d)$ に近い計算量で見積もれるようにした。高次元空間での正確な確率密度の学習は、次世代の生成モデルの基盤となる問いである。

### Latent ODE：時系列への展開

生成モデルだけが恩恵を受けたわけではない。同じく2018～2019年にかけて、**Latent ODE**（[Rubanova et al., 2019](#ref-rubanova2019)）が時系列データへの応用を開拓した。
...
Neural ODEの最も深い遺産は、**Flow Matching**（[Lipman et al., 2023](#ref-lipman2023); [Liu et al., 2023](#ref-liu2023flow)）として結実した。
...
「召喚術（一撃変換）」から「映画制作（軌道の学習）」へ。この比喩が単なる比喩ではなく、**数学的に厳密な枠組み**として定式化されたことこそが、Neural ODEの革命だった。ニューラルネットワークは「写像を近似するもの」から「流れを定義するもの」へと、その自己理解を更新した。

## 拡散モデルへの橋渡し

### 拡散モデルの直感

**拡散モデル**（次回詳述）は、Neural ODEの考え方をさらに発展させたものである（[Ho et al., 2020](#ref-ho2020)）。
...
深層学習における「時間の発見」は、一度の跳躍で起きたわけではない。二つの技術的進歩と、一つの概念的統合が積み重なって、ようやく実現した。
...
**ニューラルネットワークとODEの統合**：これが最も根本的な転換だった。「深層学習は写像を近似する」という自己理解から、「深層学習はベクトル場を学習し、それを積分することで変換を実現する」という理解へ。パラメータを共有した無限深さ、勾配計算のODE定式化、ODEソルバーとの統合。これらは、後の[FFJORD](#連続正規化フロー-ffjord)、[Latent ODE](#latent-ode-時系列への展開)、そして前述の[Flow Matching](#flow-matchingへの道)へと連なる道を開き、2018年の論文が現在の生成AIの根幹に系譜として直結していることが分かる。

この見方は、生成モデルのパラダイム変化を理解する一つのレンズを与える。一撃の「召喚術」から、徐々に形が立ち上がる「映画制作」へ。次回では、この「映画」の具体例として、拡散モデルとSDEを扱う。
...
[第9回](09.md)「拡散と凝縮」では、拡散モデルとSDEの対応を詳しく扱う。
...
独立に初期化したResNetとNeural ODEの出力ノルムを並べても、モデル構造の違いと重みの違いを分離できない。
そこで次の例では、同じ初期値と同じベクトル場を使い、一方を固定ステップのEuler残差更新、もう一方を適応的ODEソルバーで積分する。
ここで測る最終状態のL2差は、モデルの優劣ではなくEuler法の離散化誤差である。
一般にはステップを細かくすると差は小さくなるが、実際の値はベクトル場とODEソルバーの許容誤差にも依存する。
...
- <a id="ref-he2016"></a>He, K., Zhang, X., Ren, S., & Sun, J. (2016a). Deep Residual Learning for Image Recognition. *CVPR 2016*. arXiv: [1512.03385](https://arxiv.org/abs/1512.03385)
    - ResNetの原論文。残差接続が深いネットワークの学習を可能にすることを示した。
- <a id="ref-he2016identity"></a>He, K., Zhang, X., Ren, S., & Sun, J. (2016b). Identity Mappings in Deep Residual Networks. *ECCV 2016*. arXiv: [1603.05027](https://arxiv.org/abs/1603.05027)
    - Pre-activation ResNetの提案。残差接続の設計についてさらに分析。

### 正規化層

- <a id="ref-ioffe2015"></a>Ioffe, S., & Szegedy, C. (2015). Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift. *ICML 2015*. arXiv: [1502.03167](https://arxiv.org/abs/1502.03167)
    - Batch Normalizationの原論文。
- <a id="ref-ba2016"></a>Ba, J. L., Kiros, J. R., & Hinton, G. E. (2016). Layer Normalization. arXiv: [1607.06450](https://arxiv.org/abs/1607.06450)
    - Layer Normalizationの原論文。Transformerで広く使用される。
- <a id="ref-zhang2019"></a>Zhang, B., & Sennrich, R. (2019). Root Mean Square Layer Normalization. *NeurIPS 2019*. arXiv: [1910.07467](https://arxiv.org/abs/1910.07467)
    - RMSNormの原論文。大規模LLMで採用例が増加。

### Neural ODE

- <a id="ref-chen2018"></a>Chen, R. T. Q., Rubanova, Y., Bettencourt, J., & Duvenaud, D. (2018). Neural Ordinary Differential Equations. *NeurIPS 2018*. arXiv: [1806.07366](https://arxiv.org/abs/1806.07366)
    - Neural ODEの原論文。連続深度モデルと随伴法による効率的な学習を提案。NeurIPS 2018 Best Papersの一つに選出。
- <a id="ref-dupont2019"></a>Dupont, E., Doucet, A., & Teh, Y. W. (2019). Augmented Neural ODEs. *NeurIPS 2019*. arXiv: [1904.01681](https://arxiv.org/abs/1904.01681)
    - Neural ODEの表現力の限界と、次元を拡張することによる解決策。
- <a id="ref-grathwohl2019"></a>Grathwohl, W., Chen, R. T. Q., Bettencourt, J., Sutskever, I., & Duvenaud, D. (2019). FFJORD: Free-form Continuous Dynamics for Scalable Reversible Generative Models. *ICLR 2019*. arXiv: [1810.01367](https://arxiv.org/abs/1810.01367)
    - Neural ODEを正規化フローへ応用。密度変化に現れるdivergence（ヤコビアンのトレース）をHutchinson推定量でスケーラブルに推定する手法を提案。
- <a id="ref-rubanova2019"></a>Rubanova, Y., Chen, R. T. Q., & Duvenaud, D. (2019). Latent Ordinary Differential Equations for Irregularly-Sampled Time Series. *NeurIPS 2019*. arXiv: [1907.03907](https://arxiv.org/abs/1907.03907)
...
- <a id="ref-liu2023flow"></a>Liu, X., Gong, C., & Liu, Q. (2023). Flow Straight and Fast: Learning to Generate and Transfer Data with Rectified Flow. *ICLR 2023*. arXiv: [2209.03003](https://arxiv.org/abs/2209.03003)
    - Rectified Flowの提案。データとノイズを直線軌道で結ぶ連続正規化フローの枠組み。Flow Matchingと並ぶODEベース生成モデルの基盤。

### 古典的生成モデル

- <a id="ref-goodfellow2014"></a>Goodfellow, I., et al. (2014). Generative Adversarial Nets. *NeurIPS 2014*. arXiv: [1406.2661](https://arxiv.org/abs/1406.2661)
