...
埋め込み空間でも同じことが言えるのではないか。「りんご」という単語の表現は、文脈によってぶれるだろうか。「量子もつれ」という専門用語の表現は、学習データの量によって確信度が異なるだろうか。従来の点埋め込みは、この「ぶれ」や「確信度」という次元をそもそも持たなかった。分布埋め込みの視点から振り返れば、**確信度を無限大に固定していたと解釈できる**。固定していたことにすら、気づかずにいた。
...
我々がこれまで使ってきた従来の点埋め込み（正規化して使う場合のWord2Vec、正規化されたTransformer埋め込みなど）は、この表の最後の行——すなわち $\kappa = \infty$ という極限状態——を暗黙のうちに仮定していたと解釈できる。単位球面上の一点に配置することは、分布埋め込みの視点から振り返れば、「この方向に確信度100%で存在する」と宣言することに相当すると見なせる。
