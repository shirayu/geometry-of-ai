...
第2回で見たように、従来の埋め込みではベクトルの長さ（ノルム）がさまざまな意味を担っていた。
...
点埋め込みは分布埋め込みと対立するものではなく、 $\kappa = \infty$ という極限ケースとして包含される。この視点により、従来の手法と新しい手法が統一的に理解できる。

### 視点の階層：「点」という言葉の多義性

本回では、「点」から「分布」への移行を扱った。ただし、**我々が学習した分布のパラメータ $(\boldsymbol{\mu}, \kappa)$ 自体は、パラメータ空間という『メタな空間』における一つの点である。**
