# 第7回: 点から分布へ: 不確実性を表現する幾何学

> [!TIP]
> 読了後に、[理解度チェック](./07/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#第7回-点から分布へ-不確実性を表現する幾何学)でも確認できる。

## 注意事項

- $\kappa$ （集中度）を「確信度」として解釈できるのは、埋め込みをvMF分布として明示的にモデル化した場合に限られる。
- 単に $L_2$ 正規化して $\cos$ 類似度を使うだけでは、 $\kappa$ は自動的には得られない（点埋め込みに「霧」はない）。
- エントロピーの近似式 $H \approx \frac{d-1}{2}\log\frac{2\pi e}{\kappa}$ は、高 $\kappa$ の漸近近似であり、一般には成り立たない。特に高次元では、 $\kappa$ が次元に比べて十分大きいかを確認する必要がある。次元 $d$ に依存する係数を省略した $H \approx -\log\kappa$ という表記も見かけるが、次元間の比較では無視できない差になる。
- 本回で扱う「分布埋め込み」は、標準的な深層学習ではまだ主流ではない。しかし、不確実性を扱う設計の重要性は増しており、将来の標準になる可能性がある。

## 導入：解像度を上げる

[第3回](03.md)で、表現空間を「プラネタリウム」として捉えた。単位球面上に配置された点が、意味の「星」として輝いている。計算効率の良い、優れた近似だった。星は動かず、瞬かず、ただそこにある。それで十分だと、これまでは思えていた。

望遠鏡の倍率を上げれば、話が変わってくる。肉眼では「光る点」に見えた星も、高性能な望遠鏡で観察すれば、激しく燃え盛るガスの「広がり（霧）」であることがわかる。シャープに見える星は確信度が高く、ぼやけて見える星は不確実性が高い。この「にじみ」には重要な情報が含まれている。

埋め込み空間でも同じことが言えるのではないか。「りんご」という単語の表現は、文脈によってぶれるはずだ。「量子もつれ」という専門用語の表現は、学習データの量によって確信度が異なるはずだ。従来の点埋め込みは、この「ぶれ」や「確信度」という次元をそもそも持たなかった。分布埋め込みの視点から振り返れば、**確信度を無限大に固定していたと解釈できる**。固定していたことにすら、気づかずにいた。

本回では、この固定を解放する。**解像度を上げる**ことで、概念を「点（Point Estimate）」としてだけでなく、不確実性を含んだ「分布（Distribution）」として捉え直す。これにより、今まで見えなかった「迷い」や「知らなさ」を可視化できる。「分かりません」と言えるAIへの、第一歩である。

## ノルムの分離という思想

### 従来：混沌としたノルム

[第2回](02.md)で見たように、従来の埋め込みではベクトルの長さ（ノルム）が、一つではなくいくつもの意味を同時に担っていた。
...
これらの意味が混在しているため、ノルムを解釈することは困難だった。このベクトルが長いのは、頻度が高いからか、意味が強いからか、それとも単なる偶然か。見た目だけでは、区別のしようがない。
...
ただし、従来の点埋め込みがこの枠組みから外れているわけではない。点埋め込みは、**確信度パラメータを無限大に固定した特殊ケース**と見なすことができる。分布表現は、この固定を解放し、確信度を学習可能なパラメータとして扱う一般化である。
...
分布表現には、先駆けとなった成功例がある。**変分オートエンコーダ（VAE）**（[Kingma & Welling, 2014](#ref-kingma2014)）は、データを潜在空間上の分布として符号化し、そこからサンプリングして復元する。「点ではなく分布を学ぶ」という発想自体は、実はここからすでに始まっていた。
...
密度が最大の場所に、サンプルも最も集まっているはずだ。そう思うのが自然である。ところが、ガウス分布が生む空間には、その素朴な期待を裏切る**不均一性**がある。

標準正規分布 $\mathcal{N}(\mathbf{0}, \mathbf{I})$ の確率密度関数は原点で最大値をとる。しかし、高次元では**密度が最大の場所にサンプルが集まるとは限らない**。次元 $d$ が大きくなると、半径 $r$ の球殻の体積が $r^{d-1}$ に比例して急増するため、密度と体積の積、すなわち確率質量は、原点から離れた $r \approx \sqrt{d}$ 付近の薄い殻に集中する（concentration of measure）。これが**典型集合**と呼ばれる領域であり、特に半径方向の薄い帯として見た呼び方が**典型帯**である。
...
> つまり、高次元のサンプルは原点にはおらず、この半径 $\sqrt{d}$ 付近のごく薄い「殻」（**典型集合（Typical Set）**）に張り付いている。VAEで事前分布と事後分布が乖離する「Typical Set Mismatch」も、この幾何学的な力が引き起こす必然的な現象である。

### 球面への脱出：vMFという一般化

ガウス事前分布の根本的な制約は、**半径方向の自由度を強く持つ一方で、角度（方向）主体の構造に不向き**なことにある。潜在表現が意味する「何であるか」が主に方向で決まる場合、半径方向の変動はノイズでしかない。それにもかかわらず、ガウス事前分布は半径方向にも広い自由度を許し、律儀に典型帯という制約をかけてくる。要らない自由度に、わざわざ縛りを設けているようなものだ。

[第3回](03.md)で「プラネタリウム」として導入した球面表現は、この問題に対する幾何学的な回答である。単位球面 $\mathbb{S}^{d-1}$ 上では、すべての点が原点から等距離にあり、半径方向の自由度がそもそも存在しない。情報はすべて方向に集約される。

[第2回](02.md)、[第3回](03.md)で見た $L_2$ 正規化の効果も、この「半径の消去」によるものであった。ベクトルを単位球面に配置することは、幾何学的には「半径方向の情報を持たない」と宣言することである。点埋め込みの視点で言えば、これは **「不確実性（分散）の情報を持たない」、つまり確信度を無限大に固定する** ことに相当すると見なせる。

さらに、球面上で一様分布を事前分布に置けば、ガウス空間にあった半径方向の典型帯のような偏りは生じない。ただし、学習された事後分布は当然偏る（それは意味の構造を反映した望ましい偏りである）。**空間の幾何そのものが半径方向の不均一性を持たない**ことが、この設計の眼目である。
...
Davidson et al. (2018) は、VAEの潜在空間にvMF分布を導入した**Hyperspherical VAE**を提案し、潜在構造が球面的な場合にガウス事前分布より有利になりうることを実験的に示した（[Davidson et al., 2018](#ref-davidson2018)）。これは、データの潜在構造に合った幾何学を事前分布に採用すること（すなわち「正則化の形を変える」こと）の重要性を示す事例である。
...
**von Mises-Fisher（vMF）分布**は、単位球面上の方向分布である（[Mardia & Jupp, 2000](#ref-mardia2000)）。ユークリッド空間でのガウス分布の「球面版」と考えることができる。
...
一つのパラメータで、確信の強さをどう表せばよいのか。vMF分布が選んだ答えは $\kappa$ である。分布がどれだけ平均方向 $\boldsymbol{\mu}$ の周りに「集中」しているかを、この一つの数が語る。
...
我々がこれまで使ってきた従来の点埋め込み（正規化して使う場合のWord2Vec、正規化されたTransformer埋め込みなど）は、この表の最後の行、すなわち $\kappa = \infty$ という極限状態を、暗黙のうちに仮定していたと解釈できる。単位球面上の一点に配置することは、分布埋め込みの視点から振り返れば、「この方向に確信度100%で存在する」と宣言することに相当すると見なせる。
...
きれいな話には、たいてい代償がある。vMF分布の正規化定数 $C_d(\kappa)$ は、修正Bessel関数を含む複雑な形をしている：
...
> [!CAUTION]
> **実装上の注意：** vMF分布からのサンプリングや対数尤度の計算は、ナイーブな実装では数値的に不安定になりやすい。後述の[実装ノート](#実装ノート-分布埋め込みを動かす)で、安定した計算方法を紹介する。
...
> - $\kappa$ が大きいとき:
>     - 分布が鋭くなり積分値 $Z$ が小さくなるため、正規化定数 $C$ は大きくなり、帳尻を合わせる。
>
...
霧の広がり具合を、どう一つの数で測ればよいのか。

**エントロピーとは、分布という「霧」の体積である。**

[第4回](04.md)で登場した情報量 $-\log p(x)$ が「確実性からの遠さ」だったとすれば、エントロピーはその遠さの平均であり、分布全体が空間を占める「体積」に対応する：
...
ここで $A_d(\kappa) = I_{d/2}(\kappa) / I_{d/2 - 1}(\kappa)$ は Bessel 関数の比である（[Diethe, 2015](#ref-diethe2015)）。

### 近似式の条件

文献やチュートリアルでは、次の単純な近似式をよく見かける。

$$H \approx \frac{d-1}{2} \log \frac{2\pi e}{\kappa}$$

一見すると、どんな $\kappa$ でも使えそうに見える。
しかし、これは**高** $\kappa$ **の漸近近似**であり、一般には成り立たない。
さらに高次元では、 $\log\kappa$ に掛かる係数 $(d-1)/2$ の影響が大きくなる。
例えば $d=768$ では係数は $383.5$ であり、 $H\approx-\log\kappa$ と省略した式に比べて、 $\log\kappa$ に対する傾きの絶対値が383.5倍になる。
これはエントロピー値そのものが常に383.5倍になるという意味ではない。
エントロピーには $d$ に依存する加法項もあるため、異なる次元のモデル間で値を比較する場合は完全な式か数値計算を用いる必要がある。

| 条件 | 近似の妥当性 |
| --- | --- |
| 高 $\kappa$ （固定した $d$ に対して $\kappa \gg 1$ ） | 良い漸近近似 |
| 高次元かつ $\kappa$ が次元に比べて十分大きくない | 局所的なガウス近似に注意が必要 |
...
> [!NOTE]
> **[第9回](09.md)への伏線：** [第9回](09.md)（拡散モデル）では、この「霧の体積」が事前分布（等方的なガウスノイズ）全体に向かって拡大していく過程、すなわちエントロピーが単調増大していく軌道を扱う。「データへの逆拡散」とは、エントロピーを減らして霧を凝縮させる操作である。
...
単に $L_2$ 正規化しただけの点埋め込みでは、 $\kappa$ に相当する量は存在しない。「確信度」を持つためには、設計段階で分布表現を採用する必要がある（[Gal & Ghahramani, 2016](#ref-gal2016)）。

## Out-of-Distribution（OOD）検知

### 問題：ドーム上の「盲点」

プラネタリウムのドーム（単位球面）には、星（学習された概念）が点在している。星と星の間には、当然、何もない領域が広がっている。では、その星のない領域に入力が来たとき、モデルはどうすべきか。
...
この構造が、**Out-of-Distribution（OOD）入力に対する脆弱性**の一因である（[Hendrycks & Gimpel, 2017](#ref-hendrycks2017)）。
...
分布表現（ $\kappa$ を学習可能にする設計）を使えば、OOD検知に有力なシグナルが得られる（[Sensoy et al., 2018](#ref-sensoy2018)）。

**低 $\kappa$ 領域の検出**：入力に対して推定される $\kappa$ が低い場合、モデルはその入力について「よく分からない」と言っている。これはOODの可能性を示唆する。分布埋め込みでは、未知の入力に対して「場所は犬のあたりだが、霧のように薄くて自信がない（ $\kappa$ 小）」という応答が可能になる。この **「霧の濃さ（ $\kappa$ ）」という第二の軸** こそが、AIに謙虚さを与える。

**密度による検出**：vMF混合モデルを使えば、入力が高密度領域にあるか低密度領域にあるかを評価できる（[Lee et al., 2018](#ref-lee2018ood)）。低密度領域はOODの可能性が高い。
...
> [!NOTE]
> **「分かりません」と言えるAI：** OOD検知は、AIの信頼性を支える基礎である。医療診断や自動運転など、高リスクなアプリケーションでは、「分かりません」と言えるかどうかが安全性を左右する。分布表現は、この能力を表現空間のレベルで実現する一つのアプローチである。

## ハルシネーションの幾何学

### ハルシネーションとは何か

「分からない」と言えないなら、モデルはどうするのか。何か答えを出すしかない。大規模言語モデル（LLM）の**ハルシネーション**は、その帰結として、事実と異なる情報を、あたかも確信を持っているかのように生成する現象である（[Ji et al., 2023](#ref-ji2023)）。

幾何学的に見ると、ハルシネーションは以下のように解釈できる：

> **ハルシネーション = 確信度（ $\kappa$ ）の情報がないまま、星座を強制的に結ぶ行為**

星がまばらな領域（学習データが少ない、または概念が曖昧な領域）で、モデルはそれでも何らかの出力を生成しなければならない。OOD検知が扱うのは「その領域に本当に星がない」場合だったが、ハルシネーションが扱うのは一歩進んで「星がないにもかかわらず、無理やり大きな $\kappa$ を出力してしまう」場合である。 $\kappa$ を適切に推定できれば、この種のハルシネーションのリスクを検知できる可能性がある。
...
二つの埋め込み空間を、どうやって比べればよいのか。異なるモデルや異なる学習から得られた表現を比較したい場面は少なくない。例えば：
...
ここで生の出力 $\tilde{\kappa}$ をそのまま $\kappa$ として使わないのは、 $\kappa > 0$ の制約を満たす必要があるためである。Softplusで正の値に変換したのち、シグモイドで $[\kappa_{\min}, \kappa_{\max}]$ の範囲に収める実装がよく使われる（実装ノート参照）。
...
「頻度が高い単語（例：'run', 'set', 'get'）」は、なぜ埋め込み空間で扱いづらいのだろうか。よく使われる単語ほど、意味は磨かれて明確になっていそうなものだ。だとすれば、扱いにくさの理由は別のところにあるはずである。

**特に静的埋め込み（Word2Vec、GloVeなど）では** 、頻度が高い単語はノルムが大きくなる傾向が観察されてきた（ただし、正規化や学習設定により変わる）。しかし、 **近年の研究は、頻度の背後にある要因を指摘している** 。

Nagata et al. (2023) は、埋め込みの **分散（Variance）** が意味の違いを捉える重要な指標であることを実証した（[Nagata et al., 2023](#ref-nagata2023)）。さらに、Nagata & Tanaka-Ishii (2025) は、この現象の背後にある言語学的原理を明らかにした（[Nagata & Tanaka-Ishii, 2025](#ref-nagata2025)）：

1. **Zipfの意味-頻度則**：頻度が高い単語ほど、多くの意味を持つ（多義性が高い）。
2. **文脈的多様性（Contextual Diversity）**：頻度が高いから意味が広いのではなく、**「多様な文脈で使われる」から意味が広がる**のである。

これを本講義の「プラネタリウム」のメタファーで捉え直すと、どうなるか？

**多様な文脈 = 広い視野角**：さまざまな文脈に出現する単語は、球面上の「一点」ではなく、 **「広い領域」** を占有しなければならない。
...
**従来のアプローチとの接続**：[第2回](02.md)で見た「頻度が高い単語は長いベクトル」という観察（主に静的埋め込みで見られる傾向）は、この文脈的多様性を「ノルム」という形で暗黙的に捉えようとしていたと解釈できる。ここで言う「ノルム」は**個々の単語ベクトルの長さ**であり、Nagata et al. (2023) が計量として用いる**平均単語ベクトルのノルム**（分散の一種）とは異なる量である点に注意されたい。しかし、いずれにせよノルムは方向情報と混在するため解釈が困難だった。分布埋め込み（ $\kappa$ の導入）は、この混在を解消し、 **方向＝意味、 $\kappa$ ＝確信度（文脈的多様性の逆数に対応、ただし直接的な等価ではない）** という明確な分離を実現する。
...
| **典型集合** | 高次元ガウスで確率質量が集中する領域 | 潜在空間の不均一性の要因 |
...
従来の点埋め込みから分布埋め込みへの移行は、単なる技術的な拡張に見えるかもしれない。だが、それだけではない。**「分かりません」と言える能力**をモデルに与えるという、質的な変化がここにはある。
...
点埋め込みは分布埋め込みと対立するものではない。 $\kappa = \infty$ という極限ケースとして、そこに包含される。従来の手法を捨てる必要はなく、新しい手法の中に見出せばよい。

### 視点の階層：「点」という言葉の多義性

「点」から「分布」へ。ここまではその移行を扱ってきた。しかし、話はそれで終わらない。**我々が学習した分布のパラメータ $(\boldsymbol{\mu}, \kappa)$ 自体は、パラメータ空間という『メタな空間』における一つの点である。**

点を克服したはずが、また点に戻ってくる。この入れ子構造は混乱を招きやすいが、以下の3つの階層を区別すれば整理できる：

| 階層 | 実体 | 直感的イメージ | 不確実性の有無 |
| --- | --- | --- | --- |
| **レベル1：表現空間の点** | $\mathbf{h} \in \mathbb{S}^{d-1}$ | プラネタリウムの星（単一の方向） | なし（ $\kappa = \infty$ ） |
| **レベル2：表現空間の分布** | $p(\mathbf{h}; \boldsymbol{\mu}, \kappa)$ | 星の周りの霧（広がりを持つ分布） | あり（ $\kappa$ が有限） |
| **レベル3：統計多様体の点** | $(\boldsymbol{\mu}, \kappa)$ | その霧の形を指定する座標 | （次の階層で議論） |

従来の点埋め込み（レベル1）を分布埋め込み（レベル2）に拡張したことで、解像度が上がった。しかし、その分布を指定するパラメータ $(\boldsymbol{\mu}, \kappa)$ は、より高次の空間（統計多様体）における「点」である。この第3の階層の詳細は **[Appendix 4](appendix.4.md)「空間の物差し再考」** に譲る。

プラネタリウムのメタファーで言えば：

> **点だけでなく、その周りの「霧」を見よ。そしてその霧を決める「設計図」にも注目せよ。**

星の位置（点埋め込み）だけでなく、星の周りのにじみ具合（分布の広がり）にも情報がある。そして、そのにじみ方を決めるパラメータ（統計多様体の点）にも、また別の幾何学が宿っている。霧が濃い領域では慎重に、霧が晴れた領域では自信を持って進めばよい。この使い分けが、信頼できるAIへの道を開く。
...
[第8回](08.md)「時間の発見」では、時間軸を持つ表現を議論する。
...
vMF分布からのサンプリングは、rejection samplingやWood's algorithmを使う（[Wood, 1994](#ref-wood1994)）。以下は高次元で効率的な方法：
...
- <a id="ref-kingma2014"></a>Kingma, D. P., & Welling, M. (2014). Auto-Encoding Variational Bayes. *ICLR 2014*. arXiv: [1312.6114](https://arxiv.org/abs/1312.6114)
    - VAEの原論文。潜在空間をガウス分布として正則化する枠組みを提案。本回では、ガウス空間の不均一性と球面表現への動機付けの文脈で参照。
- <a id="ref-nagata2023"></a>Nagata, R., Takamura, H., Otani, N., & Kawasaki, Y. (2023). Variance Matters: Detecting Semantic Differences without Corpus/Word Alignment. *EMNLP 2023*. DOI: [10.18653/v1/2023.emnlp-main.965](https://doi.org/10.18653/v1/2023.emnlp-main.965)
    - 本回の中心的な参考文献。埋め込みの分散が意味の違いを検出する上で重要であることを示した。
- <a id="ref-nagata2025"></a>Nagata, R., & Tanaka-Ishii, K. (2025). A New Formulation of Zipf's Meaning-Frequency Law through Contextual Diversity. *ACL 2025*. DOI: [10.18653/v1/2025.acl-long.744](https://doi.org/10.18653/v1/2025.acl-long.744)
...
- <a id="ref-mardia2000"></a>Mardia, K. V., & Jupp, P. E. (2000). *Directional Statistics*. Wiley. DOI: [10.1002/9780470316979](https://doi.org/10.1002/9780470316979)
    - 方向統計学の標準的教科書。vMF分布を含む球面上の分布理論を体系的に扱う。
- <a id="ref-davidson2018"></a>Davidson, T. R., Falorsi, L., De Cao, N., Kipf, T., & Tomczak, J. M. (2018). Hyperspherical Variational Auto-Encoders. *UAI 2018*. arXiv: [1804.00891](https://arxiv.org/abs/1804.00891)
    - VAEの潜在空間にvMF分布を導入。球面上のサンプリングとreparameterization trickを扱う。
- <a id="ref-diethe2015"></a>Diethe, T. (2015). A Note on the Kullback-Leibler Divergence for the von Mises-Fisher distribution. arXiv: [1502.07104](https://arxiv.org/abs/1502.07104)
    - vMF分布間のKLダイバージェンスの厳密な導出。数値安定な計算方法についても議論。本回の実装ノートで近似を使う際の参照先。

### 不確実性の定量化

- <a id="ref-gal2016"></a>Gal, Y., & Ghahramani, Z. (2016). Dropout as a Bayesian Approximation: Representing Model Uncertainty in Deep Learning. *ICML 2016*. arXiv: [1506.02142](https://arxiv.org/abs/1506.02142)
    - Dropoutによる不確実性推定の古典的論文。分布埋め込みとは異なるアプローチだが、不確実性の重要性を示した。
- <a id="ref-sensoy2018"></a>Sensoy, M., Kaplan, L., & Kandemir, M. (2018). Evidential Deep Learning to Quantify Classification Uncertainty. *NeurIPS 2018*. arXiv: [1806.01768](https://arxiv.org/abs/1806.01768)
    - 証拠理論に基づく不確実性の定量化。OOD検知への応用を扱う。

### OOD検知

- <a id="ref-hendrycks2017"></a>Hendrycks, D., & Gimpel, K. (2017). A Baseline for Detecting Misclassified and Out-of-Distribution Examples in Neural Networks. *ICLR 2017*. arXiv: [1610.02136](https://arxiv.org/abs/1610.02136)
    - OOD検知のベースライン手法。Softmax確率を使った単純だが効果的な方法を提案。
- <a id="ref-lee2018ood"></a>Lee, K., Lee, K., Lee, H., & Shin, J. (2018). A Simple Unified Framework for Detecting Out-of-Distribution Samples and Adversarial Attacks. *NeurIPS 2018*. arXiv: [1807.03888](https://arxiv.org/abs/1807.03888)
    - Mahalanobis距離に基づくOOD検知。特徴空間の幾何学を活用。

### ハルシネーションと信頼性

- <a id="ref-ji2023"></a>Ji, Z., et al. (2023). Survey of Hallucination in Natural Language Generation. *ACM Computing Surveys*, 55(12), 1-38. arXiv: [2202.03629](https://arxiv.org/abs/2202.03629)
...
- <a id="ref-wood1994"></a>Wood, A. T. A. (1994). Simulation of the von Mises Fisher Distribution. *Communications in Statistics - Simulation and Computation*, 23(1), 157-164. DOI: [10.1080/03610919408813161](https://doi.org/10.1080/03610919408813161)
