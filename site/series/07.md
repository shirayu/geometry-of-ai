...
分布表現には、先駆けとなった成功例がある。**変分オートエンコーダ（VAE）**（Kingma & Welling, 2014）は、データを潜在空間上の分布として符号化し、そこからサンプリングして復元する。「点ではなく分布を学ぶ」という発想自体は、実はここからすでに始まっていた。
...
Davidson et al. (2018) は、VAEの潜在空間にvMF分布を導入した**Hyperspherical VAE**を提案し、潜在構造が球面的な場合にガウス事前分布より有利になりうることを実験的に示した（Davidson et al., 2018）。これは、データの潜在構造に合った幾何学を事前分布に採用すること（すなわち「正則化の形を変える」こと）の重要性を示す事例である。
...
**von Mises-Fisher（vMF）分布**は、単位球面上の方向分布である（Mardia & Jupp, 2000）。ユークリッド空間でのガウス分布の「球面版」と考えることができる。
...
ここで $A_d(\kappa) = I_{d/2}(\kappa) / I_{d/2 - 1}(\kappa)$ は Bessel 関数の比である（Diethe, 2015）。
...
単に $L_2$ 正規化しただけの点埋め込みでは、 $\kappa$ に相当する量は存在しない。「確信度」を持つためには、設計段階で分布表現を採用する必要がある（Gal & Ghahramani, 2016）。
...
この構造が、**Out-of-Distribution（OOD）入力に対する脆弱性**の一因である（Hendrycks & Gimpel, 2017）。
...
分布表現（ $\kappa$ を学習可能にする設計）を使えば、OOD検知に有力なシグナルが得られる（Sensoy et al., 2018）。
...
**密度による検出**：vMF混合モデルを使えば、入力が高密度領域にあるか低密度領域にあるかを評価できる（Lee et al., 2018）。低密度領域はOODの可能性が高い。
...
「分からない」と言えないなら、モデルはどうするのか。何か答えを出すしかない。大規模言語モデル（LLM）の**ハルシネーション**は、その帰結として、事実と異なる情報を、あたかも確信を持っているかのように生成する現象である（Ji et al., 2023）。
...
Nagata et al. (2023) は、埋め込みの **分散（Variance）** が意味の違いを捉える重要な指標であることを実証した（Nagata et al., 2023）。さらに、Nagata & Tanaka-Ishii (2025) は、この現象の背後にある言語学的原理を明らかにした（Nagata & Tanaka-Ishii, 2025）：
...
vMF分布からのサンプリングは、rejection samplingやWood's algorithmを使う（Wood, 1994）。以下は高次元で効率的な方法：
...
- Nagata, R., Takamura, H., Otani, N., & Kawasaki, Y. (2023). Variance Matters: Detecting Semantic Differences without Corpus/Word Alignment. *EMNLP 2023*.
    - 本回の中心的な参考文献。埋め込みの分散が意味の違いを検出する上で重要であることを示した。
- Nagata, R., & Tanaka-Ishii, K. (2025). A New Formulation of Zipf's Meaning-Frequency Law through Contextual Diversity. *ACL 2025*.
...
- Davidson, T. R., Falorsi, L., De Cao, N., Kipf, T., & Tomczak, J. M. (2018). Hyperspherical Variational Auto-Encoders. *UAI 2018*. arXiv: [1804.00891](https://arxiv.org/abs/1804.00891)
...
- Sensoy, M., Kaplan, L., & Kandemir, M. (2018). Evidential Deep Learning to Quantify Classification Uncertainty. *NeurIPS 2018*. arXiv: [1806.01768](https://arxiv.org/abs/1806.01768)
...
- Lee, K., Lee, K., Lee, H., & Shin, J. (2018). A Simple Unified Framework for Detecting Out-of-Distribution Samples and Adversarial Attacks. *NeurIPS 2018*. arXiv: [1807.03888](https://arxiv.org/abs/1807.03888)
    - Mahalanobis距離に基づくOOD検知。特徴空間の幾何学を活用。

### ハルシネーションと信頼性

- Ji, Z., et al. (2023). Survey of Hallucination in Natural Language Generation. *ACM Computing Surveys*, 55(12), 1-38. arXiv: [2202.03629](https://arxiv.org/abs/2202.03629)
