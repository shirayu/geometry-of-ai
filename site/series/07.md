...
- エントロピーの近似式 $H \approx -\frac{d-1}{2}\log \kappa$ は、高次元・高 $\kappa$ の条件下での近似であり、一般には成り立たない。次元 $d$ に依存する係数を省略した $H \approx -\log\kappa$ という表記も見かけるが、次元間の比較では無視できない差になる。
...
$$H \approx -\frac{d-1}{2} \log \kappa + \text{const.}$$

一見すると、どんな $\kappa$ でも使えそうに見える。しかし、これは**高次元かつ高** $\kappa$ **という条件下**での近似であり、一般には成り立たない。次元 $d$ に依存する係数 $(d-1)/2$ を落として $H \approx -\log\kappa$ とする簡略化もよく見かけるが、 $d=768$ 程度の埋め込み次元では両者は約384倍違う。次元の影響を無視できない場面（異なる次元のモデル間でエントロピーを比較する等）では係数を省略しないこと。
