...
**[Appendix 4](appendix.4.md)「空間の物差し再考」** では、この第3の階層を扱う。そこでは、パラメータ空間上の「点」が実は確率分布そのものを代表しており、点と点の間の「距離」（KLダイバージェンス、フィッシャー情報行列）が持つ深い意味を議論する。本回で得た直感が、より普遍的な情報幾何学の視点へと接続される。
