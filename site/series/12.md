...
> [!TIP]
> 読了後に、[理解度チェック](./12/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#第12回-双曲幾何学-負の曲率の世界)でも確認できる。

## 注意事項

- 双曲空間が有効なのは、データに階層的、木構造的な性質がある場合に限られる。
- すべてのデータが双曲空間に適しているわけではない（平坦や球面が適する場合も多い）。
...
[第3回](03.md)から[第11回](11.md)まで、私たちは主に**正の曲率**を持つ空間（球面）を舞台に議論してきた。正規化されたベクトルは単位球面上の点として、コサイン類似度は角度として解釈できた。この枠組みは、意味的な類似性を測る多くのタスクで有効だった。球面さえあれば、たいていの構造は表現できるだろう。そう思いたくなる。
...
正の曲率がうまくいかないなら、負の曲率はどうか。**双曲空間（hyperbolic space）** は、まさにその発想から生まれた。階層構造を自然に表現する能力を、この空間はなぜ持っているのか。幾何学的な直感から見ていく。
...
その特徴は**指数的な成長**にある。根ノードから出発し、各ノードが $b$ 個の子を持つとしよう（分岐数 $b$ ）。すると、深さ $d$ にあるノードの数は $b^d$ である。深さが1増えるごとに、ノード数は $b$ 倍になる。数字で見ると、この増え方の速さが際立つ。
...
直感的に、親ノードの「周り」に子ノードを配置したい。素朴なアイデアとしては、悪くないはずである。ところが、ユークリッド空間では、半径 $r$ の球の表面積は $r^{d-1}$ に比例する（多項式的成長）。これは、木のノード数の指数的成長に追いつかない。
...
> （通常、 $f$ が異なる点を同一点に潰さない、すなわち $u \neq v \Rightarrow d_E(f(u), f(v)) > 0$ を仮定する。）歪みが1に近いほど、元の距離関係が正確に保存されている。一般に、 $n$ ノードの木をユークリッド空間に低歪みで埋め込むには、多くの次元を必要とすることが知られている。一方、Sarkar (2011) は双曲平面（2次元）への木の埋め込みが低歪みで可能であることを示した（[Sarkar, 2011](#ref-sarkar2011)）。

### 球面空間での困難

ユークリッド空間が力不足なら、曲がった空間を使えばいい。素直な発想だ。では、球面 $\mathbb{S}^{d-1}$ ではどうか。

球面は正の曲率を持つ。通常の2次元球面（球の表面、 $\mathbb{S}^2$ ）の場合、ある点から測地線距離 $r$ 離れた点の集合（測地線円の「円周」）の長さは $2\pi \sin(r)$ となる。曲がっている分、周囲はユークリッド空間より速く広がってくれそうに見える。

ところが、 $\sin(r)$ は $r = \pi/2$ で最大値を取り、それ以降は減少する。つまり、球面上では「中心から離れるほど周囲が広がる」のではなく、**ある地点を境に狭くなる**。これは、指数的に広がる木構造の表現には致命的である。

球面は、「対等な関係」（意味的な類似性、クラスタリング）には適しているが、「上下関係」（階層構造）には不向きなのだ。曲率の符号を変えれば済む話ではなかった。

| 空間 | 曲率 | 測地線円の周囲長（2次元、曲率 $\pm 1$ の場合） | 木構造への適性 |
| --- | --- | --- | --- |
| ユークリッド平面 | 0 | $2\pi r$ （線形） | 低い（深い階層で容量不足） |
| 単位球面 $\mathbb{S}^2$ | $+1$ | $2\pi\sin(r)$ （有界） | 低い（容量不足） |
...
球面がだめなら、逆の符号を試す番である。[第0回](00.md)で、曲率を「三角形の内角の和」で直感的に理解した。正の曲率（球面）では内角の和が180°を超え、負の曲率では180°を下回る。
...
具体的な距離公式は、次節で紹介する[ポアンカレ円板モデル](#ポアンカレ円板モデル-無限を有限に閉じ込める)で説明する。

## ポアンカレ円板モデル：無限を有限に閉じ込める

### モデルとは何か

指数的に広がる周囲を持つ空間が存在することはわかった。では、それをどう描けばいいのか。完全な双曲空間は、3次元ユークリッド空間に等長に埋め込めない（前述のHilbertの定理）。そのため、私たちは双曲空間を「見る」ために、何らかの**モデル（model）** を使う必要がある。
...
本回では、可視化に優れ、実装例も多い**ポアンカレ円板モデル**を中心に扱う（[Cannon et al., 1997](#ref-cannon1997)）。
...
円板は有限の広さしか持たないが、この中に無限に広がる双曲空間を収める。矛盾しているように聞こえるが、そうはならない。

- 円板の中心が双曲空間の「原点」に対応
- 円板の境界（単位円）は「無限遠」に対応
- 中心から境界へ向かうにつれ、双曲距離は無限大に発散

境界に近づくほど、双曲距離は際限なく伸びる。円板そのものは有限でも、その中の「歩ける距離」は無限になる。これを直感的に理解するために、エッシャーの版画「円の極限 (Circle Limit)」シリーズを思い出してほしい。中心から離れるほど図形が小さくなっていくが、双曲空間の「住人」から見ると、すべての図形は同じ大きさである。小さく見えるのは、私たちがユークリッド的な目で見ているからだ。
...
> [!CAUTION]
> **数値的安定性：** ポアンカレ円板モデルでは、境界付近（ $\|\mathbf{x}\| \to 1$ ）で数値的な不安定性が生じやすい。実装では、ノルムを $1 - \epsilon$ （例： $\epsilon = 10^{-5}$ ）でクリップするなどの対策が必要である。また、ローレンツモデルは数値的により安定であるため（[Nickel & Kiela, 2018](#ref-nickel2018)）、大規模な学習ではローレンツモデルが採用されることも多い。
...
**WordNetの埋め込み**：語彙の上位-下位関係を低次元で表現。ユークリッド空間の200次元と同等の表現力を、双曲空間の5次元で達成（[Nickel & Kiela, 2017](#ref-nickel2017)）。

**知識グラフ**：エンティティ間の「is-a」「part-of」などの階層的関係をモデル化。TransEなどのユークリッドベースの手法に対して、階層的関係の予測精度が向上（[Balažević et al., 2019](#ref-balažević2019); [Chami et al., 2019](#ref-chami2019)）。

**社会ネットワーク**：影響力の階層構造（インフルエンサー→フォロワー）をモデル化。スケールフリーネットワークの構造を自然に捉える（[Krioukov et al., 2010](#ref-krioukov2010)）。

**系統樹**：進化生物学における種の分岐構造。木構造そのものであるため、双曲空間との相性が良い。
...
曲率 $c$ をパラメータとして、 $c > 0$ なら球面的、 $c = 0$ ならユークリッド、 $c < 0$ なら双曲的な空間を表現する（記号 $K$ はカテゴリ数として[第4回](04.md)で予約済みのため、ここでは曲率に $c$ を用いる）。学習を通じて、データに最適な曲率が決定される。

これは、「幾何学をデータから発見する」という野心的な方向性であり、現在も活発に研究されている（[Gu et al., 2019](#ref-gu2019); [Bachmann et al., 2020](#ref-bachmann2020)）。

## 拡張：Mixed-Curvature Spaces

### 単一曲率の限界

双曲空間で階層は表現できた。球面で類似性も表現できた。では、この二つが同じデータの中に同居していたらどうするのか。
...
- **Ultrahyperbolic representations**（[Law & Stam, 2020](#ref-law2020)）：正と負の曲率を混合した擬リーマン多様体での表現学習
- **Product manifolds**（[Gu et al., 2019](#ref-gu2019)）：球面、双曲、ユークリッドの直積空間での埋め込み
- **Gyrovector spaces**：双曲空間におけるベクトル演算の体系化（[Ungar, 2008](#ref-ungar2008)）
...
負の曲率と聞くと、奇妙な性質を持つ特殊な空間を想像するかもしれない。しかし、その性質（周囲長の指数的成長）は、木構造や階層構造の本質と深く対応している。奇妙どころか、木の形をした世界にとっては自然な住処だったわけだ。

球面が「対等な関係」を表現するのに適しているのに対し、双曲空間は「上下関係」を表現するのに適している。これは、空間の曲率という幾何学的な概念が、データの構造という意味論的な概念と結びついていることを示している。

「どの幾何学を選ぶか」は、データの構造に依存する。万能の幾何学は存在しない。それでも、幾何学という言語を持つことで、「なぜこの設計が有効なのか」を説明し、「どのような構造に適しているのか」を予測できるようになる。

### 問い

**データの「形」を事前に知ることなく、最適な幾何学を発見できるか？**

曲率をあらかじめ選ばず、学習可能なパラメータとして扱う道はすでにある。複合曲率空間という妥協案もある。しかし、木構造でも対等な関係でもない、第三の構造にはどんな幾何学が対応するのか。この問いへの答えは、まだ探求の途上にある。
...
[第13回: 高次元の深淵: 幾何学的な恐怖と祝福](13.md)では、これまで避けてきた高次元空間の「奇妙な性質」と向き合う。
...
> [!NOTE]
> 以下のコードは PyTorch >= 1.9 を前提とする。再現性のために `torch.manual_seed(42)` 等でシードを固定することを推奨。双曲幾何学の最適化には、[Geoopt](https://github.com/geoopt/geoopt) ライブラリが有用である（[Kochurov et al., 2020](#ref-kochurov2020)）。

> [!IMPORTANT]
> **曲率パラメータについて：** 以下のコード例はすべて曲率 $c = -1$ の標準双曲空間を仮定している。一般の曲率 $c < 0$ では距離公式や指数写像の式が変わる（距離は曲率に応じてスケールし、実装ではモデルの定義に従う）。Geooptなどのライブラリでは曲率をパラメータとして指定できる。また、簡易版の実装では「直接更新して射影」という方法を用いているが、**厳密なリーマン最適化では指数写像（expmap）を用いた更新が標準**である。
...
- <a id="ref-nickel2018"></a>Nickel, M., & Kiela, D. (2018). Learning Continuous Hierarchies in the Lorentz Model of Hyperbolic Geometry. *ICML 2018*. arXiv: [1806.03417](https://arxiv.org/abs/1806.03417)
    - 数値的に安定なローレンツモデルでの双曲埋め込み。

### 理論的背景

- <a id="ref-sarkar2011"></a>Sarkar, R. (2011). Low Distortion Delaunay Embedding of Trees in Hyperbolic Plane. *Graph Drawing 2011*. DOI: [10.1007/978-3-642-25878-7_34](https://doi.org/10.1007/978-3-642-25878-7_34)
    - 木構造の双曲平面（2次元）への低歪み埋め込みを示した理論的研究。双曲空間が木構造に適している理論的根拠を与える。
- <a id="ref-krioukov2010"></a>Krioukov, D., Papadopoulos, F., Kitsak, M., Vahdat, A., & Boguñá, M. (2010). Hyperbolic Geometry of Complex Networks. *Physical Review E*, 82(3), 036106. arXiv: [1006.5169](https://arxiv.org/abs/1006.5169)
    - 複雑ネットワークと双曲幾何学の関係を解明した物理学的研究。

### 知識グラフと双曲空間

- <a id="ref-balažević2019"></a>Balažević, I., Allen, C., & Hospedales, T. (2019). Multi-relational Poincaré Graph Embeddings. *NeurIPS 2019*. arXiv: [1905.09791](https://arxiv.org/abs/1905.09791)
    - 知識グラフの多関係埋め込みへの双曲空間の適用。
- <a id="ref-chami2019"></a>Chami, I., Ying, Z., Ré, C., & Leskovec, J. (2019). Hyperbolic Graph Convolutional Neural Networks. *NeurIPS 2019*. arXiv: [1910.12933](https://arxiv.org/abs/1910.12933)
    - グラフニューラルネットワークの双曲空間への拡張。

### Mixed-Curvature Spaces

- <a id="ref-gu2019"></a>Gu, A., Sala, F., Gunel, B., & Ré, C. (2019). Learning Mixed-Curvature Representations in Product Manifolds. *ICLR 2019*. [https://openreview.net/forum?id=HJxeWnCcF7](https://openreview.net/forum?id=HJxeWnCcF7)
    - 球面、双曲、ユークリッドの直積空間での表現学習。
- <a id="ref-bachmann2020"></a>Bachmann, G., Bécigneul, G., & Ganea, O.-E. (2020). Constant Curvature Graph Convolutional Networks. *ICML 2020*. arXiv: [1911.05076](https://arxiv.org/abs/1911.05076)
    - 曲率を学習可能にしたグラフネットワーク。
- <a id="ref-law2020"></a>Law, M. T., & Stam, J. (2020). Ultrahyperbolic Representation Learning. *NeurIPS 2020*. arXiv: [2007.00211](https://arxiv.org/abs/2007.00211)
    - 正と負の曲率を混合した擬リーマン多様体での表現学習。NeurIPS 2020。
- <a id="ref-ungar2008"></a>Ungar, A. A. (2008). *Analytic Hyperbolic Geometry and Albert Einstein's Special Theory of Relativity*. World Scientific. DOI: [10.1142/6625](https://doi.org/10.1142/6625)
    - 双曲空間におけるベクトル演算（gyrovector spaces）の体系的な理論書。

### 実装とライブラリ

- <a id="ref-kochurov2020"></a>Kochurov, M., Karimov, R., & Kozlukov, S. (2020). Geoopt: Riemannian Optimization in PyTorch. arXiv: [2005.02819](https://arxiv.org/abs/2005.02819)
    - PyTorchでのリーマン最適化ライブラリ。本講義のコード例でも使用。
    - GitHub: [https://github.com/geoopt/geoopt](https://github.com/geoopt/geoopt)

### 双曲幾何学の教科書

- <a id="ref-cannon1997"></a>Cannon, J. W., Floyd, W. J., Kenyon, R., & Parry, W. R. (1997). Hyperbolic Geometry. *Flavors of Geometry (MSRI Publications, Vol. 31)*, 59–115. [https://www.cambridge.org/core/books/abs/flavors-of-geometry/hyperbolic-geometry/961E43ACDF594DF9C126D90D97DA5B74](https://www.cambridge.org/core/books/abs/flavors-of-geometry/hyperbolic-geometry/961E43ACDF594DF9C126D90D97DA5B74)
