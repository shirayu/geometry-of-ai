...
> （通常、 $f$ が異なる点を同一点に潰さない、すなわち $u \neq v \Rightarrow d_E(f(u), f(v)) > 0$ を仮定する。）歪みが1に近いほど、元の距離関係が正確に保存されている。一般に、 $n$ ノードの木をユークリッド空間に低歪みで埋め込むには、多くの次元を必要とすることが知られている。一方、Sarkar (2011) は双曲平面（2次元）への木の埋め込みが低歪みで可能であることを示した（Sarkar, 2011）。
...
本回では、可視化に優れ、実装例も多い**ポアンカレ円板モデル**を中心に扱う（Cannon et al., 1997）。
...
> [!CAUTION]
> **数値的安定性：** ポアンカレ円板モデルでは、境界付近（ $\|\mathbf{x}\| \to 1$ ）で数値的な不安定性が生じやすい。実装では、ノルムを $1 - \epsilon$ （例： $\epsilon = 10^{-5}$ ）でクリップするなどの対策が必要である。また、ローレンツモデルは数値的により安定であるため（Nickel & Kiela, 2018）、大規模な学習ではローレンツモデルが採用されることも多い。
...
**知識グラフ**：エンティティ間の「is-a」「part-of」などの階層的関係をモデル化。TransEなどのユークリッドベースの手法に対して、階層的関係の予測精度が向上（Balažević et al., 2019; Chami et al., 2019）。

**社会ネットワーク**：影響力の階層構造（インフルエンサー→フォロワー）をモデル化。スケールフリーネットワークの構造を自然に捉える（Krioukov et al., 2010）。
...
- **Ultrahyperbolic representations**（Law & Stam, 2020）：正と負の曲率を混合した擬リーマン多様体での表現学習
...
> [!NOTE]
> 以下のコードは PyTorch >= 1.9 を前提とする。再現性のために `torch.manual_seed(42)` 等でシードを固定することを推奨。双曲幾何学の最適化には、[Geoopt](https://github.com/geoopt/geoopt) ライブラリが有用である（Kochurov et al., 2020）。
...
- Nickel, M., & Kiela, D. (2018). Learning Continuous Hierarchies in the Lorentz Model of Hyperbolic Geometry. *ICML 2018*. arXiv: [1806.03417](https://arxiv.org/abs/1806.03417)
...
- Krioukov, D., Papadopoulos, F., Kitsak, M., Vahdat, A., & Boguñá, M. (2010). Hyperbolic Geometry of Complex Networks. *Physical Review E*, 82(3), 036106. arXiv: [1006.5169](https://arxiv.org/abs/1006.5169)
    - 複雑ネットワークと双曲幾何学の関係を解明した物理学的研究。

### 知識グラフと双曲空間

- Balažević, I., Allen, C., & Hospedales, T. (2019). Multi-relational Poincaré Graph Embeddings. *NeurIPS 2019*. arXiv: [1905.09791](https://arxiv.org/abs/1905.09791)
    - 知識グラフの多関係埋め込みへの双曲空間の適用。
- Chami, I., Ying, Z., Ré, C., & Leskovec, J. (2019). Hyperbolic Graph Convolutional Neural Networks. *NeurIPS 2019*. arXiv: [1910.12933](https://arxiv.org/abs/1910.12933)
...
- Bachmann, G., Bécigneul, G., & Ganea, O.-E. (2020). Constant Curvature Graph Convolutional Networks. *ICML 2020*. arXiv: [1911.05076](https://arxiv.org/abs/1911.05076)
    - 曲率を学習可能にしたグラフネットワーク。
- Law, M. T., & Stam, J. (2020). Ultrahyperbolic Representation Learning. *NeurIPS 2020*. arXiv: [2007.00211](https://arxiv.org/abs/2007.00211)
    - 正と負の曲率を混合した擬リーマン多様体での表現学習。NeurIPS 2020。
- Ungar, A. A. (2008). *Analytic Hyperbolic Geometry and Albert Einstein's Special Theory of Relativity*. World Scientific. DOI: [10.1142/6625](https://doi.org/10.1142/6625)
    - 双曲空間におけるベクトル演算（gyrovector spaces）の体系的な理論書。

### 実装とライブラリ

- Kochurov, M., Karimov, R., & Kozlukov, S. (2020). Geoopt: Riemannian Optimization in PyTorch. arXiv: [2005.02819](https://arxiv.org/abs/2005.02819)
