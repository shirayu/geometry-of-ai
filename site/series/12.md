...
第3回から第11回まで、私たちは主に**正の曲率**を持つ空間（球面）を舞台に議論してきた。正規化されたベクトルは単位球面上の点として、コサイン類似度は角度として解釈できた。この枠組みは、意味的な類似性を測る多くのタスクで有効だった。球面さえあれば、たいていの構造は表現できるだろう。そう思いたくなる。
...
球面がだめなら、逆の符号を試す番である。第0回で、曲率を「三角形の内角の和」で直感的に理解した。正の曲率（球面）では内角の和が180°を超え、負の曲率では180°を下回る。
...
第13回「高次元の深淵 ～幾何学的な恐怖と祝福～」では、これまで避けてきた高次元空間の「奇妙な性質」と向き合う。
