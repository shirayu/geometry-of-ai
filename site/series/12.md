...
本回では、この問題を解決する**双曲空間（hyperbolic space）** を導入する。負の曲率を持つこの空間は、階層構造を自然に表現する能力を持っている。なぜそうなるのかを、幾何学的な直感から理解する。
...
では、球面 $\mathbb{S}^{d-1}$ ではどうか。

球面は正の曲率を持つ。通常の2次元球面（球の表面、 $\mathbb{S}^2$ ）の場合、ある点から測地線距離 $r$ 離れた点の集合（測地線円の「円周」）の長さは $2\pi \sin(r)$ となる。
