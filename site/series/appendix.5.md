...
> [!TIP]
> 読了後に、[理解度チェック](./appendix.5/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#appendix-5-情報幾何学における双対構造-2種類のまっすぐ)でも確認できる。

## 本Appendixの位置づけ

本Appendixは、講義本編（特に[第4回: 分類を確率で読む: Softmaxと情報幾何学](04.md)）の発展的内容として、**Softmaxの背後にある情報幾何学の双対構造**を扱うものである。

[第4回](04.md)では、Softmaxを確率多様体上の写像として読み直し、自然勾配やFisher情報行列との関係を整理した。本Appendixではその議論をさらに進め、確率分布の空間に存在する **2種類の「まっすぐ」（e-接続とm-接続）** を導入し、それらが連続的なパラメータ $\alpha$ を持つ接続族の両端であることを示す。
...
「まっすぐ」とは何かを、ユークリッド空間で考えたことがある人はいないだろう。直線は直線であり、それ以外の候補は存在しないからだ。ところが、情報幾何学の最も深い洞察の一つは、確率分布の空間には **2種類の「まっすぐ」** が存在するということである。ユークリッド空間の常識が通用しない場所に来ている。確率分布の空間の本質的な非対称性がここに反映されている。
確率の世界は平らではなく「歪んでいる」ため、普通の定規では測れない。
そこで「定規」の代わりに「接続」という道具を導入する。

## e-接続とm-接続

**接続（connection）** とは、多様体上でベクトルを「平行移動」する方法を定める構造である。通常のユークリッド空間では一意だが、曲がった空間では話が違う。複数の「まっすぐ」の定義がありうる。

情報幾何学では、Fisher計量と整合的な2つの接続が特に重要である：

> [!NOTE]
> 以下の自然パラメータ $\boldsymbol{\theta}$ は、[第4回](04.md)・[Appendix 4](appendix.4.md)と同じく分布を指定するパラメータであり、ニューラルネットワークの重み全体ではない（対応の全体像は[第0回](00.md)を参照）。
...
このテンソル $T$ は、確率分布の対数尤度の3次微分から構成される量であり、統計多様体がユークリッド空間とは異なる「歪み」を持つことを反映している。 $T = 0$ であればすべての $\alpha$ -接続は一致するが、一般の統計多様体では $T \neq 0$ であるため、 $\alpha$ の値に応じて異なる「まっすぐ」が生じる。
...
| --- | --- | --- |
| $+1$ | **e-接続** $\nabla^{(1)}$ | 指数型分布族で平坦 |
| $0$ | **Levi-Civita接続** $\nabla^{(0)}$ | [計量と捩率の両立](#levi-civita接続-の位置づけ) |
| $-1$ | **m-接続** $\nabla^{(-1)}$ | 混合分布族で平坦 |
...
> - $\alpha = -1$ ：確率分布の空間での直線（足し算＝混合の世界）
>
> パラメータ $\alpha$ を $+1$ から $-1$ へ連続的に動かすと、「まっすぐ」の定義がロジット空間での直線から確率分布の空間での直線へと滑らかに変形していく。
...
> [!CAUTION]
> リーマン幾何学において測地線は変分原理（経路の長さを微小に変化させたとき増減しない、すなわち停留するという条件）で定義される局所的な性質であり、大域的な最短経路を保証するものではない。
...
情報幾何学において主役を演じるのは、この双対性の方である。そして、計量整合性を手放した代わりに得られるのが、次項で述べる **平坦性** という強力な構造である。

### なぜ情報幾何では alpha=±1 なのか

リーマン幾何学の立場から見れば、Levi-Civita接続（ $\alpha=0$ ）こそが最も自然で「正統な」選択に思える。計量を保ち、捩れもない。これ以上の候補はないはずだ。では、なぜ情報幾何学はわざわざ $\alpha = \pm 1$ を好むのか。
...
> **Softmaxの構造を見るために：**
>
> Softmaxが「自然な」変換であること、すなわち自然パラメータ $\boldsymbol{\theta}$ （ロジット）と期待値パラメータ $\boldsymbol{\eta}$ （確率）を結ぶ勾配写像であることを理解するには、**Levi-Civita接続ではなく $\alpha = \pm 1$ の世界** を見る必要がある。Levi-Civita接続のもとでは、この双対パラメータの整然とした関係（双対平坦性）が見えてこない。
>
> 「長さを変えない素直なまっすぐ」を手放すことで、確率分布の空間の最も美しい構造（2つの平坦座標系とそれを結ぶSoftmax）が姿を現すのである。

## 双対平坦空間

計量整合性を手放した先に、何が待っているのか。指数型分布族が持つ最も美しい性質の一つが **双対平坦性（dually flat）** である。
...
二つの座標系 $\boldsymbol{\theta}$ と $\boldsymbol{\eta}$ は、ルジャンドル変換で結ばれている。式を追えば確かにそうなるのだが、これが何をしている操作なのかは、式だけでは見えてこない。ルジャンドル変換は、数式上は単なる変数の交換に見えるが、幾何学的には **「曲線を点の軌跡として見るか、接線の包絡線として見るか」** という視点の転換を意味する。
...
切片の符号反転を新しい関数 $\phi(\boldsymbol{\eta})$ と定義すると、元の関数 $\psi$ と $\phi$ は互いにルジャンドル変換の関係になる。つまり、**ルジャンドル変換とは、曲線を「点の軌跡」と見るか「接線の包絡線」と見るかの翻訳作業** である。元の関数が（強）凸であれば、この翻訳は可逆であり、情報は一切失われない。
...
ルジャンドル変換は、物理学の複数の分野で中心的な役割を果たす。情報幾何学の双対構造は、これらの物理的構造と数学的な対応関係を持ち、単なる表面的なアナロジーを超えた共通の原理である「凸関数の双対性」に根ざしている。以下では物理学の予備知識がなくても読めるよう、概念の簡単な説明を添える。
...
この視点に立てば、Softmax関数は **「ロジットという速度を確率という運動量に変換する操作」** と見なせる。解析力学では、速度と運動量の両方を座標軸にとった空間を **相空間** と呼び、その上での変数の切り替え（正準変換）が力学の中心構造である。情報幾何学におけるSoftmaxは、ルジャンドル変換が果たす役割という点で、この正準変換に似た構造を持っている。
...
ユークリッド距離による最近傍（直交射影）と、KLダイバージェンスに基づく情報射影（e-射影・m-射影）は、異なる計量に基づく操作であり、一般に異なる点を与える。

## まとめ：双対構造の全体像

e-接続とm-接続、そして両者を繋ぐ $\alpha$ -接続。この双対構造は、Softmaxという一つの関数を出発点にしながら、講義全体の随所に姿を見せる。

| 講義回 | 接続点 |
| --- | --- |
| [第0回](00.md)（幾何学という言語） | 接続、測地線、曲率の基本概念。本Appendixの $\alpha$ -接続はその具体例 |
| [第4回](04.md)（分類を確率で読む） | Softmax＝ルジャンドル変換の勾配写像。双対平坦構造が自然勾配とFisher情報行列の背景を与える |
| [第5回](05.md)（分類を境界で読む） | e-測地線（ロジット空間の直線）上での分離が、角度マージンの議論と接続する |
| [第9回](09.md)（拡散と凝縮） | スコア関数 $\nabla \log p$ は、m-座標からe-座標への変換と構造的に類似 |
| [第12回](12.md)（双曲幾何学） | 負の曲率空間は双対平坦ではないが、指数写像と対数写像の関係に類似の双対性が現れる |
| [Appendix 4](appendix.4.md)（物差し再考） | KLダイバージェンスの非対称性が、e/m接続の双対性として幾何学的に説明される |
