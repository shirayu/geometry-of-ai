...
第1回で見たように、従来の機械学習手法は**入力に依存しない固定された計算経路**を持っていた。これは、**静的な地図**を持ち歩くようなものだ。地形が変わっても、同じ地図を読み続ける。
...
第13回で導入したMixture of Experts（MoE）は、複数の「専門家」（Expert）ネットワークを用意し、**入力に応じて一部の専門家だけを活性化する**アーキテクチャである。
...
第2回と第13回で繰り返し述べてきたように、高次元空間ではランダムベクトル同士が**ほぼ直交**する。
