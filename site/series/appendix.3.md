...
**本Appendixが達成すること**：AttentionとMoEを「選択的処理」という共通原理で理解する／効率化技術（GQA, LoRA, MoD）を統一的に整理する／高次元空間の性質とスパース化の関係を直感的に把握する。本資料は「正解」を示すものではなく「見方」を提供するものであり、この視点が有用かどうかは読者が扱う問題、データ、モデルに依存する。
...
本Appendixで扱った「剪定」は計算資源の効率化という文脈だったが、「何を計算しないか」を決める構えは、Retrieval-Augmented Generation（外部知識の選択的取得）やアライメント（望ましい行動の選択的強化）にも通じる。選ぶ対象が違うだけで、「高次元空間からの選択的射影」という同じ幾何学的操作として理解できる可能性がある。
