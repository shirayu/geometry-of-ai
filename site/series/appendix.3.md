...
[第1回](01.md)で見たように、従来の機械学習手法は入力に依存しない固定された計算経路を持っていた。Transformer（[第6回](06.md)）以降のAttention機構は、入力データ自身が計算経路を決定するという点で質的に異なる。本Appendixでは、この「入力に応じて計算経路が動的に変わる」という性質を**動的剪定**と呼び、Attention、Mixture of Experts、GQA/LoRA/MoD、FlashAttentionといった具体例を通じて、その幾何学的な意味を掘り下げる。
...
この動的性こそが、現代のAIモデルが持つ「適応性」の根源である。
...
AttentionとMoEだけが、無駄を削る手立てではない。近年のTransformer効率化手法の多くは、対象こそ違えど、「空間のどの部分を剪定するか」という同じ軸で整理できる。
...
Attention、MoE、GQA、LoRA、MoD。名前も対象も別々のこれらの手法に、一つだけ共通する構えがある。動的剪定の本質は **「何を計算するか」ではなく「何を計算しないか」を決めるプロセス** である。
...
「何を捨てるか」を決める行為は、この文脈だけのものではない。認知科学の観点から見ると、「情報を選択してエッセンスを抽出する」という行為は、**抽象化**（Abstraction）の一形態である。
...
ここまで、剪定という一つの言葉で数多くの技術を束ねてきた。束ねた分だけ、切り落とした注釈も多い。本Appendixは、**教育的な統一視点を提供する**ことを目的としており、以下の点を理解されたい：
...
- FlashAttentionの「パッキング」は教育的比喩であり、実体はI/O削減
...
**「AIとは何か」**。本講義シリーズに通底するこの問いに、動的剪定の視点は一つの答え方を与える。

動的剪定の視点から見れば、AIは「高次元空間における適応的な情報選択システム」である。どの情報を保持し、どの情報を捨てるかを、文脈に応じて動的に決定する。

しかし、この選択基準自体は、**学習データと目的関数に依存する**。何を「重要」と見なすかは、設計者と訓練データが決める。技術的に洗練された選択機構の奥に、誰が何を重要と決めたのかという、技術では閉じない問いが残る。

## まとめ：動的剪定の統一視点

Attentionは接続に重みをつけ、MoEはExpertを選び、GQAはヘッドを間引き、LoRAは更新の自由度を絞り、MoDは層を飛ばす。手つきはばらばらに見えて、動いている軸は一つだった。「動的剪定」という視点は、この一本の軸を可視化するためのものである。ただし、次の区別を混同すると、視点そのものが崩れる：
...
本Appendixで扱った「剪定」は、**計算資源の効率化**という文脈だった。しかし、「何を計算しないか」を決める構えは、計算資源の話に留まらない。同じ原理は**情報の選択**という、より広い文脈にも適用できる。

- Retrieval-Augmented Generation (RAG): 外部知識からの選択的取得
- アライメント: 望ましい行動の選択的強化
- 解釈可能性: 重要な特徴の選択的提示

RAGは知識を、アライメントは行動を、解釈可能性は特徴を選ぶ。選ぶ対象がそれぞれ違うだけで、「高次元空間からの選択的射影」という同じ幾何学的操作として理解できる可能性がある。
