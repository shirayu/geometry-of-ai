...
- ArcFace・CosFaceの「角度マージン」は、特徴ベクトルと重みベクトルの両方が $L_2$ 正規化されていることが前提である。SphereFace（A-Softmax）は重みのみ正規化し、特徴は正規化しない点に注意。
...
> [!NOTE]
> **SphereFaceの正規化について：** SphereFace（A-Softmax）の原論文は**重みのみを正規化**し、特徴ベクトルは正規化しない。その後、CosFace・ArcFaceが特徴と重みの両方を正規化する設計を採用した。三手法を「球面上のマージン手法」として一括りにすることがあるが、厳密にはSphereFaceは上記の意味でCosFace・ArcFaceとは異なる。
