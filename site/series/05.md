...
長らく、この二つは別々の教科書の別々の章に住み分けていた。境界を引く理論と確率を見積もる理論を、同じ土俵で比較する必要がなかったからである。ところが、第3回と第4回で見た「球面への跳躍」（表現を単位球面上に制約し、角度で類似度を測る設計）を採用すると、事情が変わる。距離の代わりに角度を主役に据えた途端、境界を引く発想と確率を見積もる発想が、同じ図形の上で出会ってしまうのである。
...
その前提を外すとどうなるか。第3回で見たように、表現を単位球面 $\mathbb{S}^{d-1}$ 上に制約すると、世界の見え方が変わる。
