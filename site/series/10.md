...
CoTの働き、Test-time compute scaling、そしてModel Collapse（Shumailov et al., 2023）やDimensional Collapse（Jing et al., 2022）といった表現の崩壊現象。これらは一見ばらばらな話題に見える。だが、「表現空間上の移動」という一つの見方でつながっている。
...
**Chain of Thought（CoT）**（Wei et al., 2022）は、LLMに「ステップバイステップで考える」よう促すプロンプティング技術である（Kojima et al., 2022）。
...
**2. サンプリング回数の増加（Self-consistency）**：同じ問題に対して複数回（独立に）推論を行い、多数決や集約で最終回答を決定する（Wang et al., 2023）。計算量は試行回数に比例。

**3. 探索アルゴリズムの適用（Tree of Thoughts）**：複数の思考経路を木構造で並行して探索し、評価関数で最も有望な経路を選択する（Yao et al., 2023）。計算量は探索幅×深さに依存。
...
一回の推論での軌跡は、モデルの重みを変えない。だが、モデル自身が世代を重ねて学習し直されるとき、軌跡の集合そのものが変形していく場合がある。**Model Collapse**は、生成モデルが自己生成データで再学習すると、生成の多様性が失われる現象である（Shumailov et al., 2023）。
...
- Jing, L., Vincent, P., LeCun, Y., & Tian, Y. (2022). Understanding Dimensional Collapse in Contrastive Self-Supervised Learning. *ICLR 2022*. arXiv: [2110.09348](https://arxiv.org/abs/2110.09348)
