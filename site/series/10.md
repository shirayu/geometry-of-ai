...
> [!TIP]
> 読了後に、[理解度チェック](./10/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#第10回-思考の連鎖-推論の軌跡)でも確認できる。

## 注意事項

- CoTを「多様体上の探索軌跡」と見る解釈は比喩であり、厳密な数学的対応ではない。
- Test-time compute scalingの効果はタスクとモデルに依存し、普遍的ではない。
...
前回、拡散モデルにおいて「ノイズからデータへの軌道」を学習する枠組みを見た。時間発展を明示的に扱うことで、生成プロセスを制御できるようになった。同じ視点は、時間発展を扱うモデルだけのものではない。**推論** にも適用できる。

「ステップバイステップで考えよう」。このひとことをプロンプトに付け足すだけで、正答率が跳ね上がることがある。付け足していないのと同じ情報量しか与えていないのに、である。従来の理解では、Chain of Thought（CoT）は「中間ステップを出力することで、モデルが複雑な推論を分解できる」と説明される。分解、という言葉はもっともらしい。しかし、何を分解しているのか、なぜ分解すると解けるのかまでは、この説明は答えていない。
...
CoTの働き、Test-time compute scaling、そしてModel Collapse（[Shumailov et al., 2023](#ref-shumailov2023)）やDimensional Collapse（[Jing et al., 2022](#ref-jing2022)）といった表現の崩壊現象。これらは一見ばらばらな話題に見える。だが、「表現空間上の移動」という一つの見方でつながっている。
...
直感的には、「最短経路」が最も効率的に思える。実際、単純な問題ではそのとおりである。ところが、表現空間の**地形**を考えると、この直感がそのまま通用しない場面が出てくる。
...
> [!CAUTION]
> **山岳地帯の比喩の限界：** 「険しい領域」「峠」という表現は、表現空間内の何らかの構造（誤りやすさのばらつき）を直感的に伝えるための比喩である。実際の表現空間に、山岳地形のような測定可能な「険しさ」の指標があるわけではなく、どの経路が「峠」に当たるかを事前に特定する方法も確立していない。
...
**Chain of Thought（CoT）**（[Wei et al., 2022](#ref-wei2022)）は、LLMに「ステップバイステップで考える」よう促すプロンプティング技術である（[Kojima et al., 2022](#ref-kojima2022)）。
...
CoTが軌跡の「足場」だとして、その足場は何でできているのか。ここで、Transformerの内部動作を思い出そう。
...
なぜ中間トークンが「足場」として機能するのか。答えたいところだが、正直に言えば、確立した説明はまだない。以下は**もっともらしい仮説**であり、実際には複数のメカニズムが複合的に働いている可能性が高い。
...
三つの仮説と三つの可能性、あわせて六通りの説明が並んだ。どれか一つが正解というより、問題の種類によって効いている仮説が違う、というのが実情に近いのだろう。
...
学習を終えたモデルの重みは固定されている。にもかかわらず、推論時にかける計算量を増やすだけで性能が上がる場合がある。これが**Test-time compute scaling**と呼ばれる現象である。

従来のスケーリング則（[Kaplan et al., 2020](#ref-kaplan2020); [Hoffmann et al., 2022](#ref-hoffmann2022)）は、主に**学習時**のパラメータ数・データ量・計算量に焦点を当てていた。Test-time compute scalingは、**推論時**にも同様のスケーリング効果があることを示唆する。

| スケーリングの種類 | 対象 | 増やすもの |
| --- | --- | --- |
| 学習時スケーリング | モデル学習 | パラメータ、データ、計算量 |
| Test-time スケーリング | 推論 | [推論時の計算量](#計算量を増やす方法-分類) |

### 計算量を増やす方法（分類）

「Test-time compute」とひとまとめに呼んでしまうと、まるで単一のノブを回しているように聞こえる。しかし、実態はそうではない。この括りの中には、**異なるメカニズム**で計算量を増やす手法が同居している。混同すると、「同じ原理で一律に伸びる」という誤解を招く。
...
**2. サンプリング回数の増加（Self-consistency）**：同じ問題に対して複数回（独立に）推論を行い、多数決や集約で最終回答を決定する（[Wang et al., 2023](#ref-wang2023)）。計算量は試行回数に比例。

**3. 探索アルゴリズムの適用（Tree of Thoughts）**：複数の思考経路を木構造で並行して探索し、評価関数で最も有望な経路を選択する（[Yao et al., 2023](#ref-yao2023)）。計算量は探索幅×深さに依存。

**4. 検証と修正（Self-reflection, Verifier）**：生成した回答を別のモデルや同一モデルで検証し、必要に応じて修正・再生成する。計算量は検証ステップ数に依存。

四つとも「計算量を増やす」という結果は同じでも、増やしている対象（トークン数、試行回数、探索幅、検証回数）はそれぞれ別物である。だとすれば、どの問題にどの手法が効くかも、当然そろっては来ない。
...
四つの手法を計算量という軸だけで見ると、ばらばらな工夫の寄せ集めに見える。表現空間という軸に置き直すと、見え方が変わる（ただし、これは直感的な比喩であり、厳密な対応ではない）。
...
「軌跡」という見方のありがたみは、軌跡を許さないモデルと並べてはじめて際立つ。言語モデルの歴史を振り返ると、 **隠れマルコフモデル（HMM）** が重要な位置を占める。
...
HMMの中心的な仮定は**マルコフ性**である。現在の状態は、直前の状態のみに依存し、それ以前の履歴には依存しない。

一つ前だけを見ればよい、というのは計算上は都合がいい。だが、言語モデルとしては大きな制約となる。文の意味は、直前の単語だけでなく、文全体の文脈に依存することが多いからである。

> [!NOTE]
> **対比の単純化について：** ここでのHMMは「一次マルコフモデル」の基本形を指している。実際には、n-gram化（高次マルコフ）、状態の設計の工夫、階層的HMMなど、文脈を拡張する手法も多数存在する。本対比は、「局所的依存 vs 大域的依存」という違いを強調するための単純化である。
...
HMMが縛られていた「直前の状態しか見えない」という制約は、Transformerには課されていない。TransformerのAttention機構は、**系列全体**を参照できる。これにより、マルコフ性の制約を超えた、大域的な依存関係を捉えられる。
...
各ステップで最善を選んでいるのだから、全体としても最善に違いない。そう思いたくなる。しかし、貪欲法は**局所最適**に陥りやすい。各ステップで最良の選択をしても、全体として最良の系列になるとは限らない。
...
どの戦略にも、確実さと多様さのどちらを取るかという同じトレードオフが姿を変えて現れる。
...
一回の推論での軌跡は、モデルの重みを変えない。だが、モデル自身が世代を重ねて学習し直されるとき、軌跡の集合そのものが変形していく場合がある。**Model Collapse**は、生成モデルが自己生成データで再学習すると、生成の多様性が失われる現象である（[Shumailov et al., 2023](#ref-shumailov2023)）。
...
一つの世代だけを見れば、偏りはわずかである。それが世代をまたいで積み重なることで、無視できない収縮になる。幾何学的には、「確率密度の高い領域への収縮力」が働いているようなものである。各世代で、分布の「裾」が切り捨てられ、中心部への集中が進む。
...
三つの対策に共通するのは、収縮の力に逆らうのではなく、多様性を外部から補い続けるという発想である。
...
> [!NOTE]
> **文献の射程について：** Dimensional Collapseに関する代表的な分析（[Jing et al., 2022](#ref-jing2022)）は、主に対照学習（contrastive learning）における表現を対象としている。LLMの埋め込みや内部表現に同様の現象が起こるかどうかは、学習方法（言語モデリング vs 対照学習）や表現の種類（入力埋め込み、中間層、最終層）によって異なる可能性がある。本節の議論は、表現学習一般に対する幾何学的な直感を与えることを目的としており、LLMに直接当てはまると断定するものではない。

### 現象の説明

Model Collapseは世代をまたいだ収縮だった。もう一つ、同じ「収縮」という言葉で語られながら、世代を経ずとも一回の学習の中で起こりうる現象がある。**Dimensional Collapse** は、表現空間の **有効次元が減少** する現象である。
...
名目上の次元数だけを見ていては、この縮退には気づけない。有効ランクという指標を手にしてはじめて、空間の実効的な広さが測れるようになる。
...
「ステップバイステップで考えよう」。導入で立てたこの一文の効き目を、幾何学の言葉に置き換えるところから始まった。答えは、推論を「表現空間上の軌跡」として捉える視点にあった（ただし、これは比喩的な理解であり、数学的に厳密な対応ではない）。
...
**Model CollapseとDimensional Collapse**は、表現や生成の多様性が失われる現象である。幾何学的には「多様体の縮退」や「空間の平坦化」として理解できる。軌跡を辿る能力がどれだけ豊かでも、辿る先の空間自体が痩せていけば、その能力は意味を失う。持続可能なモデル開発において、両者は無視できない問題として残る。
...
[第11回](11.md)「異なる空間をつなぐ」では、異なるモダリティの空間をどう接続するかを考える。
...
比喩のまま終わらせず、ここまでの現象をいくつか手元で再現しておく。有効ランク、Model Collapse、CoT軌跡、ビームサーチの順に並べる。
...
### Model Collapseのシミュレーション（簡略化された概念デモ）

> [!CAUTION]
> 以下のシミュレーションは、2次元データに対する最近傍MSEフィッティングという単純化された生成モデルを使っている。実際のLLMにおけるModel Collapseのメカニズム（高次元の言語分布におけるモード崩壊）を再現したものではなく、「世代を重ねると多様性が失われうる」という定性的な傾向を、単純な例で確認するためのデモである。
...
> [!CAUTION]
> 以下の軌跡は、開始点と終了点を直線・ベジェ曲線で結んで人工的に生成したものであり、実際のLLMの内部状態（hidden states）を観測した結果ではない。「易しい問題は直線的、難しい問題は迂回が必要」という直感を視覚化するための概念デモである。
...
- <a id="ref-wei2022"></a>Wei, J., et al. (2022). Chain-of-Thought Prompting Elicits Reasoning in Large Language Models. *NeurIPS 2022*. arXiv: [2201.11903](https://arxiv.org/abs/2201.11903)
    - CoTの原論文。中間ステップを出力することで推論能力が向上。
- <a id="ref-kojima2022"></a>Kojima, T., Gu, S. S., Reid, M., Matsuo, Y., & Iwasawa, Y. (2022). Large Language Models are Zero-Shot Reasoners. *NeurIPS 2022*. arXiv: [2205.11916](https://arxiv.org/abs/2205.11916)
    - 「Let's think step by step」というシンプルなプロンプトでもCoT効果が得られる。

### 推論と探索

- <a id="ref-yao2023"></a>Yao, S., et al. (2023). Tree of Thoughts: Deliberate Problem Solving with Large Language Models. *NeurIPS 2023*. arXiv: [2305.10601](https://arxiv.org/abs/2305.10601)
    - 複数の思考経路を木構造で探索。
- <a id="ref-wang2023"></a>Wang, X., et al. (2023). Self-Consistency Improves Chain of Thought Reasoning in Language Models. *ICLR 2023*. arXiv: [2203.11171](https://arxiv.org/abs/2203.11171)
    - 複数回のCoTサンプリングと多数決による精度向上。

### スケーリング則

- <a id="ref-kaplan2020"></a>Kaplan, J., et al. (2020). Scaling Laws for Neural Language Models. arXiv: [2001.08361](https://arxiv.org/abs/2001.08361)
    - ニューラル言語モデルのスケーリング則。
- <a id="ref-hoffmann2022"></a>Hoffmann, J., et al. (2022). Training Compute-Optimal Large Language Models. *NeurIPS 2022*. arXiv: [2203.15556](https://arxiv.org/abs/2203.15556)
    - Chinchillaスケーリング則。パラメータとデータの最適バランス。

### Model Collapse

- <a id="ref-shumailov2023"></a>Shumailov, I., Shumaylov, Z., Zhao, Y., Gal, Y., Papernot, N., & Anderson, R. (2023). The Curse of Recursion: Training on Generated Data Makes Models Forget. arXiv: [2305.17493](https://arxiv.org/abs/2305.17493)
    - Model Collapseの分析。自己生成データでの再学習による多様性低下。

### Dimensional Collapse

- <a id="ref-jing2022"></a>Jing, L., Vincent, P., LeCun, Y., & Tian, Y. (2022). Understanding Dimensional Collapse in Contrastive Self-Supervised Learning. *ICLR 2022*. arXiv: [2110.09348](https://arxiv.org/abs/2110.09348)
