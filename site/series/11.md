...
> このページで登場した用語は、[キーワード集](/series/keywords#第11回-感覚の統合-異種多様体の結婚)でも確認できる。
...
機械学習においても、この「感覚の統合」は長年の課題であった。テキスト、画像、音声、動画、それぞれのモダリティは全く異なる形式でデータが表現されている。文法に従う離散的な記号列と、連続的なピクセルの集まりを、同じ土俵に乗せることなど、そもそもできるのだろうか。できない、と思うのが自然な反応である。ところが、CLIPをはじめとする対照学習は、この不可能に見える統合を、驚くほど単純な操作で実現してみせた。
