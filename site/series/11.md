# 第11回：異なる空間をつなぐ ～マルチモーダル表現の幾何学～

> [!TIP]
> 読了後に、[理解度チェック](./11/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#第11回-異なる空間をつなぐ-マルチモーダル表現の幾何学)でも確認できる。

## 注意事項

- CLIPの「同じ空間に整列」は、テキストと画像の両エンコーダ出力が $L_2$ 正規化されている場合に $\cos$ 類似度が角度として解釈できる。
- ここでいう「統一多様体」は、複数モダリティの意味的な共通表現空間という仮説的な設計目標であり、実際に単一の滑らかな多様体になっている保証はない。
- コントラスト学習の「引力と斥力」は比喩的表現。
- ImageBindやONE-PEACEなどの最新手法も、共有空間がどれほど意味を共有しているかについては議論の余地がある。

## 導入：異なる感覚器官を持つ知性

人間は、視覚、聴覚、触覚、言語といった複数の感覚を統合して世界を理解している。目の前のリンゴを見て、その名前を言い、手に取ったときの重さを感じる。これらの情報は、脳内で何らかの形で統合され、「リンゴ」という一貫した概念を形成する。当たり前にできてしまうので、普段はその複雑さに気づかない。

機械学習においても、異なるモダリティの空間をつなぐことは長年の課題であった。テキスト、画像、音声、動画、それぞれのモダリティは全く異なる形式でデータが表現されている。文法に従う離散的な記号列と、連続的なピクセルの集まりを、同じ土俵に乗せることなど、そもそもできるのだろうか。できない、と思うのが自然な反応である。ところが、CLIPをはじめとする対照学習は、この不可能に見える統合を、驚くほど単純な操作で実現してみせた。

本回では、マルチモーダル学習を幾何学的な視点から再検討する（[Baltrusaitis et al., 2019](#ref-baltrusaitis2019)）。特に、CLIPに代表される対照学習が「異なる多様体を同じ空間に整列させる」という操作を、球面幾何学の言葉で理解する。そして、この共有空間への整列がもたらす利点と限界、さらには次世代への示唆を探る。
...
[第0回](00.md)で導入した多様体仮説を思い出そう。高次元空間に埋め込まれたデータは、実際にはより低次元の多様体上に集中している。

この視点に立つと、各モダリティのデータは**異なる多様体**に住んでいると考えられる。テキストの多様体は離散的な構造を反映し、画像の多様体は空間的な連続性を反映する。これらは、同じ高次元空間に埋め込まれていても、**異なる「形」** を持っている。

形の異なる二つの多様体を、どう接続すればよいのか。単純に同じ座標系に押し込めばよい、というわけにはいかないはずである。
...
**CLIP（Contrastive Language-Image Pre-training）**（[Radford et al., 2021](#ref-radford2021)）は、テキストと画像を同じ埋め込み空間に配置するという、シンプルだが強力なアイデアを実現した。
...
$$\mathcal{L} _{t \to i} = -\frac{1}{N}\sum _{i=1}^{N}\log \frac{\exp(\text{sim}(\mathbf{t} _i, \mathbf{i} _i)/\tau)}{\sum _{j=1}^{N} \exp(\text{sim}(\mathbf{t} _i, \mathbf{i} _j)/\tau)}$$
...
CLIPでは、これに加えて画像からテキストへの方向 $\mathcal{L} _{i \to t}$ も計算し、両方向の平均 $\mathcal{L} = (\mathcal{L} _{t \to i} + \mathcal{L} _{i \to t}) / 2$ を最終的な損失とする（対称化）。

CLIPでは、テキストと画像の埋め込みは $L _2$ 正規化されるため、 $\text{sim}(\mathbf{t}, \mathbf{i}) = \cos\theta$ となる。これにより、類似度は純粋に**角度**で測られる。

> [!IMPORTANT]
> **正規化の重要性：** CLIPが「角度ベースの類似度」を実現できるのは、両エンコーダの出力を $L_2$ 正規化しているからである。正規化なしでは、ベクトルのノルム（大きさ）が類似度に影響し、角度の解釈が成り立たなくなる。これは[第3回](03.md)・[第6回](06.md)で議論した正規化の設計原則と整合する。

### 球面上の「引力」と「斥力」

InfoNCE損失は何をしているのか。式の中身を追うだけでは、その動きは見えてこない。幾何学的に解釈し直してみよう。

正規化された埋め込みは、単位球面 $\mathbb{S}^{d-1}$ 上の点として表現される。CLIPの学習は、この球面上で以下の操作を行っていると見なせる：
...
> [!NOTE]
> **「引力と斥力」は比喩：** この物理的メタファーは直感的な理解を助けるが、実際の最適化は勾配降下法で行われており、力学シミュレーションではない。また、InfoNCE損失は負例を「できるだけ $\cos\theta = -1$ に追い込む」わけではなく、**Softmax内での正例の相対的な順位を上げる**圧力が中心である。負例の最終的な配置は、バッチ内の分布状況や温度パラメータに依存する。

### 温度パラメータの役割

温度 $\tau$ は、[第4回](04.md)で議論したSoftmaxの温度と同じ役割を果たす。
...
## 共有空間の問題：「無理やり同じドームに押し込む」歪み

### モダリティ固有の構造が取り出しにくくなる可能性

うまくいっているように見える。しかし、CLIPのアプローチには、根本的な問題がある。異なるモダリティを同じ空間に押し込むことで、**各モダリティに固有の構造が最終表現から取り出しにくくなる**可能性があるのだ。

**画像の空間的隣接関係**：画像では、「猫の顔」と「猫の尻尾」は空間的に離れているが、同じ「猫」という概念に属する。しかし、テキストでは「顔」と「尻尾」は別の単語であり、その空間的関係は表現されない。共有空間では、この画像固有の空間構造が明示的には保持されにくい。

**テキストの構文構造**：テキストでは、主語-動詞-目的語の関係や、修飾関係といった構文構造が意味を担う。画像にはこのような階層的な文法構造は存在しない。共有空間では、この構文的な情報が線形に取り出しにくくなる可能性がある。
...
同じ空間に埋め込まれているのだから、テキストと画像は入り混じって分布していてもよさそうなものだ。ところが、実際の学習済みCLIPモデルを調べると、テキスト埋め込みと画像埋め込みは、同じ空間内でも**異なる領域**に分布していることが観察されている（[Liang et al., 2022](#ref-liang2022)）。これを**モダリティギャップ（modality gap）** と呼ぶ。

球面上で考えると、テキストの埋め込みが集まる領域と、画像の埋め込みが集まる領域が、ある程度分離しているのだ。これは、共有空間での対応が完全ではないことを示唆している。

なぜこの分離が生じるのか。
[Liang et al., 2022](#ref-liang2022)は、**モデルの初期化と対照学習による最適化の組合せ**を原因として分析している。
ランダムに初期化された別々のエンコーダは、それぞれの出力を球面上の狭い錐（cone）状の領域へ配置し、画像とテキストの錐は初期状態から離れうる。
その後の対照学習は対応ペアを引き寄せる一方、バッチ内の他の標本を反発させる項も持つため、初期化由来のギャップを必ずしも閉じない。
論文の分析では、この保存のされ方は温度にも依存する。
したがってモダリティギャップは、単なる学習不足でも、アーキテクチャだけの帰結でもなく、初期表現と対照学習目的の相互作用によって残りうる系統的な偏りである。
...
2. **構成的な理解**：「赤いリンゴの上に青いボール」と「青いリンゴの上に赤いボール」の区別は、属性と物体の組み合わせ（構成性）の理解を必要とするが、CLIPはこれが苦手である（[Yuksekgonul et al., 2023](#ref-yuksekgonul2023)）。
3. **カウンティング**：「3匹の猫」と「5匹の猫」の区別も困難である。

これらの限界は、「共有空間への埋め込み」というアプローチが、モダリティ間の**構造的な対応関係**を完全には捉えられていないことを示唆している。

## 歴史からの教訓：空間を「広げて」解決する

### Frustratingly Easy Domain Adaptation (FEDA)

共有空間への埋め込みだけが、異なる世界を接続する唯一の道なのだろうか。歴史をさかのぼると、正反対の発想で同じ問題に挑んだ例が見つかる。

2007年、Hal Daumé IIIが提案した **FEDA（Frustratingly Easy Domain Adaptation）** は、ドメイン適応という課題に対して、驚くほど単純な「空間の拡張」で解決を図った（[Daumé III, 2007](#ref-daumé2007)）。

当時、ドメイン適応の研究では、手の込んだ特徴設計や学習アルゴリズム側の工夫が多数提案されていた。複雑さで殴り合う競争になっていた、と言ってもよい。そこにFEDAは、**特徴量を3倍にコピーして繋げるだけ**という、たった数行で実装できる手法を持ち込み、多くの設定で既存の強力なベースラインと競争できる性能を示したのである。
...
これは、**共通の性質と個別の性質を幾何学的に分離する**という、明快な設計指針である。ソース固有ブロックとターゲット固有ブロックは、構成上互いに干渉しない（ソースデータではターゲットブロックが0、ターゲットデータではソースブロックが0）という意味で「直交的」である。

### 「情報の棲み分け」という思想

この手法が「腹立たしいほど簡単（Frustratingly Easy）」と呼ばれつつも強力だったのは、複雑な写像を学ぶ代わりに、**空間の構造（直交性）をあらかじめ定義しておく**ことの威力を示したからである。
...
| アプローチ | 幾何学的操作 | 利点 | 課題 |
| --- | --- | --- | --- |
| **CLIP型（共有空間）** | 異なる多様体を強引に重ね合わせる | 共通概念の抽出に強い | 固有構造が干渉しやすい（モダリティギャップ） |
...
2007年の論文が、今の設計にまで影響を残しているとは考えにくい。そう思うかもしれない。しかし、FEDAの精神は、名前こそ表に出る機会は減ったが、類似の発想が現代の最先端技術の中に見られる：
...
- **Mixture of Experts（MoE）**：[第13回](13.md)で扱うこの手法も、「共有知識」と「専門知識」を分離するという点で、情報の棲み分けという考え方を共有している。
...
どちらが優れているかは、タスクとデータ次第である。ただ、**「共有空間への整列」と「分離」のバランスをどう取るか**という問いは、マルチモーダル学習の根本的な課題であり続けている。

## 次世代への示唆：翻訳か共有か

### アプローチ1：モダリティ間の構造を保つ写像

同じ空間に押し込むのではなく、それぞれの空間を保ったまま橋を架けるという道もある。**「翻訳」** というアプローチである。
...
すべての入力に対して同じ「統合の仕方」を適用するのではなく、タスクや文脈に応じて、どのモダリティをどの程度重視するかを動的に決定する。これは[第6回](06.md)で見たAttention機構の精神と通じる。

## 具体例：ImageBindとその先

### ImageBind：6モダリティの統合

CLIPが二つのモダリティで示した設計は、もっと多くのモダリティに広げられるはずである。**ImageBind**（[Girdhar et al., 2023](#ref-girdhar2023)）は、6つのモダリティ（画像、テキスト、音声、深度、熱画像、IMU）を単一の埋め込み空間に統合することを試みた。
...
しかし、ImageBindのような手法が本当に「意味を共有する表現」を実現しているかは、議論の余地がある。
...
**疑問4（画像中心主義）：** ImageBindでは画像エンコーダが固定（Frozen）されており、他のモダリティのエンコーダが画像空間に「合わせにいく」設計になっている。これは**画像中心主義（Visiocentrism、本講義における造語であり確立した学術用語ではない）** とも呼べるバイアスを生む。音声や深度の表現が、画像で表現しやすい概念に偏り、画像では捉えにくいが他のモダリティでは自然な概念（例：音声の韻律、触覚の質感）が適切に表現されない可能性がある。

これらの問いに対する明確な答えはまだない。マルチモーダル学習は、活発な研究領域であり続けている。

> [!IMPORTANT]
> **共有空間における意味対応の程度と質：** 複数のモダリティを「同じ空間に埋め込む」ことと、それらが「意味的に対応している」ことは、異なる。前者は技術的に達成できるが、後者は評価が難しい。ベンチマークでの性能向上が、必ずしも「真の意味共有」を意味するわけではない。

## まとめ：異種空間の架け橋

離散的な記号列と連続的なピクセルが、同じ球面上で隣り合う。冒頭では不可能に見えたその統合が、対照学習という一つの操作でどこまで実現し、どこで綻びを見せたかを振り返る。
...
| **モダリティギャップ** | 異なるモダリティ間の埋め込み領域の分離 | 共有空間の不完全さの指標 |
| **FEDA** | 空間を拡張し、共通/固有情報を直交化する手法 | 「分離」による解決の古典的傑作 |
| **共有空間 vs 翻訳** | 単一空間への埋め込み vs 多様体間の写像 | 次世代設計の選択肢 |
...
だが、「統一多様体」は設計目標であり、達成された事実ではない。モダリティギャップの存在や、構成的理解の限界は、現在の手法がまだ「真の意味共有」には至っていないことを示唆している。

次世代への道筋として、「単一空間への統合」ではなく「翻訳」や「階層的統合」といったアプローチも検討されている。各モダリティの固有構造を尊重しつつ、意味的な対応を実現する。この両立が、マルチモーダル学習の次の課題である。

### 問い

**統一多様体は可能か、それとも複数の多様体を「橋渡し」すべきか。**

この問いに対する答えは、まだ出ていない。それでも、幾何学的な視点を持つことで、「何が失われ、何が保存されているか」は、明確に議論できるようになる。
...
[第12回](12.md)「双曲幾何学 ～負の曲率の世界～」では、球面とは異なる「曲がった空間」を探る。
...
> [!NOTE]
> **バッチサイズと負例の数：** InfoNCE損失の性質上、バッチサイズ $N$ が大きいほど負例（分母の和の項数）が増え、相互情報量の下界がタイトになるため学習効果が高まることが知られている（[Oord et al., 2018](#ref-oord2018)）。以下のコード例では `batch_size=32` としているが、これは教育目的の簡易例である。実際のCLIP学習では数万単位（元論文では32,768）のバッチサイズが重要であり、そのために分散学習や勾配キャッシュなどの技術が用いられる。
...
- <a id="ref-radford2021"></a>Radford, A., et al. (2021). Learning Transferable Visual Models From Natural Language Supervision. *ICML 2021*. arXiv: [2103.00020](https://arxiv.org/abs/2103.00020)
    - CLIPの原論文。対照学習によるテキスト-画像の共有空間を提案。
- <a id="ref-oord2018"></a>Oord, A. v. d., Li, Y., & Vinyals, O. (2018). Representation Learning with Contrastive Predictive Coding. arXiv: [1807.03748](https://arxiv.org/abs/1807.03748)
    - InfoNCE損失を提案。対照学習の理論的基盤。

### ドメイン適応と空間の拡張

- <a id="ref-daumé2007"></a>Daumé III, H. (2007). Frustratingly Easy Domain Adaptation. *ACL 2007*. [https://aclanthology.org/P07-1033/](https://aclanthology.org/P07-1033/)
    - 空間を拡張し、共通/固有情報を直交化する手法を提案。シンプルだが強力なドメイン適応の古典的傑作。

### モダリティギャップ

- <a id="ref-liang2022"></a>Liang, V. W., Zhang, Y., Kwon, Y., Yeung, S., & Zou, J. Y. (2022). Mind the Gap: Understanding the Modality Gap in Multi-modal Contrastive Representation Learning. *NeurIPS 2022*. arXiv: [2203.02053](https://arxiv.org/abs/2203.02053)
    - モダリティギャップを発見・分析した論文。共有空間の限界を示す。

### CLIPの限界と構成性

- <a id="ref-yuksekgonul2023"></a>Yuksekgonul, M., Bianchi, F., Kalluri, P., Jurafsky, D., & Zou, J. (2023). When and Why Vision-Language Models Behave like Bags-of-Words, and What to Do About It. *ICLR 2023*. arXiv: [2210.01936](https://arxiv.org/abs/2210.01936)
    - CLIPの構成的理解の限界を分析。属性と物体の組み合わせにおける問題を指摘。

### ImageBindと多モダリティ統合

- <a id="ref-girdhar2023"></a>Girdhar, R., et al. (2023). ImageBind: One Embedding Space To Bind Them All. *CVPR 2023*. arXiv: [2305.05665](https://arxiv.org/abs/2305.05665)
    - 6モダリティを単一空間に統合。画像をアンカーとした設計を提案。

### マルチモーダル学習のサーベイ

- <a id="ref-baltrusaitis2019"></a>Baltrusaitis, T., Ahuja, C., & Morency, L.-P. (2019). Multimodal Machine Learning: A Survey and Taxonomy. *IEEE TPAMI*, 41(2), 423-443. DOI: [10.1109/TPAMI.2018.2798607](https://doi.org/10.1109/TPAMI.2018.2798607)
