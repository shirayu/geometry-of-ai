...
なぜこの分離が生じるのか。[Liang et al., 2022](#ref-liang2022)は、その一因を**モデルの初期化**に求めている。ランダムに初期化された段階のニューラルネットワークは、出力を球面上の狭い錐（cone）状の領域に押し込める性質を持つ（各層の非線形変換が繰り返されることで、出力の分散が方向によって偏る）。対照学習によって同一ペア（対応する画像とテキスト）を引き寄せても、この初期化由来の偏りが完全には解消されず、モダリティごとに異なる錐状の領域に留まりやすい。つまりモダリティギャップは、学習が未完成だから残る誤差というより、ニューラルネットワークの構造自体に起因する系統的な偏りである。
