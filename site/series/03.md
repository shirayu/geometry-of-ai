...
**nGPT（Normalized GPT）** は、2024年10月にNVIDIAのLoshchilovらが発表したアーキテクチャである（Loshchilov et al., 2025）。そのアイデアは、一部を球面に寄せるような遠慮ではない。**Transformerのすべての表現を単位球面上に制約する**ことである。
...
その答えが **von Mises-Fisher（vMF）分布** である（Mardia & Jupp, 2000; Banerjee et al., 2005）。
...
> [!IMPORTANT]
> **概念と実装のギャップ：** 上記のコードは「nGPT風」の設計を示すものであり、nGPTの正確な実装ではない。nGPTの詳細は原論文（Loshchilov et al., 2025）を参照されたい。
...
- Wang, F., Xiang, X., Cheng, J., & Yuille, A. L. (2017). NormFace: L2 Hypersphere Embedding for Face Verification. *ACM MM 2017*. arXiv: [1704.06369](https://arxiv.org/abs/1704.06369)
    - 顔認証における球面埋め込みの先駆的研究。
