...
**nGPT（Normalized GPT）** は、2024年10月にNVIDIAのLoshchilovらが発表したアーキテクチャである（Loshchilov et al., 2024）。そのアイデアは、**Transformerのすべての表現を単位球面上に制約する**ことである。
...
球面制約は「長さと向きの混乱」を解消する強力な設計だが、万能ではない。ノルムが意味を持つタスクや、階層構造を表現したい場面では別の幾何学（第12回：双曲空間）が有効になる。この「空間の選択」という問いは、本講義を通じて繰り返し問われる。
