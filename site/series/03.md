...
この対応表に、本講義の核心が凝縮されている。星は、広大な空間に離散的に配置された「意味の所在」である。そして重要なのは、その星が**どこにあるか（方向・配置）** だけであり、**どれだけ明るいか（大きさ・ノルム）** ではない。星の明るさをあえて捨てる——これが第2回で見た「ノルムの呪い」への回答である。意味は、星の配置（角度）だけが担う。
...
> $\mathbf{u}= \frac{\mathbf{z}}{\|\mathbf{z}\|} \in \mathbb{S}^{d-1}$ を取ると、 $\mathbf{u}$ は厳密には projected normal distribution に従う。ただし、大 $\kappa$ ・高次元の条件下では vMF 分布で近似できる。
...
| **vMF分布** | 球面上の方向分布。集中度 $\kappa$ が分布の鋭さを制御 | Softmaxとの形式的対応。第4回・第7回に接続 |
...
> [!TIP]
> **Transformerに馴染みのない読者へ：** 本回ではnGPTの設計思想を通じて「球面制約」の意味を理解した。nGPTが改良の対象としているTransformerそのものの全体像——どんな構成要素があり、それぞれが何をしているのか——については、第6回「Transformerという測量士」の導入の後に整理する。第4回・第5回はSoftmaxとマージンという、Transformerに限定されない概念を扱うので、Transformerの詳細を知らなくても読み進められる。
