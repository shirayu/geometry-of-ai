...
> [!IMPORTANT]
> **[第4回](04.md)への伏線：** この対応は、Softmaxを情報幾何学の観点から再解釈する際の出発点となる。温度パラメータ $T$ （または $\kappa$ ）が、確率分布空間でどのような役割を果たすかを、次回詳しく見る。
...
本回で見たvMF分布とSoftmaxの形式的な対応（「内積が指数関数に入る」という共通構造）を出発点として、「温度」パラメータが確率分布の空間でどのような幾何学的意味を持つかを整理する。さらに、自然勾配がなぜ有効なのかを、確率分布空間の曲率構造から説明する。
