...
[第7回「不確実性の復権」](07.md)では、ハルシネーションを「不確実性の表出」として扱った。低い $\kappa$ を検出すれば、片が付く。そう思えた。
...
| [第7回](07.md)（不確実性の復権） | 不確実性推定（無知の捏造）で検出できる範囲と、確信した嘘（誤信念の表出）という検出できない領域の切り分け |
