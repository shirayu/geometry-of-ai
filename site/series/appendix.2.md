...
> このページで登場した用語は、[キーワード集](/series/keywords#appendix-2-多様体の純度問題-幾何学がまだ解けていない課題)でも確認できる。

## 本Appendixの位置づけ

本編は「空間の形」を設計する言語を整えた。形さえ整えば、あとは自然についてくるように思える。ところが、その空間に注ぎ込まれる**データの真偽**については、幾何学は沈黙している。
...
> 表現空間において、論理整合性や事実整合性に反するサンプルが、
> 学習分布にどれだけ**混入しているか**、および学習結果に**どれだけ影響を与えるか**。

- **混入率**：問題のあるサンプルの割合
- **影響度**：そのサンプル（またはクラスター）を除去または重み変更したときの、検証誤差、整合性テスト、推論安定性の変化量

混入率が低くても影響度が高い場合（例：頻出パターンに矛盾するデータ）、純度は低いと見なす。

**扱うこと：** 幾何学的安定化（表現空間の設計）と、データ選別（真偽判定）のギャップ／現行手法の幾何学的翻訳と、その限界／将来の研究方向（仮説レベル）。

**扱わないこと：** 特定アーキテクチャの批評（nGPT、GPT-4など）／社会制度論（誰がデータを管理すべきか）／最終的なアライメント論（何が「正しい」価値観か）。
...
[第3回: プラネタリウムの建設: 球面へのパラダイムシフト](03.md)の限界について。
...
| **EL2Nスコア**（[Paul et al., 2021](#ref-paul2021)） | 予測誤差の大きいデータを選別 | **距離** | モデル・学習段階に依存 | 損失距離の代理 | 「難しい」と「間違い」の区別不能 |
| **Influence Functions**（[Koh & Liang, 2017](#ref-koh2017)） | 除去時の損失変化を逆算 | **曲率** | モデル・近似手法に依存 | Hessian近傍の寄与 | 計算コスト、近似の不安定さ |
| **kNN外れ値検出** | 近傍密度が低い点を除去 | **密度** | 層・正規化・距離関数で結果が激変 | 疎領域の検出 | 「珍しい」と「間違い」の区別不能 |
| **TDA異常検知**（[Carlsson, 2009](#ref-carlsson2009)） | Persistenceの異常を検出 | **位相** | 近傍閾値・埋め込みスケールで位相が変わる | トポロジーの破綻 | 高次元での計算困難 |
...
> 内在次元が低い場合や、強い構造がある場合は緩和されうる（[Levina & Bickel, 2004](#ref-levina2004); [Facco et al., 2017](#ref-facco2017)）。
> ただし、汎用的なデータ選別ツールとしては信頼性が不足している。

### 鶏と卵問題

異常を検出する基準を、どこに置けばよいのか。曲率異常や位相異常を検出するには、まず「正しい多様体」が存在する必要がある。
ところが、その多様体自体が「ゴミデータを含んで学習されている」可能性がある。
...
三角不等式は常に成り立つ。しかし、この式が保証するのは「AとCの距離が、AB間とBC間の距離の和を超えない」という**対称的な**近さの制約にすぎない。論理的含意はそもそも**非対称**な関係であり（「AならばB」が成り立っても「BならばA」が成り立つとは限らない）、対称な距離をいくら精密に測っても、この向きの情報は復元できない。同様に **「反例の存在」** （ $\exists x : \neg P(x)$ ）も、表現距離だけでは表現できない。
...
### [第7回](07.md)との対比で見る純度問題の深刻さ

[第7回: 点から分布へ: 不確実性を表現する幾何学](07.md)では、ハルシネーションを「不確実性の表出」として扱った。低い $\kappa$ を検出すれば、片が付く。そう思えた。
ところが、本Appendixで問題とするハルシネーションは、それとは**本質的に異なる**。

**違いの要点：**
> [第7回](07.md)は「**無知の捏造**」を扱った。
...
- **無知の捏造**：集中度 $\kappa$ が低い状態。分布が広がり、方向 $\boldsymbol{\mu}$ の不確実性が高い。
- **誤信念の表出**：集中度 $\kappa$ は高いが、方向 $\boldsymbol{\mu}$ が誤っている状態。確信を持って間違った方向を指す。

| 観点 | [第7回](07.md): 無知の捏造 | Appendix 2: 誤信念の表出 |
| ------ | ----------------- | ------------------------- |
| **メカニズム** | 知識の欠如から生じる曖昧な予測 | 訓練データの誤りから学習した確信 |
| **幾何学的状態** | 集中度 $\kappa$ が低い<br>星がない空白地帯で、無理やり星座を結んでいる | 集中度 $\kappa$ は高いが方向 $\boldsymbol{\mu}$ が嘘<br>「1+1=3」という偽の星が、はっきりと刻まれている |
...
[第7回](07.md)の不確実性推定は「知らないことを知っている」状態を検出できる。
これは重要な進歩だが、**「間違って知っている」状態**には無力である。

**具体例：**

| 質問 | [第7回](07.md)で検出可能なケース | Appendix 2で問題となるケース |
| ------ | ------------------------- | ---------------------------- |
...
「原理的な識別不能性」にて述べたように、幾何学は**内在的性質**（距離、曲率、位相）しか見ない。この制約が、ここでどう効いてくるのか。
...
**予告：**
続編「情報幾何学とAIの動態論」[第8回](08.md)では、Chain of Thoughtのエネルギー地形を扱う。
...
| 静的（本Appendix） | 動的（続編） |
| ------------------- | -------------- |
| 誤った方向 $\boldsymbol{\mu}$ に高い集中度 $\kappa$ | 偽の極小値（shallow local minimum） |
| データの純度問題 | エネルギー地形の汚染 |
| 幾何学単独では識別不可 | 軌道の追跡で部分的に診断可能？（研究中） |

> [!NOTE]
> [第7回](07.md)の「無知の捏造」は **entropy-based detection** で対処できる。
...
> 影響度 = そのサンプルを除去または重み変更したときの、検証誤差、整合性テスト、推論安定性の変化量。
...
| Tool Use（外部検証） | 計算や検索ツールで事実を検証 | - | **性能**：検証成功率・コスト・レイテンシのPareto改善 | 幾何学的手法ではない |

### 成功指標の分類

- **整合性ベンチ**：推移律違反率、矛盾テスト、事実検証成功率など（純度の「論理整合性と事実整合性」に対応）
...
| 異常の現れ方 | 表現配置の歪み | 望ましくない推論経路として現れる可能性 |
| 現状の対処 | 外部フィルタリング | デコーディング制御、外部ツールによる検証 |
...
**予告：**
続編の動態論では、異常データに由来する表現の歪みを、推論経路の偏りとして観測・修正できるかという仮説を検討する予定である。
ただし、「浅い谷へのトラップ」とハルシネーションを同一視できるという確立した結果はなく、現時点では検証すべき研究上の問いである。
...
| **ハルシネーションの2類型** | 誤信念の表出（確信した嘘）と無知の捏造（不確実な嘘）は異なる問題 | 対処法も異なる |
...
| 講義回 | 接続点 |
| --- | --- |
| [第3回](03.md)（球面と正規化） | 正規化が安定させるのは「計算の形式」であり、「データの内容」の真偽ではないという限界の出発点 |
| [第7回](07.md)（点から分布へ） | 不確実性推定（無知の捏造）で検出できる範囲と、確信した嘘（誤信念の表出）という検出できない領域の切り分け |
| [第13回](13.md)（スパース性/高次元） | 高次元での距離の集中が、幾何学的なデータ選別を難しくする要因の一つ |
| [第10回](10.md)（CoTの軌跡） | CoTを一連の中間出力として捉える視点。ただし、出力テキストは内部状態の軌道そのものではなく、データ品質との因果関係も未解明 |
...
- <a id="ref-koh2017"></a>Koh, P. W., & Liang, P. (2017). Understanding Black-box Predictions via Influence Functions. *ICML 2017*, 1885–1894. arXiv: [1703.04730](https://arxiv.org/abs/1703.04730)
    - 影響関数による個別訓練データの予測への寄与を定量化。データ品質評価の理論的基礎。
- <a id="ref-paul2021"></a>Paul, M., Ganguli, S., & Dziugaite, G. K. (2021). Deep Learning on a Data Diet: Finding Important Examples Early in Training. *NeurIPS 2021*. arXiv: [2107.07075](https://arxiv.org/abs/2107.07075)
