...
第2回で「ノルムの呪い」として高次元の問題に触れた。あのとき見た性質は、実は入り口に過ぎない。本回では議論をさらに深め、高次元空間の性質が**恐怖**（予測不可能性、敵対的サンプル）と**祝福**（表現力、スパース性の活用）の両面を持つことを示す。そして、この性質を積極的に活用するMixture of Experts（MoE）アーキテクチャへと議論を接続する。
