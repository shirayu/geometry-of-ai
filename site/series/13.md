...
これは、「最も近い点」と「最も遠い点」の区別がつきにくくなることを意味する。kNNの根幹である「近さ」の概念が曖昧になるのだ（Beyer et al., 1999）。
...
> また、高次元空間での近傍探索には**Hubness問題**が知られている。これは、特定の点（hub）が多くのクエリの最近傍として過剰に選ばれる現象であり、コサイン類似度でも完全には解消されない（Radovanović et al., 2010; Angiulli, 2018）。対策として、相互近傍（mutual nearest neighbors）や局所スケーリング（local scaling）などの手法が提案されている。
...
代表的な手法とその幾何学的アイデアを整理しよう（Indyk & Motwani, 1998; Jégou et al., 2011; Malkov & Yashunin, 2020）：
...
- **Mixtral**（Jiang et al., 2023）: 8 Experts, Top-2 routing
...
- **Soft MoE**（Puigcerver et al., 2024）: 離散的ルーティングを連続化（[Appendix 1](appendix.1.md)「離散と連続の界面」との接続）
- **Llama 4**（Meta AI, 2025）: MoEを採用（ただしLlama 3はdense）
...
- Angiulli, F. (2018). On the Behavior of Intrinsically High-Dimensional Spaces: Distances, Direct and Reverse Nearest Neighbors, and Hubness. *Journal of Machine Learning Research*, 18(170), 1–60.
...
- Indyk, P., & Motwani, R. (1998). Approximate Nearest Neighbors: Towards Removing the Curse of Dimensionality. *STOC 1998*, 604–613.
    - LSH（局所性鋭敏型ハッシュ）による近似近傍探索の理論的基礎。
- Jégou, H., Douze, M., & Schmid, C. (2011). Product Quantization for Nearest Neighbor Search. *IEEE Transactions on Pattern Analysis and Machine Intelligence*, 33(1), 117–128. DOI: [10.1109/TPAMI.2010.57](https://doi.org/10.1109/TPAMI.2010.57)
    - 積量子化によるコンパクトなベクトル表現と高速近似近傍探索。大規模ベクトルDBの基礎技術。
- Malkov, Y., & Yashunin, D. (2020). Efficient and Robust Approximate Nearest Neighbor Search using Hierarchical Navigable Small World Graphs. *IEEE Transactions on Pattern Analysis and Machine Intelligence*, 42(4), 824–836. arXiv: [1603.09320](https://arxiv.org/abs/1603.09320) (2016)
    - HNSWグラフによる近似近傍探索。現代のベクトル検索で広く参照・実装される代表的アルゴリズム。

### RAGとMoE

- Lewis, P., et al. (2020). Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks. *NeurIPS 2020*, 9459–9474. arXiv: [2005.11401](https://arxiv.org/abs/2005.11401)
...
- Puigcerver, J., Riquelme, C., Mustafa, B., & Houlsby, N. (2024). From Sparse to Soft Mixtures of Experts. *ICLR 2024*. arXiv: [2308.00951](https://arxiv.org/abs/2308.00951)
...
- Yang, M., et al. (2024). Hyperbolic Fine-Tuning for Large Language Models (HypLoRA). arXiv: [2410.04010](https://arxiv.org/abs/2410.04010)
