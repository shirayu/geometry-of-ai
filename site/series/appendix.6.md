...
> [!IMPORTANT]
> **ここから $\boldsymbol{\theta}$ の指すものが切り替わる：** [第4回](04.md)や[Appendix 4](appendix.4.md)・[Appendix 5](appendix.5.md)での $\boldsymbol{\theta}$ は、分布を指定する少数のパラメータ（3クラス分類なら、確率 $\boldsymbol{\pi}$ に相当する3次元程度）を指していた。本Appendixでは $\boldsymbol{\theta}$ を**ネットワークの重み $\mathbf{w}$ 全体**（同じ3クラス分類器でも数百万次元以上）とみなし、これを「統計モデルのパラメータ」として形式的にFisher情報行列の定義に代入する。すなわち、ネットワークの出力 $p(y \mid \mathbf{x}; \mathbf{w})$ を1つの確率モデルとみなし、その重み $\mathbf{w}$ を動かしたときの感度を測る、という立場である。この読み替えにより、「学習で実際に動かす対象（重み）」と「Fisher情報行列」を直接結びつけて論じられるようになる（対応の全体像は[第0回](00.md)を参照）。
...
このカルバック・ライブラー・ダイバージェンス $K(w)$ が特異点で退化している（微分が0になる方向が存在する）とき、特異点解消により各局所座標チャート $\alpha$ の新しい座標 $u$ で

$$K(g_\alpha(u)) = u_1^{2k_{\alpha 1}} u_2^{2k_{\alpha 2}} \cdots u_d^{2k_{\alpha d}}$$

という正規交差形に書き直せる。
ただし、RLCTは $K$ の指数だけでは決まらない。
事前分布の密度と座標変換のヤコビアンを合わせた因子も、同じチャートで

$$\varphi(g_\alpha(u))\lvert g_\alpha'(u)\rvert = b_\alpha(u)\prod_{j=1}^d \lvert u_j\rvert^{h_{\alpha j}}$$

と表される。
ここで $b_\alpha(u)$ は正の解析関数である。
一般のRLCTは

$$\lambda = \min_\alpha \min_{j:k_{\alpha j}>0} \frac{h_{\alpha j}+1}{2k_{\alpha j}}$$

で与えられる。
多重度 $m$ は、大まかには大域的な最小値 $\lambda$ を同時に達成する座標軸の最大数であり、最小値を与えるチャート間でも最大を取る。
$h_{\alpha j}=0$ の単一チャートだけを見る場合に限り、 $\lambda=\min_j 1/(2k_{\alpha j})$ という簡略式になる。
したがって、指数 $k_{\alpha j}$ が大きい方向ほど谷を広げる一方、事前分布と座標変換に由来する指数 $h_{\alpha j}$ も谷の実効的な広さに寄与する。
