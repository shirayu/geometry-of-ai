...
> [!NOTE]
> **本回の「星」とは：** 見出しの「星」は、凝縮の終点として立ち上がる**生成されたデータ点**を指す。第7回で「星」が概念（埋め込み）を、その周りの「霧」が不確実性を表したのとは役割が少し異なる。本回では、一様な霧（ガウスノイズ）から一点の星（データ）が凝縮していく過程に注目する。
...
**vMF分布との接続**：球面上の「ガウス分布」であるvon Mises-Fisher（vMF）分布（第3回）を、拡散過程の終端として自然に扱える。
