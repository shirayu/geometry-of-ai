...
> [!TIP]
> 読了後に、[理解度チェック](./09/quiz)で要点を確認できる。
> このページで登場した用語は、[キーワード集](/series/keywords#第9回-拡散と凝縮-熱力学との融合)でも確認できる。

## 注意事項

- 標準的な拡散モデル（DDPM等）はユークリッド空間 $\mathbb{R}^d$ 上で定義される。最終状態はガウス分布。
- 「球面上の拡散」は別の定式化であり、標準手法とは異なる。
- SDE/ODEの式は概念的な表現であり、実装では離散化や近似が入る。
- スコア関数の推定は、実際にはノイズ予測ネットワークを介して行われる。理論と実装の対応に注意。
- 本回で扱う「時間」は、物理的な時間ではなく、拡散と逆拡散のプロセスの「進行度」を指す。

## 導入：霧から星が凝縮する

前回、「時間の発見」として、生成プロセスを軌道として捉える視点を導入した。Neural ODEは、離散的な層の積み重ねを連続時間の流れとして再解釈する枠組みを与えた。層を重ねるという操作が、実は一本の連続した道のりだったという発見である。

その道のりを今度は逆走できるとしたら、どうなるか。軸になるのは**確率過程**の導入である。

拡散モデル（Diffusion Models）は、近年の生成AIの中核技術となっている（[Ho et al., 2020](#ref-ho2020)）。画像生成（Stable Diffusion, DALL-E）、動画生成（Sora）、音声合成など、あらゆるモダリティで成功を収めている。これほど広く使われている以上、背後にはよほど複雑な理論があるはずだ、と身構えてしまうかもしれないが、その直感は驚くほどシンプルである。

**インクを水に垂らすと拡散する**。最初は明確な形を持っていたインクが、時間とともに広がり、最終的には一様に薄まる。これが **Forward Process（順過程）** である。ここまでは、誰もが経験として知っている。

では、この過程を「逆再生」できたらどうなるか。一様に拡散したインクが、徐々に凝縮し、元の形を取り戻す。現実には起こりえない光景だが、これが **Reverse Process（逆過程）** であり、拡散モデルの生成プロセスそのものである。

> [!NOTE]
> **本回の「星」とは：** 見出しの「星」は、凝縮の終点として立ち上がる**生成されたデータ点**を指す。[第7回](07.md)で「星」が概念（埋め込み）を、その周りの「霧」が不確実性を表したのとは役割が少し異なる。本回では、一様な霧（ガウスノイズ）から一点の星（データ）が凝縮していく過程に注目する。

> [!NOTE]
> **熱力学との類似（ただし比喩である）：** この描像は、熱力学における「エントロピー増大」と「時間の矢」を想起させる。拡散は自然に起こるが、その逆は自然には起こらない。ただし、拡散モデルの逆過程は**物理法則を時間反転しているわけではない**。Forward Processとは**別の確率過程を学習して構成**している（[Sohl-Dickstein et al., 2015](#ref-sohldickstein2015)）。熱力学的な比喩は直感を助けるが、物理的可逆性とは異なる概念である。

「逆再生」という言い方は比喩として便利だが、そのままでは数式にならない。以下では、この直感を数学的に定式化し、幾何学的な視点から再解釈していく。SDE（確率微分方程式）とODE（常微分方程式）の対応、スコア関数の意味、そして「ベクトル場に沿った積分」という視点が、その手がかりになる。

## 標準的な拡散モデル： $\mathbb{R}^d$ 上の定式化

### Forward Process：データからノイズへ

「逆再生」を作るには、まず「再生」そのもの、つまりデータを徐々にノイズで汚していく過程を正確に定義しなければならない。これが **Forward Process** である。

標準的な定式化（VP-SDE: Variance Preserving SDE）では、以下のSDEで記述される（[Song et al., 2021b](#ref-song2021)）：
...
SDEを1000ステップ刻みで律儀に解いていくのだろうか、と身構えたくなるところだが、そうはならない。このSDEには**解が閉形式で書ける**という、扱いやすい性質がある。
...
$$
\text{データ } \mathbf{x}_0 \;\longrightarrow\;
\mathbf{x}_t = \sqrt{\bar{\alpha}_t}\\,\mathbf{x}_0 + \sqrt{1-\bar{\alpha}_t}\\,\boldsymbol{\epsilon}
\quad(\text{任意の } t \text{ へ直接ジャンプ可能})
$$

### 最終状態：ガウス分布への収束

この式を時間の果てまで追っていくと、行き着く先は一つに定まる。 $t \to \infty$ で $\bar{\alpha}_t \to 0$ となるため、Forward Processの最終状態は：

$$\mathbf{x}_\infty \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$$

すなわち、**標準ガウス分布**に収束する。元のデータがどんな形をしていたかにかかわらず、である。

> [!IMPORTANT]
> **ガウス分布と球面の違い：** ガウス分布 $\mathcal{N}(\mathbf{0}, \mathbf{I})$ は $\mathbb{R}^d$ 全体に広がる分布であり、球面 $\mathbb{S}^{d-1}$ 上の一様分布とは全く異なる。高次元では、ガウス分布のサンプルは「原点から $\sqrt{d}$ 程度の距離」に集中するため、「薄い球殻」に近い振る舞いを見せるが、これは球面上の分布ではない。

### Reverse Process：ノイズからデータへ

最終状態が一様なガウス分布だとすれば、そこから元のデータへ戻る道筋は無数にありそうに思える。だが、Forward Processを時間反転した **Reverse Process** は、一つのSDEとして具体的に書き下せる：
...
Reverse Processの式に現れた $\nabla _{\mathbf{x}} \log p_t(\mathbf{x})$ は、記号の見た目こそいかめしいが、正体はそう複雑ではない。スコア関数 $\nabla_{\mathbf{x}} \log p(\mathbf{x})$ は、確率分布の「勾配」を表す。

抽象的な定義だけでは像を結びにくい。具体的に考えてみよう。1次元のガウス分布 $p(x) = \mathcal{N}(0, 1)$ の場合：
...
では、このスコア関数をニューラルネットワークに学習させればよい。単純にそう構えたいところだが、話はそこで止まる。スコア関数を直接学習するのは困難である。 $p_t(\mathbf{x})$ は未知であり、正規化定数の計算も難しい。

**スコアマッチング**（[Hyvärinen, 2005](#ref-hyvärinen2005)）は、この問題を回避する手法である（[Song & Ermon, 2019](#ref-song2019)）。以下の損失関数を最小化することで、スコア関数を近似するネットワーク $s_\theta(\mathbf{x}, t)$ を学習できる：
...
Forward Processの閉形式解 $\mathbf{x}_t = \sqrt{\bar{\alpha}_t} \mathbf{x}_0 + \sqrt{1 - \bar{\alpha}_t} \boldsymbol{\epsilon}$ は、 $\mathbf{x}_0$ を固定すれば $\mathbf{x}_t$ が平均 $\sqrt{\bar{\alpha}_t}\mathbf{x}_0$ 、分散 $(1-\bar{\alpha}_t)\mathbf{I}$ のガウス分布に従うことを意味する。したがって：

$$p_{t|0}(\mathbf{x}_t \mid \mathbf{x}_0) \propto \exp\left(-\frac{\|\mathbf{x}_t - \sqrt{\bar{\alpha}_t}\mathbf{x}_0\|^2}{2(1-\bar{\alpha}_t)}\right)$$

この対数を $\mathbf{x}_t$ について微分すると：

$$\nabla _{\mathbf{x} _t} \log p _{t|0}(\mathbf{x}_t \mid \mathbf{x}_0) = -\frac{\mathbf{x}_t - \sqrt{\bar{\alpha}_t}\mathbf{x}_0}{1-\bar{\alpha}_t} = -\frac{\boldsymbol{\epsilon}}{\sqrt{1 - \bar{\alpha}_t}}$$

最後の等号は、閉形式解を $\boldsymbol{\epsilon} = (\mathbf{x}_t - \sqrt{\bar{\alpha}_t}\mathbf{x}_0)/\sqrt{1-\bar{\alpha}_t}$ について解いて代入したものである。これが「ノイズ予測」と「スコア推定」が等価である理由である。
...
Reverse SDEには、一つ厄介な性質がある。確率的であり、同じ初期ノイズから異なるサンプルが生成される。再現性がほしい場面では、これは扱いにくい。

しかし、**同じ周辺分布** $p_t(\mathbf{x})$ を保ちながら、**決定論的**に状態を遷移させる方法がある。これが **確率フローODE（Probability Flow ODE）** である：
...
**2. 高速サンプリング**：ODEソルバーは、SDEサンプラーより少ないステップで収束することが多い。DDIM（[Song et al., 2021a](#ref-song2021denoising)）やDPM-Solver（[Lu et al., 2022](#ref-lu2022)）などの高速サンプラーは、この性質を利用している。
...
ここまでの式変形を追うと、微分方程式を解いているだけのようにも見える。だが視点を変えると、もう一つの読み方ができる。確率フローODEの右辺は、 $\mathbb{R}^d$ 上の**ベクトル場**を定義する。各点 $\mathbf{x}$ に対して、「次にどの方向に動くべきか」を指示するベクトルが割り当てられている。
...
ここで $\eta$ は**ステップサイズ**（学習率のようなハイパーパラメータ）である。 $k \to \infty$ で、 $\mathbf{x}_k$ の分布は $p(\mathbf{x})$ に収束する（適切な条件下で）（[Welling & Teh, 2011](#ref-welling2011)）。
...
先ほどのLangevin動力学の式を、Reverse Processの式と並べてみると、見覚えのある形に気づく。Langevin動力学は、拡散モデルの **Reverse Process（離散化版）** と構造的に類似している。
...
Langevin動力学の起源をたどると、拡散モデルより古い時代の試みに行き着く。**エネルギーベースモデル（Energy-Based Model, EBM）** は、確率分布を以下の形で表現する（[LeCun et al., 2006](#ref-lecun2006)）：

$$p(\mathbf{x}) = \frac{\exp(-E(\mathbf{x}))}{Z}$$

ここで $E(\mathbf{x})$ は**エネルギー関数**、 $Z = \int \exp(-E(\mathbf{x})) d\mathbf{x}$ は**分配関数**（正規化定数）である。

エネルギー関数 $E(\mathbf{x})$ 自体はニューラルネットワークで自由に表現できる。問題はむしろ別のところにある。EBMの問題は、 $Z$ の計算が高次元では一般に困難なことである。

### スコアマッチングによる突破

ところが、対数を取って勾配を取るという一手間だけで、この壁は消える。スコア関数は、 $Z$ に依存しない：
...
ここまでスコア関数を式として扱ってきたが、その正体は数式の中だけにとどまるものではない。幾何学的には以下のように解釈できる：
...
word2vecの「王 - 男 + 女 = 女王」というベクトル演算は、かつては「おもちゃのような例題（toy problem）」と見なされることもあった。「たまたま上手くいく例」に過ぎないのではないか、と。ベクトルの足し引きが意味の演算になるというのは、確かに都合が良すぎる話に聞こえる。

ところが、確率フローODEの視点から見ると、これを**ベクトル場に沿った積分**という枠組みで**再解釈**することができる。

$$\text{king} \xrightarrow{-\text{male direction}+\text{female direction}} \text{queen}$$
...
ここまでSDE、スコア関数、ODE、Langevin動力学と個別に見てきたが、これらはすべて同じものを別の角度から照らしていたに過ぎない。拡散モデルの本質は、「確率分布の時間発展」を制御することである。
...
graph LR
    ...
    classDef state fill:#F8FAFC,stroke:#475569,stroke-width:1.3px,color:#0F172A;
    class RT,Rd,R2,R1,R0 state;
    style R fill:#F8FAFC,stroke:#64748B,stroke-width:1.2px,color:#0F172A;
    linkStyle 0,1,2,3 stroke:#2E8B57,stroke-width:2px;
```

$$
\text{Reverse: } p_T \approx \mathcal{N}(\mathbf{0}, \mathbf{I}) \to p_t \to p_0 = p_{\text{data}}
$$

この「確率分布の時間発展」という視点は、空間が $\mathbb{R}^d$ であれ球面であれ変わらない。空間の選択は、分布の「形」や「収束先」に影響するが、「時間発展を制御する」という基本原理は変わらない。

## Flow Matching：共通の定式化

### Flow Matchingとは

だとすれば、スコアやノイズという迂回路を経由せず、時間発展そのものを直接学習してしまう道もあるはずだ。近年、**Flow Matching**（[Lipman et al., 2023](#ref-lipman2023)）という枠組みが注目を集めている理由は、まさにそこにある。これは、拡散モデルと連続正規化フロー（Continuous Normalizing Flows）を統一的に扱う視点を提供する。
...
スコアを学習することと、ベクトル場を学習することは、どこまで同じでどこから違うのか。Flow Matchingは、拡散モデルの一般化と見なせる。
...
インクは水に垂らせば拡散する。逆再生は現実には起こらない。その素朴な観察から出発したはずが、気づけばSDE、スコア関数、ODE、EBM、そしてword2vecまでが一本の線でつながっていた。
...
この「流れの制御」という見方は、対象を $\mathbf{x}$ 空間の外へ向けても崩れない。以下、二つの発展的トピックで確かめる。

- **球面上の拡散**は別の定式化であり、最終状態は球面上の一様分布である。標準手法とは異なるが、方向データや正規化された表現との親和性を持つ。
- **学習の熱力学**は、生成過程（ $\mathbf{x}$ 空間）とは別に、学習過程（ $\boldsymbol{\theta}$ 空間）を不可逆輸送として捉える視点を与える。
...
ここまでは、データ生成プロセス（ $\mathbf{x}$ 空間での拡散と凝縮）を熱力学的に捉えてきた。同じ描像は、学習そのもの、つまりパラメータが変化していく過程にも及ぶ。ここで視点を一段階上げ、**「ニューラルネットワークが学習するプロセスそのもの」** を熱力学的に記述する最新の理論を紹介する（[Okanohara, 2026](#ref-okanohara2026)）。

これは、学習を「モデルのパラメータ分布（アンサンブル）の有限時間輸送」として定式化し、学習に伴うコストを熱力学的な「仕事」と「散逸」に対応づける野心的な枠組みである。

### 学習とは「パラメータ分布の輸送」である

学習を指して「モデルが賢くなる」と言うのは日常の言い回しとしては自然だが、この理論はもう少し踏み込んだ言い方を要求する。拡散モデルの生成過程は「データ分布の変形」だったが、学習過程は **「パラメータ分布（または仮説空間）の変形」** と見なせる。
...
損失関数が熱力学的な仕事の上界だとすれば、学習には避けられない代償があることになる。この理論の重要な帰結は、**認識論的コスト (Epistemic Costs)** の概念である。

情報理論における**ランダウアーの原理**（情報の消去には物理的なエネルギーコストが伴う）を学習に応用すると、モデルが外部データから情報を獲得し、内部状態（重み）を更新して不確実性を減らすこと自体に、物理的なコストの**理論的な下限**が存在することが示唆される。

- **学習のコスト**: 重みの更新 $\approx$ 情報の書き込みと古い情報の消去（忘却）
- **不可逆性**: 一度学習した（収束した）状態から、自然に元のランダムな初期状態に戻ることはない（学習における時間の矢）。

拡散モデルが「ノイズを除去して画像を凝縮させる」のと同様に、学習アルゴリズム（SGD等）は「パラメータ空間のノイズ（不確実性）を除去して知識を凝縮させる」熱機関として機能しているという**物理的な解釈**が可能になる（※これは情報処理の不可逆性に関する理論的な話であり、GPUの消費電力を直接表すものではない）。

> [!IMPORTANT]
> **生成と学習の区別:**
>
> - **拡散モデル（生成）**: $\mathbf{x}$ 空間（画像空間）での分布変形。ノイズ $\to$ データ。
> - **学習の熱力学**: $\boldsymbol{\theta}$ 空間（パラメータ空間）での分布変形。初期重み $\to$ 学習済み重み。
>
...
ここまで述べた標準的な拡散モデルは、 $\mathbb{R}^d$ 上で定義され、最終状態はガウス分布である。だが、この講義でここまで繰り返し扱ってきたのは、データが球面上に住んでいる場面だった。ならば拡散も球面上で定義できるはずだ、という発想は自然である。

実際、**球面 $\mathbb{S}^{d-1}$ 上の拡散**という別の定式化も研究されている。この場合：

- **状態空間**：単位球面 $\mathbb{S}^{d-1} \subset \mathbb{R}^d$
...
| 球面拡散 | $\mathbb{S}^{d-1}$ | 球面上の一様分布 | 方向データ、分子構造 |

### 球面拡散の動機

わざわざ球面上で定義し直す価値はどこにあるのか。球面拡散が有用な場面として、以下が挙げられる：

**方向データ**：風向、分子の結合角など、量そのものが「方向」であるデータ。

**正規化された表現**：nGPT（[第3回](03.md)）のように、すべての表現を単位球面上に制約するアーキテクチャとの親和性。

**vMF分布との接続**：球面上の「ガウス分布」であるvon Mises-Fisher（vMF）分布（[第3回](03.md)）を、拡散過程の終端として自然に扱える。

いずれも、データの持つ構造をそのまま活かせるという意味で理にかなっている。ただし、理にかなっていることと、枯れた技術であることは別の話である。実際、ユークリッド空間用の拡散を球面上の表現に直接適用すると、更新に球面の法線方向の成分が混入するため、点は球面から外れ、一般にノルムは保存されない。
...
[第10回](10.md)「思考の連鎖」では、推論過程を幾何学的な軌跡として扱う。
...
数式の上では美しく閉じていたForward Processも、実装に落とすと地味な部品の組み合わせになる。標準的な拡散モデル（DDPM, DDIM）は $\mathbb{R}^d$ 上で実装される。
...
ノイズスケジュール、q_sample、損失関数。ここまでの式に出てきた記号は、コードの上ではこの3つの部品に対応する。
...
スコア関数は式の上では「対数密度の勾配」でしかないが、ベクトル場として描いてみると、分布のモードへ吸い込まれていく様子が一目でわかる。
...
最後に、発展的トピックで触れた球面上の拡散を、ごく簡略化した形で動かしてみる。コード内のコメントにもあるとおり、あくまで直感をつかむためのデモであり、厳密な離散化ではない。
...
- <a id="ref-ho2020"></a>Ho, J., Jain, A., & Abbeel, P. (2020). Denoising Diffusion Probabilistic Models. *NeurIPS 2020*. arXiv: [2006.11239](https://arxiv.org/abs/2006.11239)
    - DDPMの原論文。現代の拡散モデルの基礎。
- <a id="ref-sohldickstein2015"></a>Sohl-Dickstein, J., Weiss, E., Maheswaranathan, N., & Ganguli, S. (2015). Deep Unsupervised Learning using Nonequilibrium Thermodynamics. *ICML 2015*. arXiv: [1503.03585](https://arxiv.org/abs/1503.03585)
    - 拡散モデルの初期の定式化。非平衡熱力学との接続。

### スコアベース生成モデル

- <a id="ref-song2019"></a>Song, Y., & Ermon, S. (2019). Generative Modeling by Estimating Gradients of the Data Distribution. *NeurIPS 2019*. arXiv: [1907.05600](https://arxiv.org/abs/1907.05600)
    - スコアマッチングに基づく生成モデル。
- <a id="ref-song2021"></a>Song, Y., Sohl-Dickstein, J., Kingma, D. P., Kumar, A., Ermon, S., & Poole, B. (2021b). Score-Based Generative Modeling through Stochastic Differential Equations. *ICLR 2021*. arXiv: [2011.13456](https://arxiv.org/abs/2011.13456)
    - SDEに基づく統一的な定式化。VP-SDE、VE-SDE、確率フローODEを導入。

### 高速サンプリング

- <a id="ref-song2021denoising"></a>Song, J., Meng, C., & Ermon, S. (2021a). Denoising Diffusion Implicit Models. *ICLR 2021*. arXiv: [2010.02502](https://arxiv.org/abs/2010.02502)
    - DDIMの原論文。確率フローODEに基づく高速サンプリング。
- <a id="ref-lu2022"></a>Lu, C., Zhou, Y., Bao, F., Chen, J., Li, C., & Zhu, J. (2022). DPM-Solver: A Fast ODE Solver for Diffusion Probabilistic Model Sampling in Around 10 Steps. *NeurIPS 2022*. arXiv: [2206.00927](https://arxiv.org/abs/2206.00927)
    - 高速ODEソルバー。10～20ステップでの高品質サンプリング。

### Flow Matching

- <a id="ref-lipman2023"></a>Lipman, Y., Chen, R. T. Q., Ben-Hamu, H., Nickel, M., & Le, M. (2023). Flow Matching for Generative Modeling. *ICLR 2023*. arXiv: [2210.02747](https://arxiv.org/abs/2210.02747)
    - Flow Matchingの原論文。拡散モデルと連続正規化フローの統一。

### スコアマッチング

- <a id="ref-hyvärinen2005"></a>Hyvärinen, A. (2005). Estimation of Non-Normalized Statistical Models by Score Matching. *JMLR*, 6, 695–709. [https://jmlr.org/papers/v6/hyvarinen05a.html](https://jmlr.org/papers/v6/hyvarinen05a.html)
    - スコアマッチングの原論文。

### エネルギーベースモデル

- <a id="ref-lecun2006"></a>LeCun, Y., Chopra, S., Hadsell, R., Ranzato, M., & Huang, F. J. (2006). A Tutorial on Energy-Based Learning. *Predicting Structured Data*, MIT Press. [http://yann.lecun.com/exdb/publis/pdf/lecun-06.pdf](http://yann.lecun.com/exdb/publis/pdf/lecun-06.pdf)
    - EBMのチュートリアル。

### Langevin動力学

- <a id="ref-welling2011"></a>Welling, M., & Teh, Y. W. (2011). Bayesian Learning via Stochastic Gradient Langevin Dynamics. *ICML 2011*. [https://ora.ox.ac.uk/objects/uuid:13ec7031-519b-4223-81cb-dd8de9213836](https://ora.ox.ac.uk/objects/uuid:13ec7031-519b-4223-81cb-dd8de9213836)
    - 確率的勾配Langevin動力学（SGLD）。ベイズ学習への応用。

### 学習の熱力学

- <a id="ref-okanohara2026"></a>Okanohara, D. (2026). A Thermodynamic Theory of Learning I. arXiv: [2601.17607](https://arxiv.org/abs/2601.17607)
    - 学習をパラメータ分布の有限時間輸送として定式化し、学習コストを熱力学的な仕事と散逸に対応づける理論。
