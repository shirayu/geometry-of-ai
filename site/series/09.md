...
拡散モデル（Diffusion Models）は、近年の生成AIの中核技術となっている。画像生成（Stable Diffusion, DALL-E）、動画生成（Sora）、音声合成など、あらゆるモダリティで成功を収めている。これほど広く使われている以上、背後にはよほど複雑な理論があるはずだ、と身構えてしまうかもしれない。

ところが、その直感は驚くほどシンプルである。
