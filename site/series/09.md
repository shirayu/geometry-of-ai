...
だとすれば、スコアやノイズという迂回路を経由せず、時間発展そのものを直接学習してしまえないだろうか。近年、**Flow Matching**（Lipman et al., 2023）という枠組みが注目を集めている理由は、まさにそこにある。これは、拡散モデルと連続正規化フロー（Continuous Normalizing Flows）を統一的に扱う視点を提供する。
...
ここまでは、データ生成プロセス（ $\mathbf{x}$ 空間での拡散と凝縮）を熱力学的に捉えてきた。では、学習そのもの、つまりパラメータが変化していく過程にも同じ描像が使えるだろうか。ここで視点を一段階上げ、**「ニューラルネットワークが学習するプロセスそのもの」** を熱力学的に記述する最新(2026.01)の理論を紹介する。
