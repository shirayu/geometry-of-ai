...
このような変形を **連続変形（homeomorphism）** と呼び、連続変形で移り合える図形は「トポロジー的に同じ」（同相）とみなす。
...
深層学習モデルは、入力データを高次元の表現空間に写像する。この表現空間のトポロジー的構造が、モデルの性能や振る舞いに影響を与える可能性がある。
...
> [!CAUTION]
> **計算コストに注意：** パーシステントホモロジーの計算量は、点の数 $n$ に対して最悪 $O(n^3)$ 程度になる。大規模なデータセットでは、サブサンプリングや近似手法（例：Ripser++）の使用を検討すること。
