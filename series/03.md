...
<img src="03_tangent_space.svg" alt="球面上の接空間とリーマン勾配" width="100%">

図: 球面 $S^{d-1}$ 上の点 $\mathbf{x}$ における接空間 $T_\mathbf{x} S^{d-1}$ とリーマン勾配。ユークリッド勾配 $\nabla f(\mathbf{x})$ から半径成分を除去することで、接空間内のリーマン勾配 $\text{grad}_S f(\mathbf{x})$ が得られる。実装では、ユークリッド更新後に正規化（射影）する近似法がよく用いられる。
