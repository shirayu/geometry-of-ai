...
> [!NOTE]
> **条件付きガウスとしての解釈：** vMF分布は、等方的なガウス分布を単位球面上に条件付けた分布として導出できる。すなわち、 $\mathbf{x} \sim N(\boldsymbol{\mu}, \sigma^2 I)$ を $\|\mathbf{x}\| = 1$ に条件付けると、vMF分布に従う（ $\kappa$ は $\|\boldsymbol{\mu}\|/\sigma^2$ に対応）。この関係から「球面上の正規分布」と呼ばれることがあるが、数学的には異なる性質も持つ。
...
def demonstrate_instability():
    ...
    # float16では小さい値がアンダーフローしやすい
    # float16の最小正規化数は約 6e-5
    small_fp16 = torch.tensor([1e-5, 1e-5, 1e-5], dtype=torch.float16)
    
    print(f"入力 (float16): {small_fp16}")
    print(f"ノルム: {small_fp16.norm()}")  # 非常に小さい、または0
    
    # eps なしで正規化すると不安定
    # （ノルムが0に近いとInfやNaNが発生しうる）
    norm = small_fp16.norm()
    if norm < 1e-6:
        print("Warning: ノルムが非常に小さい → 正規化が不安定")
    
    # eps ありで正規化（安全）
    normalized_safe = F.normalize(small_fp16.float(), eps=1e-12).half()
    print(f"Safe (eps使用): {normalized_safe}")
