...
>
> 両者とも関数 $f$ を用いるが、それをどう反復（離散の場合）または積分（連続の場合）してプロセスを定義するかが根本的に異なる。
>
...
> - 出力の意味： **「移動先の座標」** そのもの。
> - 直感イメージ：**「テレポート」**。
>   入力 $\mathbf{x}$ を、関数が一撃で別の地点 $\mathbf{y}$ へワープさせる。（※VAEは確率的なサンプリングを含むが、逐次積分を行わず「一回の処理」で生成する点でこちらに分類される。）始点と終点の関係のみが重要であり、その間の連続的な変化（軌跡）は明示されない（ブラックボックスとなる）。
>
...
> - 直感イメージ：**「海流」**。
>   入力 $\mathbf{x}$ は、その地点にある矢印に従って **「少しずつ押し流される」** 。微小な変化を足し合わせ続ける（積分・反復する）ことで、初めて移動というプロセスが生じる。
