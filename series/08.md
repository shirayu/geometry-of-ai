...
> **ベクトル場とは:**
>
> ニューラルネットワークを単なる「ベクトル値関数（入力と出力がベクトルの関数）」とみなす視点と「ベクトル場」とみなす始点ｊは、決定的な解釈の違いがある。
>
...
> 「写像（Map）」と「場（Field）」も数式上はいずれも $\mathbf{y} = f(\mathbf{x})$ と書けるが、**出力 $\mathbf{y}$ の意味**が根本的に異なる。
>
> **写像としての解釈（GANやVAEなど）**出力は**「移動先の座標」**そのものである。イメージは**「テレポート」**。
> 入力 $\mathbf{x}$ を、一撃で別の地点 $\mathbf{y}$ へワープさせる。
> 始点と終点の関係のみが重要であり、その間の経路やプロセスはブラックボックスとなる。
>
> ベクトル場としての解釈（ResNetやNeural ODE）は、**「その地点における速度ベクトル」**である。
> イメージは**「海流」**。
> 入力 $\mathbf{x}$ は、その地点にある矢印 $f(\mathbf{x})$ に従って **「押し流される」** 。
> $\mathbf{x}$ に微小な変化 $f(\mathbf{x})$ を足し合わせ続ける（積分する）ことで、初めて移動というプロセスが生じる。
>
> ResNetやDiffusion Modelの本質は、ネットワークに「完成品（座標）」を直接作らせるのではなく、 **「状態をどう変化させるべきかというルール（ベクトル場）」** を学習させた点にある。
