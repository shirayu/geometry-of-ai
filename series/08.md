...
入力 h(0)
  |
  |  ← ベクトル場 f(h, t, θ) が各地点で「どっちへ進むか」を指示
  |     （この f だけがニューラルネットワーク）
  ↓
...
ResNetは「層」という固定された離散ステップで状態を変換する。Chef が決まった工程数で料理を仕上げるようなものだ。工程を増やしたければ、材料（パラメータ）を増やすしかない。
...
> [!NOTE]
> **f が受け取る引数について：** $f(\mathbf{h}, t, \theta)$ は状態 $\mathbf{h}$ と時刻 $t$ の両方を入力として受け取る。時刻 $t$ を入力に含めることで、「時刻によって押す方向を変える」非自律系（non-autonomous）のベクトル場を表現できる。時刻を使わない設計（自律系）はよりシンプルだが、表現力が下がる。
...
確率密度の連続時間変化を記述するには、ベクトル場 $f$ の **divergence（ヤコビアンのトレース $\operatorname{tr}(\partial f/\partial \mathbf{h})$）** を積分する必要がある。このトレースを厳密に求めるには、自動微分でJacobian-vector productを $d$ 回実行する必要があり（各基底方向に1回ずつ）、高次元ではスケールしなかった。
