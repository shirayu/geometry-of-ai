...
>
> ニューラルネットワークを単なる「ベクトル値関数（入力と出力がベクトルの関数）」とみなす視点と、「ベクトル場」とみなす視点には、決定的な解釈の違いがある。
>
> **ベクトル場** とは、ある空間の**あらゆる点**に対して、一つの **ベクトル（矢印）** が割り当てられている状態を指す。天気図の風向図や、川の流速分布をイメージするとわかりやすい。空間のどの位置にいても、「その地点に来たら、どの方向へ、どのくらいの速さで進むべきか」という指示書（矢印）が敷き詰められている状態である。
>
> 両者は数式上 $f(\mathbf{x})$ という関数を用いるが、**出力の使い方（プロセスの定義）**が根本的に異なる。
>
...
> - 出力の意味：**「移動先の座標」**そのもの。
> - 直感イメージ：**「テレポート」**。
>   入力 $\mathbf{x}$ を、関数が一撃で別の地点 $\mathbf{y}$ へワープさせる。（※VAEは確率的なサンプリングを含むが、逐次積分を行わず「一回の処理」で生成する点でこちらに分類される。）始点と終点の関係のみが重要であり、その間の経路はブラックボックスとなる。
>
> **2. ベクトル場としての解釈（ResNetやNeural ODE、拡散モデル）**
>
> - 数式イメージ： $\mathbf{x}_{t+1} = \mathbf{x}_t + \Delta t \cdot f(\mathbf{x}_t)$ （離散） または $\frac{d\mathbf{x}}{dt} = f(\mathbf{x}, t)$ （連続）
> - 出力の意味：**「その地点における更新量（速度ベクトル）」**。
> - 直感イメージ：**「海流」**。
>   入力 $\mathbf{x}$ は、その地点にある矢印 $f(\mathbf{x})$ に従って **「少しずつ押し流される」** 。微小な変化を足し合わせ続ける（積分・反復する）ことで、初めて移動というプロセスが生じる。（※ResNetはステップ幅 $\Delta t=1$ と置いた更新則とみなせる）
>
> ResNetが残差を学習することや、Diffusion Modelがノイズ除去の「方向」を学習して徐々に画像を生成するのは、ネットワークに「完成品（座標）」を直接作らせるのではなく、**「状態をどう変化させるべきかというルール（ベクトル場）」**を学習させていると解釈できる。（拡散モデルは本質的には確率過程だが、実装上はODE/SDEの積分として扱えるため）
> これにより、深層学習は「一撃の召喚術」から、時間をかけた「プロセスの制御」へと進化したといえる。
> [!NOTE]
