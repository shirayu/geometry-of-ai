...
Neural ODEが2018年にNeurIPS Best Paperを受賞したとき、深層学習の研究者たちはシンプルな問いに打ちのめされた。
...
ResNetでは、各層 $\mathbf{W}_1, \mathbf{W}_2, \ldots, \mathbf{W}_L$ が異なるパラメータを持つ。深くするほどパラメータ数は増える。
...
Neural ODEの発表から数か月後、**FFJORD**（Grathwohl et al., 2019）が登場した。従来の正規化フロー（Normalizing Flows）は、確率密度の変換に使う写像の**ヤコビアン行列式**を計算する必要があった。これが $d \times d$ 行列の行列式を求める作業で、高次元では計算量が $O(d^3)$ となり、実用上の障壁になっていた。

FFJORD はNeural ODEのベクトル場を使い、密度の時間変化もODEで記述できることを示した。さらにハッチンソン推定量を組み合わせることで、ヤコビアン行列式の計算を $O(d)$ に落とした。高次元空間での正確な確率密度の学習——これは、次世代の生成モデルの基盤となる問いである。
...
Flow Matchingは「どんなベクトル場を学習すれば、目標の確率分布に到達できるか」という問いを、シンプルな回帰問題に変換した。拡散モデルの複雑なスコアマッチングとは異なり、直線的な軌道（単純なODE経路）を目標として設定することで、学習が大幅に安定した。現在のStable Diffusion 3やFlux等の画像生成モデルはこの技術を採用しており、Neural ODEの子孫が最前線で動いている。
...
**ニューラルネットワークとODEの統合**：これが最も根本的な転換だった。「深層学習は写像を近似する」という自己理解から、「深層学習はベクトル場を学習し、それを積分することで変換を実現する」という理解へ。パラメータを共有した無限深さ、勾配計算のODE定式化、ODEソルバーとの統合——これらは、後のFJJORD、Latent ODE、そしてFlow Matchingへと連なる道を開いた。現在の高性能画像生成モデル（Stable Diffusion 3、Flux等）がFlow Matchingを採用していることを考えると、2018年の論文が現在の生成AIの根幹に直接つながっていることが分かる。
...
- Chen, R. T. Q., Rubanova, Y., Bettencourt, J., & Duvenaud, D. (2018). Neural Ordinary Differential Equations. *NeurIPS 2018*. arXiv: [arXiv:1806.07366](https://arxiv.org/abs/1806.07366).
    - Neural ODEの原論文。連続深度モデルと随伴法による効率的な学習を提案。NeurIPS Best Paper受賞。
...
- Grathwohl, W., Chen, R. T. Q., Bettencourt, J., Sutskever, I., & Duvenaud, D. (2019). FFJORD: Free-form Continuous Dynamics for Scalable Reversible Generative Models. *ICLR 2019*. arXiv: [arXiv:1810.01367](https://arxiv.org/abs/1810.01367).
    - Neural ODEを正規化フローへ応用。ヤコビアン行列式計算をO(d)に削減。
