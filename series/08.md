...
>
> ニューラルネットワークを単なる「ベクトル値関数（入力と出力がベクトルの関数）」とみなす視点と、「ベクトル場」とみなす視点には、**本質的な解釈の違い**がある。
>
...
> - 出力の意味：**「移動先の座標」**そのもの。
> - 直感イメージ：**「テレポート」**。
>   入力 $\mathbf{x}$ を、関数が一撃で別の地点 $\mathbf{y}$ へワープさせる。（※VAEは確率的なサンプリングを含むが、逐次積分を行わず「一回の処理」で生成する点でこちらに分類される。）始点と終点の関係のみが重要であり、その間の経路は明示されない（ブラックボックスとなる）。
>
...
> - 直感イメージ：**「海流」**。
>   入力 $\mathbf{x}$ は、その地点にある矢印に従って **「少しずつ押し流される」** 。微小な変化を足し合わせ続ける（積分・反復する）ことで、初めて移動というプロセスが生じる。（※ResNetはステップ幅 $\Delta t=1$ と置いた更新則、拡散モデルは確率過程（SDE）だが生成時は積分の反復として実装できるため、ベクトル場的に解釈できる。）
>
> ResNetが残差を学習することや、Diffusion Modelがノイズ除去の「方向」を学習して徐々に画像を生成するのは、ネットワークに「完成品（座標）」を直接作らせるのではなく、**「状態をどう変化させるべきかというルール（ベクトル場）」**を学習させているといえる。
> これにより、深層学習は「一撃の召喚術」から、時間をかけた「プロセスの制御」へと進化した。
