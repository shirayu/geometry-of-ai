...
> [!NOTE]
> **熱力学との類似：** この描像は、熱力学における「エントロピー増大」と「時間の矢」を想起させる。拡散は自然に起こるが、その逆は自然には起こらない。拡散モデルは、この「逆再生」を学習によって実現する。
...
多峰性の分布では、スコアは「最も近いモードに向かう」方向を指す。
...
Langevin動力学は、拡散モデルの**Reverse Process（離散化版）**とほぼ同じ形をしている。
...
### word2vecの再評価：ベクトル演算の復権

ここで、一見無関係に思える話題を接続しよう。

word2vecの「王 - 男 + 女 = 女王」というベクトル演算は、かつては「おもちゃのような例題（toy problem）」と見なされることもあった。「たまたま上手くいく例」に過ぎないのではないか、と。

しかし、確率フローODEの視点から見ると、これは**極めて厳密な力学計算**として再評価できる。

$$\text{「王」} \xrightarrow{-\text{「男」方向}+\text{「女」方向}} \text{「女王」}$$

この操作は、「意味空間における接ベクトルに沿った積分」と解釈できる。

> [!NOTE]
> **「昔の直線は、実は接ベクトルの断片だった」：** word2vecの線形演算は、巨大な意味多様体の上を走る「接ベクトル」の断片と見なせる。現代の生成モデルは、この断片を無数に繋ぎ合わせ、滑らかな「思考の道」を作れるようになった。確率フローODEは、まさにその「繋ぎ合わせ」の数学的表現である。
...
def ddim_sample(model, schedule, shape, steps=50, eta=0.0, device="cpu"):
    ...
    for i in range(steps):
        ...
        alpha_t_next = alphas_cumprod[int(t_next)] if t_next >= 0 else torch.tensor(1.0)
        ...
        sigma_t = (
            eta * torch.sqrt((1 - alpha_t_next) / (1 - alpha_t)) * torch.sqrt(1 - alpha_t / alpha_t_next)
        )
...
注意: これは教育目的の簡略化された実装であり、
実際の球面拡散モデルはより複雑な数学（リーマン計量、測地線など）を必要とする。
"""
...
**word2vecの「ベクトル演算」**は、確率フローODEの視点から再評価できる。「意味空間における接ベクトルに沿った積分」として、より厳密な力学的解釈を与えることができる。
