...
しかし、標準的なSoftmax損失は「正解クラスの確率を上げる」だけで、クラス間の分離を明示的に強制しない。その結果、学習された表現は必ずしも「同じクラスは近く、異なるクラスは遠く」という構造を持たない。
...
> [!NOTE]
> **なぜArcFaceが主流になったか：** ArcFaceは角度空間で一定のマージンを課すため、(1) 幾何学的解釈が明快、(2) すべての角度で一様なマージン効果、(3) 顔認識ベンチマークで優れた性能、という利点がある。現在、顔認識・顔照合の分野では事実上の標準手法となっている。
...
| 距離ベース vs 確率ベース | 角度 = 距離 ≈ 負の対数確率（vMFの視点） |
| 境界の明示 vs 確率の推定 | 両方を同時に行う設計が可能 |

> [!NOTE]
> **vMF分布との接続：** 第3回で見たvon Mises-Fisher分布を思い出そう。vMF分布では、確率密度が $\exp(\kappa \boldsymbol{\mu}^\top \mathbf{x})$ に比例する。これは「角度が小さいほど確率が高い」ことを意味し、角度と確率の自然な対応を与える。ArcFaceのマージンは、この対応の上で「正解クラスへの要求を厳しくする」操作と見なせる。
