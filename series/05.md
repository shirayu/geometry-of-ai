...
> [!TIP]
> **数学的背景：** ラグランジュの未定乗数法を用いて最適化問題を解くと、重みベクトル $\mathbf{w}$ は以下のようにデータ点の線形結合で表される：$$\mathbf{w} = \sum_{i=1}^{n} \alpha_i y_i \mathbf{x}_i$$ここで、 $\alpha_i > 0$ となるデータ点 $\mathbf{x}_i$ こそがサポートベクターである。それ以外の点では $\alpha_i = 0$ となり、決定境界の数式には一切寄与しない。
