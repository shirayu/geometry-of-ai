...
> [!NOTE]
> **ハードマージンとソフトマージン：** 上記の定式化は **ハードマージンSVM** であり、データが完全に線形分離可能であることを前提とする。現実のデータでは分離不可能な場合が多いため、**ソフトマージンSVM** が使われる。ソフトマージンでは、スラック変数 $\xi_i \geq 0$ を導入し、制約を $y_i(\mathbf{w}^\top \mathbf{x}_i + b) \geq 1 - \xi_i$ に緩和する。正則化パラメータ $C$ がマージン最大化と誤分類許容のトレードオフを制御する。
...
> [!NOTE]
> **大円になる条件：** 境界が大円になるのは、分類が $\mathbf{w}^\top \mathbf{x}$（バイアス項なし）の符号で決まる場合である。これは、ArcFaceのように正規化された特徴と重みの内積で分類を行う設計と整合する。一般にバイアス $b$ を含めると、境界は原点を通らない超平面との交線となり、球面上では大円ではなく小円になる。
