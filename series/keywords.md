...
## Appendix 3: 多様体の品質管理 ～幾何学的評価指標～

学習された多様体が、データの真の構造をどれだけ忠実に、かつ効率的に捉えているかを定量化する。

### 構造の忠実度

| キーワード | 説明 |
| --- | --- |
| **信頼性 (Trustworthiness)** | 低次元に投影した後、近傍にある点は元の高次元空間でも近傍であったかを測る指標。 |
| **連続性 (Continuity)** | 元の空間で近傍だった点が、低次元投影後も近傍に保たれているかを測る指標。 |
| **局所構造の保存** | 多様体の微小な領域における距離関係が、変換前後でどれだけ維持されているかの度合い。 |

### 表現の効率性と分離性

| キーワード | 説明 |
| --- | --- |
| **内在次元推定** | データが実質的に分布している最小限の次元数。これがモデルの容量に対して小さすぎると冗長、大きすぎると情報不足となる。 |
| **線形分離可能性** | 多様体上のクラス（概念）が、単純な超平面でどれだけきれいに切り分けられるかという「意味の整理整頓」の度合い。 |
| **特異値スペクトル** | 表現行列の特異値の分布。多様体が特定の次元に潰れているか、あるいは空間を有効に使っているかを診断する。 |

## Appendix 4: 世界モデルの幾何学 ～整合性と予測～

AIが単なる統計的予測を超え、物理法則や因果関係を内包した「世界の地図」を構築できているか。

### 内部表現の整合性

| キーワード | 説明 |
| --- | --- |
| **世界モデル (World Models)** | 外部環境の動態を内部的にシミュレーションする仕組み。幾何学的には、環境の変化を多様体上の「遷移」として記述する。 |
| **幾何学的整合性** | AIが予測する未来の状態が、物理的・論理的な制約（多様体の壁）を突き破らず、整合した経路上にあるという性質。 |
| **反実仮想の軌跡** | 「もし〜だったら」という仮定に基づき、未経験の状況を多様体上の新たな経路として探索・予測する能力。 |

### 因果と制約

| キーワード | 説明 |
| --- | --- |
| **因果多様体** | 原因から結果への流れが、時間軸に沿った一方向の「流れ」として構造化された空間。 |
| **不変表現** | 視点や照明の変化といった「本質に関わらない変形」に対して、多様体上の位置が変化しない（あるいは予測可能に変化する）性質。 |
| **ハミルトン力学の導入** | エネルギー保存則などの物理的制約をニューラルネットワークの損失関数に組み込み、軌道の正しさを保証する試み。 |
