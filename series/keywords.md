...
| **接空間** | 多様体上の点の近傍を「平ら」に近似する線形空間。リーマン計量はこの空間に内積を与える。 |
...
| **コサイン類似度** | ベクトルのなす角に基づく類似度尺度。角度の重要性の文脈で登場。 |
...
| **リーマン最適化** | 多様体制約を満たすように、接空間とリトラクションを用いて最適化する枠組み。 |
...
| **分配関数** (Log-Partition) | 全ての候補を正規化する項。SoftmaxやCRFで $\log \sum \exp$ として現れる。 |
...
| **VAE** | 潜在分布の推定とサンプリングを通じ、確率的に再構成を行う生成モデル。 |
...
| **Layer Normalization** | サンプル内の特徴方向で正規化する手法。 |
...
| **ベクトル場** | 各点に「進むべき方向」を与える場。確率フローODEの右辺は生成のベクトル場になる。 |
...
| **von Mises-Fisher (vMF) 分布** | 球面上の「ガウス分布」に相当する分布。球面拡散の文脈で重要。 |
...
| **双曲空間** | 負の曲率を持つ空間。中心から離れるほど周囲長が指数的に増大し、階層構造に適する。 |
...
| **双曲距離** | 双曲空間における距離。境界に近いほど同じユークリッド差でも距離が急増する。 |
| **Poincaré Embeddings** | ポアンカレ円板への埋め込みで階層構造を低歪みに表現する手法。 |
| **リーマン最適化** | 多様体上での勾配法。指数写像・対数写像を用いた更新が標準となる。 |
| **階層的埋め込み** | 木構造（親子関係）を、双曲空間の「原点からの距離（階層の深さ）」と「角度（分岐）」として自然に表現する手法。 |
| **Mixed-curvature spaces** | 球面・双曲・ユークリッドなど複数の曲率を組み合わせた表現空間。 |
...
| キーワード | 説明 |
| --- | --- |
| **距離の集中** | 次元が大きいほど点間距離が平均付近に集中し、近い・遠いの区別が曖昧になる現象。 |
| **赤道集中現象** | 高次元球面では、表面積の大半が赤道付近に集中する性質。 |
| **ランダム直交性** | 高次元でランダムベクトル同士がほぼ直交になる性質。 |
| **内在次元** | データが実際に分布している多様体の次元。アンビエント次元との差が重要。 |
| **kNNの破綻** | 距離の集中により、最近傍と最遠傍の差が小さくなり近傍検索が不安定になる問題。 |
| **コサイン類似度** | 角度に基づく類似度。正規化後はユークリッド距離と順位が一致する。 |
...
| **MoE (Mixture of Experts)** | 入力に応じて一部のExpertのみを活性化するスパースなモデル。 |
| **ルーティング (Top-k)** | どのExpertを使うかを決める機構。内積ベースのkNN的選択と解釈できる。 |
...
| **Mixed-curvature spaces** | 複数の曲率を組み合わせた空間設計の研究方向。 |
