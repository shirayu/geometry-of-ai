...
**幾何学的視点：積空間上のエネルギー最小化**

長さ $T$ の系列を考える。各時刻 $t$ でのラベルは $K$ 個のカテゴリのいずれかを取る。Softmaxは各時刻 $t$ で独立に確率単体 $\Delta^{K-1}$ 上の点を選ぶ。つまり、出力空間は **積空間** $(\Delta^{K-1})^T$ である。

この積空間は $K^T$ 個の頂点（すべての可能な系列）を持つ高次元の凸多面体である。しかし、そのすべての頂点が「意味のあるパス」というわけではない。CRFは、この空間に **エネルギー関数** を導入することで、妥当なパスに高い確率を、矛盾したパスに低い確率を割り当てる。

CRFのエネルギー関数は以下の形を取る：

$$E(\mathbf{y} | \mathbf{x}) = \sum_{t=1}^{T} \psi_t(y_t, \mathbf{x}) + \sum_{t=1}^{T-1} \phi(y_t, y_{t+1})$$

ここで、

- $\psi_t(y_t, \mathbf{x})$ は **発散ポテンシャル**（emission potential）：時刻 $t$ で入力 $\mathbf{x}$ からラベル $y_t$ への親和性（これがSoftmaxのロジットに相当）
- $\phi(y_t, y_{t+1})$ は **遷移ポテンシャル**（transition potential）：隣接するラベル間の整合性を表す重み

系列全体の確率は、エネルギーのボルツマン分布として定義される：

$$p(\mathbf{y} | \mathbf{x}) = \frac{1}{Z(\mathbf{x})} \exp(-E(\mathbf{y} | \mathbf{x}))$$

ここで $Z(\mathbf{x})$ は分配関数（すべての可能な系列にわたる和）である。

**Softmaxとの対比：点 vs パス**

| 側面 | Softmax（独立） | CRF（構造化） |
| --- | --- | --- |
| 出力空間 | 各時刻の単体 $\Delta^{K-1}$ | 積空間 $(\Delta^{K-1})^T$ |
| 決定単位 | 各時刻の「点」 | 系列全体の「パス」 |
| 制約 | なし（各時刻独立） | 遷移ポテンシャル $\phi$ による隣接制約 |
| 幾何学的解釈 | 単体への射影 | 積空間上のエネルギー地形 |
| 計算量 | $O(TK)$ | $O(TK^2)$（動的計画法） |

**構造化予測の祖としてのCRF**

CRFは、単なる「系列ラベリングの道具」ではない。それは **構造化予測** （structured prediction）という広大な分野の出発点である。画像セグメンテーション、構文解析、グラフ上のラベル付けなど、多くの問題は「確率空間における経路の妥当性の制約」として定式化できる。

深層学習の時代においても、この視点は健在である。Transformer+CRFのハイブリッドモデル（Devlin et al., 2019のBERT-CRFなど）は、局所的な文脈理解（Transformer）と大域的な系列整合性（CRF）を組み合わせることで、固有表現抽出などで高い性能を発揮する。

さらに、この「局所的な決定を大域的な制約で組織化する」という発想は、注意機構やグラフニューラルネットワークにも通底している。CRFは、深層学習以前の知恵が現代の構造化予測にどう引き継がれているかを示す好例である。

> [!NOTE]
> **計算効率とViterbiアルゴリズム：** CRFで最も確からしい系列を見つけるには、 $K^T$ 通りすべてを評価するのではなく、**Viterbiアルゴリズム**（動的計画法）を使う。これは、幾何学的には「積空間上の測地線探索」に対応し、 $O(TK^2)$ で最適パスを求められる。この効率化こそが、CRFを実用的にする鍵である。

> [!NOTE]
> **ニューラルCRF：** 現代の実装では、発散ポテンシャル $\psi_t$ をニューラルネットワーク（BiLSTMやTransformer）で計算することが多い。遷移ポテンシャル $\phi$ は学習可能な $K \times K$ 行列として扱う。この組み合わせにより、深層学習の表現力とCRFの構造化予測が統合される。
...
- Devlin, J., Chang, M.-W., Lee, K., & Toutanova, K. (2019). BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding. *NAACL 2019*. arXiv: [arXiv:1810.04805](https://arxiv.org/abs/1810.04805).
    - BERTの原論文。BERT+CRFのハイブリッドモデルは固有表現抽出で広く使われる。
