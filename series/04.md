...
この因子分解により、CRFは局所的なスコア（各時刻の出力ポテンシャル $\psi_t$ と隣接時刻間の遷移ポテンシャル $\phi$ ）の和として、系列全体のエネルギーを定義する。特に、**線形鎖構造（各時刻が前後の時刻とのみ依存する）** を仮定することで、動的計画法が適用でき、 $K^T$ 次元の確率空間を扱いながら計算量を $O(TK^2)$ に抑えられる。
...
- $\psi_t(y_t, \mathbf{x})$ は **出力ポテンシャル**（emission potential）：時刻 $t$ で入力 $\mathbf{x}$ からラベル $y_t$ への親和性を表すスコア（独立Softmaxのロジットに相当）
- $\phi(y_t, y_{t+1})$ は **遷移ポテンシャル**（transition potential）：隣接するラベル $y_t$ から $y_{t+1}$ への遷移の好ましさを表す学習可能な重み

系列全体の確率は、エネルギーのボルツマン分布として定義される：

$$p(\mathbf{y} | \mathbf{x}) = \frac{1}{Z(\mathbf{x})} \exp(-E(\mathbf{y} | \mathbf{x}))$$

ここで $Z(\mathbf{x})$ は分配関数（すべての可能な系列 $K^T$ 通りにわたる和）である。
...
- Souza, F., Nogueira, R., & Lotufo, R. (2019). Portuguese Named Entity Recognition using BERT-CRF. *arXiv:1909.10649v2*.
