...
この因子分解により、CRFは局所的なスコア（各時刻の出力ポテンシャル $\psi_t$ と隣接時刻間の遷移ポテンシャル $\phi$ ）の和として、系列全体のエネルギーを定義する。これにより、 $K^T$ 次元の確率空間を扱いながら、計算量を $O(TK^2)$ に抑えられる。
...
| --- | --- | --- |
| 出力空間 | 各時刻の単体 $\Delta^{K-1}$ | 系列上の単体 $\Delta^{K^T-1}$ |
...
- Souza, F., Nogueira, R., & Lotufo, R. (2020). Portuguese Named Entity Recognition using BERT-CRF. *arXiv:1909.10649*.
