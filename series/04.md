...
カテゴリカル分布 $p(k) = \pi_k$ の場合、フィッシャー情報行列の構造は座標系の取り方に依存する。 $\pi_1, \ldots, \pi_K$ をそのまま座標として扱うと、 $\sum_k \pi_k = 1$ の制約により行列は特異（退化）になる。独立な $(K-1)$ 次元座標に落とすと、オフ対角成分も現れる。直感的には、「確率が小さいカテゴリほど、わずかな変化で分布が大きく変わる（感度が高い）」ということであり、この性質は座標系によらず成り立つ。
...
> [!NOTE]
> **自然勾配の実用化：** 自然勾配の計算はフィッシャー情報行列の逆行列を必要とし、大規模モデルでは直接計算が困難である。K-FAC（Martens & Grosse, 2015）はクロネッカー積による近似を、Sophia（Liu et al., 2023）はヘシアンの対角近似を用いて、二次最適化の実用化を進めている。
...
- Liu, H., Li, Z., Hall, D., Liang, P., & Ma, T. (2023). Sophia: A Scalable Stochastic Second-order Optimizer for Language Model Pre-training. *ICLR 2024*. arXiv: [arXiv:2305.14342](https://arxiv.org/abs/2305.14342).
    - 大規模言語モデルのための二次最適化手法。ヘシアンの対角近似を用いる。
