...
逆向きは $z_i = \log p_i + C$ （ $C$ は任意定数）となるが、これは $p_i > 0$ の範囲（単体の内部）でのみ定義される。境界 $p_i = 0$ では $\log p_i$ が発散するため、逆写像は定義されない。また、定数 $C$ はゲージ固定（例： $\sum_i z_i = 0$ を課す）によって一意に決まる。この不定性こそが、Softmaxが商空間を経由する構造の現れである。
...
> 2. **自然勾配の座標不変性**：自然勾配は、Fisher計量に関して **座標系によらず最急降下** を実現する。指数型分布族では自然パラメータと期待値パラメータの間の変換が簡潔であり、Fisher情報行列が比較的扱いやすい構造を持つ（例：カテゴリカル分布では **適切なパラメータ化により計算が効率化される**）。
