...
> **コラム：境界の呪いと歴史的スムージング**
>
> 深層学習以前の自然言語処理（n-gramモデルなど）において、最大の敵は「未知語」や「ゼロ頻度」の問題だった。学習データに一度も現れなかった単語の確率は $0$ になる。これを幾何学的に見ると、**確率単体の「境界（面や辺）」に張り付いている状態**に対応する。
>
> 後述する情報幾何学の視点では、境界上においてFisher計量が発散し、KLダイバージェンス（負の対数尤度）も無限大になる（ $\log 0 = -\infty$ ）。つまり、一度でも「確率0」を宣言してしまうと、損失関数が爆発し、学習や評価が破綻するのだ。
>
> かつての先人たちは、この「境界の呪い」から逃れるために、涙ぐましい努力を重ねた：
>
> - **Laplace平滑化（加算スムージング）**: 全カウントに $+1$ して、無理やり境界から引き剥がす。
> - **Kneser-Ney平滑化**: 低頻度語に確率質量を再配分する、職人芸的なアルゴリズム。
>
> これらはすべて、幾何学的には **「分布を無理やり単体の内部（interior）へ押し込む」** 操作に他ならない。
>
> Softmaxの偉大な点は、この問題を構造的に解決したことだ。 $\exp(x)$ は $x$ が有限である限り決して $0$ にならない。つまり、Softmaxを通すだけで、モデルは**自動的に、かつ滑らかに単体の内部に配置される**。現代の私たちが煩雑な平滑化処理を忘れ、エンドツーエンド学習に没頭できるのは、Softmaxが私たちを安全な「内部（滑らかな多様体上）」に留めてくれているからなのだ。
