...
$-\log p(x)$ は **情報量（self-information / surprisal）** と呼ばれる量である。これは「確実性からの遠さ」として捉えることができる：

- $p = 1$ （確実な出来事） $\implies -\log p = 0$ （遠さゼロ、驚きなし）
- $p \approx 0$ （希少な出来事） $\implies -\log p \to \infty$ （遠さ無限大、大きな驚き）

損失を最小化することは、正解クラスを「遠い場所」から「手元（ $p \to 1$ ）」へ引き寄せることに相当する。言い換えれば、**正解クラスをできるだけ「当然のこと（遠さゼロ）」として扱えるよう、モデルのパラメータを動かしている**。

エントロピー $H(P) = \mathbb{E}_{x \sim P}[-\log p(x)]$ は、この「遠さ」の分布 $P$ 全体に対する平均値である。第7回では、このエントロピーが分布という「霧の体積」として再登場する。
