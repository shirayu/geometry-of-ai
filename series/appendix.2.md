...
| 手法 | 何をしているか | 測定量 | 幾何学的解釈 | 限界 |
| ------ | --------------- | -------- | ------------- | ------ |
| **人手ラベリング** | 人間が「良い/悪い」を判定 | - | 外部の神託（幾何学外） | スケールしない |
| **EL2Nスコア** | 予測誤差の大きいデータを選別 | **距離** | 多様体からの「距離」の代理指標 | 「難しい」と「間違い」の区別不能 |
| **Influence Functions** | 除去時の損失変化を逆算 | **曲率** | 局所的な曲率への寄与度 | 計算コスト、近似の不安定さ |
| **kNN外れ値検出** | 近傍密度が低い点を除去 | **密度** | 多様体上の疎な領域の検出 | 「珍しい」と「間違い」の区別不能 |
| **TDA異常検知** | Persistenceの異常を検出 | **位相** | 局所トポロジーの破綻 | 高次元での計算困難、解釈の曖昧さ |
| **合成データ** | 論理エンジンで生成 | - | 多様体を「設計」する（データ選別ではない） | 分布の偏り、自己参照の罠 |
