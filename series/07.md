...
従来の点埋め込みでは、頻度が高い単語はノルムが大きくなる（原点から遠くなる）傾向があった。しかし、**最新の研究は、より本質的な視点を提供している**。
...
**幾何学的帰結**：つまり、高頻度語（高文脈多様性）は、**集中度 $\kappa$ が低くなる（分散が大きくなる）べき**である。

この視点に立てば、**「頻度」という統計量は、幾何学的な「不確実性（領域の広さ）」へと翻訳される**。vMF分布を用いた分布埋め込みは、Zipf則が要求するこの「意味の広がり」を、自然な形で空間に許容する枠組みなのである。

> [!NOTE]
> **頻度 vs 文脈的多様性**：伝統的な言語統計では「頻度」が重視されてきたが、Zipf則の真の駆動力は文脈的多様性であることが明らかになりつつある。'the' のような機能語は高頻度だが文脈的多様性は限定的であり、実際の埋め込み空間では比較的鋭い分布を持つ。一方、'run' のような動詞は頻度も高く文脈的多様性も高いため、広い領域に分散する。この区別が、分布埋め込みにおける $\kappa$ の適切な推定を可能にする。

**従来のアプローチとの接続**：第2回で見た「頻度が高い単語は長いベクトル」という観察は、この文脈的多様性を「ノルム」という形で暗黙的に捉えようとしていたと解釈できる。しかし、ノルムは方向情報と混在するため解釈が困難だった。分布埋め込み（ $\kappa$ の導入）は、この混在を解消し、**方向＝意味、 $\kappa$ ＝確信度（≒文脈的多様性の逆数）**という明確な分離を実現する。
