...
VAEの潜在空間では、正則化が事後分布をこの標準正規分布に合わせようとする。結果として、潜在表現は $\sqrt{d}$ 付近の典型帯に集中する傾向がある。この帯から外れた領域——原点付近も遠方も——では復元が不安定になりうる。空間全体が均等に使われるわけではなく、**半径方向に狭い「居住可能帯」ができる**のがガウス潜在空間の構造的な特徴である。

> [!NOTE]
> **典型集合と「殻」の直感：** 高次元ガウス分布のサンプルが球殻に集中する現象は、低次元の直感に反する。2次元や3次元では確かに原点付近にサンプルが多いが、次元が上がると体積の効果が支配的になり、半径 $\sqrt{d}$ 付近の薄い殻にほとんどの確率質量が集中する。VAEの潜在空間でも同様の現象が起き、表現が実質的に使える領域が制限される。
