...
この正則化項は、幾何学的には**原点への引力**として働く。すべての潜在表現が原点を中心としたガウス分布に引き寄せられる。散らばろうとする表現を「重力」で中心に引き戻す力と考えてよい。

### ユークリッド空間の不均一性

ここで問題になるのが、ガウス分布が生む空間の**不均一性**である。

標準正規分布 $\mathcal{N}(\mathbf{0}, \mathbf{I})$ では、原点付近の密度が最も高く、周辺に行くほど密度が急激に低下する。つまり、VAEの潜在空間には「中心は混み合い、周辺はスカスカ」という構造的な偏りが生じる。

| 領域 | 密度 | 表現の性質 |
| --- | --- | --- |
| 原点付近 | 高密度 | 多くの概念が密集、区別が困難 |
| 中間領域 | 中密度 | 標準的な表現が分布 |
| 周辺領域 | 低密度 | 表現が疎、補間が不安定 |

高次元ではこの問題はさらに深刻になる。高次元ガウス分布のサンプルは、原点ではなく $\sqrt{d}$ 付近の「殻」に集中するという現象が知られている（concentration of measure）。正則化の引力が原点を向いているのに対し、実際にサンプルが存在する領域は原点から離れた球殻上にある。この不整合が、VAEの潜在空間における表現力の制約の一因となる。

> [!NOTE]
> **「ソープバブル」現象：** 高次元ガウス分布のサンプルが球殻に集中する現象は、直感に反する。2次元や3次元のガウス分布は確かに中心付近に集まるが、次元が上がると「体積」の効果が支配的になり、半径 $\sqrt{d}$ 付近の薄い殻にほとんどのサンプルが集中する。

### 球面への脱出：vMFという回答

第3回で「プラネタリウム」として導入した球面表現は、この不均一性に対する幾何学的な回答である。

単位球面 $\mathbb{S}^{d-1}$ 上では、すべての点が原点から等距離にある。「中心ほど密、周辺ほど疎」という勾配が存在しない。nGPT（第4回）が示した $L_2$ 正規化の効果も、この等方性によるものであった。
...
| 正則化の方向 | 原点への引力 | 球面上の一様分布への引力 |
| 密度の均一性 | 不均一（中心が密） | 均一（等方的） |
| 確信度の表現 | 分散 $\sigma^2$ | 集中度 $\kappa$ |
| 意味と確信度の分離 | ノルムに意味が混入しうる | 方向＝意味、$\kappa$＝確信度 |

Davidson et al. (2018) は、VAEの潜在空間にvMF分布を導入した**Hyperspherical VAE**を提案し、ガウス事前分布の制約を球面の幾何学で克服できることを示した。これは、「重力（正則化）の方向を変える」ことで、より均一で表現力の高い潜在空間が得られることの実証である。
