...
> [!NOTE]
> **ハルシネーションの2類型：無知の捏造 vs 誤信念の表出**
>
> 本回で扱う不確実性（低κ）に起因するハルシネーションは「**無知の捏造**」と呼べる。モデルは知らないことを自覚できず、確率的に最もありそうな答えを返してしまう。これは、集中度κを測定することで検出可能である。
>
> しかし、ハルシネーションにはもう一つの類型がある：「**誤信念の表出**」である。これは、訓練データに含まれる誤りを学習した結果、**高いκで確信を持って間違った答えを返す**現象である（例：「1+1=3」と学習してしまった場合）。
>
> この2類型の決定的な違いは：
> - **無知の捏造**：κが低い → エントロピーベースの検出が有効
> - **誤信念の表出**：κは高いが方向が誤り → **幾何学的には検出不可能**
>
> 後者の問題は、データの「純度」に関わる深刻な課題であり、幾何学だけでは解決できない。詳細は[Appendix 2: 多様体の純度問題](appendix.2.md)を参照されたい。
...
> **本講義内での発展的議論**
>
> 本回で扱った「無知の捏造」型のハルシネーション（低κによる不確実性）に対し、「誤信念の表出」型のハルシネーション（高κだが誤った方向）は、[Appendix 2: 多様体の純度問題](appendix.2.md)で詳述している。そこでは、幾何学が「データの真偽」に対して原理的に中立である理由と、外部検証が必要となる構造的限界を説明する。
