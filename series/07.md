...
def _householder_rotation(x, u, v):
    ...
    しかし、vMFサンプリングの文脈では、北極周りの点を
    μ周りに移動させる目的には十分である。
    回転（行列式+1）が必要な場合は、2回の反射を組み合わせるか、
    Rodriguesの回転公式を使用すること。
    """
    # u, vは単位ベクトル
    # 反射軸: w = normalize(v - u)
    w = v - u
...
def _log_normalizer_vmf_exact(kappa, dim):
    ...
    # nu = dim / 2 - 1
    # log_ive = np.log(ive(nu, kappa) + 1e-300)  # ive は exp(-κ) * I_ν(κ)
...
class OODDetector:
    ...
    def calibrate(self, kappa_in, kappa_out, target_fpr=0.05):
        ...
        Args:
            kappa_in: 分布内データのκ [n_in]
            kappa_out: 分布外データのκ [n_out]
