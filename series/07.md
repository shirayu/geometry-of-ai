...
これは、**高次元かつ高 $\kappa$** という条件下での近似であり、一般には成り立たない。
...
> [!NOTE]
> 以下のコードは PyTorch >= 1.9 を前提とする。vMF関連の実装は、数値安定性のために注意深く設計されている。
...
def _householder_rotation(x, u, v):
    """Householder変換でuをvに写す回転をxに適用"""
    # u, vは単位ベクトル
    # 回転軸
    w = F.normalize(v - u, dim=-1)
...
```python
def kl_divergence_vmf(mu1, kappa1, mu2, kappa2):
    """2つのvMF分布間のKLダイバージェンス
    ...
    # 正規化定数の差（対数）
    log_C1 = _log_normalizer_vmf(kappa1, dim)
    log_C2 = _log_normalizer_vmf(kappa2, dim)
...
def _bessel_ratio_approx(kappa, dim):
    """Bessel関数の比 I_{d/2}(κ) / I_{d/2-1}(κ) の近似"""
    # 高κでの近似: A(κ) ≈ 1 - (d-1)/(2κ)
    nu = dim / 2 - 1
    return 1 - (2 * nu + 1) / (2 * kappa + 1e-8)


def _log_normalizer_vmf(kappa, dim):
    """vMF分布の対数正規化定数 log C_d(κ)"""
    nu = dim / 2 - 1
    # 近似: log C ≈ (d/2 - 1) * log κ - d/2 * log(2π) - κ
    # （高κ・高次元での近似）
