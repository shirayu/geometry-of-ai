...
埋め込み空間でも同じことが言える。「りんご」という単語の表現は、文脈によってぶれるだろうか。「量子もつれ」という専門用語の表現は、学習データの量によって確信度が異なるだろうか。従来の点埋め込みは、この「ぶれ」や「確信度」という次元を持たなかった——より正確には、**確信度を無限大に固定していた**。
...
第2回、第3回で見た $L_2$ 正規化の効果も、この「半径の消去」によるものであった。ベクトルを単位球面に配置することは、幾何学的には「半径方向の情報を持たない」と宣言することである。点埋め込みの視点で言えば、これは**「不確実性（分散）の情報を持たない」——つまり確信度を無限大に固定する**——ことに等しい。
...
**重要な認識**：我々がこれまで使ってきた従来の点埋め込み（Word2Vec、正規化されたTransformer埋め込みなど）は、この表の最後の行——すなわち $\kappa = \infty$ という極限状態——を暗黙のうちに仮定していたと解釈できる。単位球面上の一点に配置することは、「この方向に確信度100%で存在する」と宣言することに等しい。
...
従来の点埋め込み（ $\kappa = \infty$ を暗黙の前提とする手法）では、入力は必ずどこかの方向に射影される。未知の入力に対しても、**「ここです（確信度100%）」と無理やり断言する**構造になっている。星がない領域でも、最も近い星に関連付けられる。これは、不確実性を表現する自由度（ $\kappa$ パラメータ）を持たないことの帰結である。
...
星がまばらな領域（学習データが少ない、または概念が曖昧な領域）で、モデルはそれでも何らかの出力を生成しなければならない。従来の点埋め込み（ $\kappa = \infty$ 固定）では、「分かりません」という選択肢がないため、確信度100%で誤った出力を生成してしまう。
