...
> [!NOTE]
> **次回への伏線：** コサイン類似度はベクトルのノルム（長さ）を無視して方向だけを見る。TF-IDFは各次元の重み付けで方向を変えるため、コサインと組み合わせても有効ではある。しかし、「重要度」をノルムに持たせる設計（長い文書ほど重要、など）とは相性が悪い。ノルムと方向、どちらに何の情報を載せるべきか——この混線が、第2回「ノルムの呪い」のテーマである。
