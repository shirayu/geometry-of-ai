...
ここで、TF(t,d) は文書 $d$ における単語 $t$ の出現頻度、DF(t) は単語 $t$ を含む文書数、 $N$ は全文書数。IDF（逆文書頻度）は、「多くの文書に出現する単語は重要度が低い」という発想を反映する。

**幾何学的には何をしているか：** TF-IDFは、単語頻度ベクトルの各次元（軸）を**スケーリング**している。頻出語の軸を縮め、希少語の軸を伸ばすことで、空間を歪める。これは、PCAの「分散が大きい方向を重視する」発想とは逆に、「希少な（分散が小さい可能性のある）方向を持ち上げる」という操作である。

**しかし限界も明確だった：** TF-IDFで重み付けしても、結局は高次元の疎（スパース）なベクトルのままである。文書同士の類似度を測る際、コサイン類似度を使うと正規化されてしまい、長さ（TF-IDFで調整した重み）の情報が部分的に失われる。また、「王」と「女王」の意味的な近さといった関係性は、依然として捉えられない。

> [!NOTE]
> **次回への伏線：** TF-IDFのような重み付けは、ベクトルの**ノルム（長さ）** に情報を載せる試みでもあった。しかし、ノルムと方向が混在すると、どちらが「本質的な類似度」を表すのかが曖昧になる。第2回「ノルムの呪い」では、この混線が引き起こす問題を詳しく分析する。
...
Bag-of-Wordsの限界を打破する試みの一つが、**共起統計**に基づく手法である。「意味が近い単語は、同じ文脈に出現しやすい」という分布仮説（Harris, 1954）に基づき、単語間の関係性を捉えようとした。
...
後のWord2Vec（Mikolov et al., 2013）やGloVe（Pennington et al., 2014）は、この共起統計を**低次元の連続ベクトル空間に圧縮**することで、Bag-of-Wordsの限界を克服した。特にGloVeは、重み付き共起行列（PPMI行列など）の因子分解として解釈でき、PMIから埋め込みへの自然な接続を示している。
