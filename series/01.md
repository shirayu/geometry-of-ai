...
PCAの線形性という限界は、研究者たちにとって明白だった。特に「スイスロール」のような非線形多様体上のデータに対しては、線形射影では本質的な構造が失われてしまう。

この問題に対する画期的な解答が、2000年のScience誌に相次いで発表された。**Isomap**（Tenenbaum et al., 2000）と**LLE（Locally Linear Embedding）**（Roweis & Saul, 2000）である。

両者は異なるアプローチを取りながらも、共通の洞察を持っていた：**データは高次元空間に埋め込まれた低次元の曲がった多様体上に分布している**という仮説である。

### Isomap：測地距離を保つ地図

Isomapのアイデアは、古くからある**多次元尺度構成法（MDS: Multidimensional Scaling）** を拡張したものである。

**MDSの基本原理：**

MDSは、与えられた点間の距離行列から、その距離関係を保つような低次元配置を求める手法である。つまり、「距離だけが分かっている」状況から、点の座標を復元する。

しかし、古典的なMDSは**ユークリッド距離**を保存する。これは、平らな空間に埋め込まれたデータには有効だが、曲がった多様体上のデータには適さない。

**Isomapの発想：測地距離の導入**

Isomapの核心的アイデアは、ユークリッド距離の代わりに**測地距離（geodesic distance）** を使うことである。

測地距離とは、多様体上の「最短経路の長さ」である。球面上の2点間の測地距離は、大円に沿った距離であり、直線距離（球を貫通する距離）ではない。

Isomapのアルゴリズム：

1. **近傍グラフの構築**：各点について、$k$最近傍点をユークリッド距離で見つけ、それらとの間にエッジを張る
2. **測地距離の近似**：グラフ上の最短経路長（ダイクストラ法やFloyd-Warshall法）を、多様体上の測地距離の近似とする
3. **MDSの適用**：近似した測地距離行列に対してMDSを適用し、低次元埋め込みを得る

**幾何学的直感：**

スイスロールを想像しよう。表面上の2点が空間的には近くても、多様体上では遠い（ロールを一周する必要がある）ことがある。Isomapは、この「表面を辿った距離」を測地距離として推定し、それを保存するような2次元平面への展開を求める。

> [!NOTE]
> **測地距離の近似精度：** Isomapの成功は、近傍グラフが多様体の構造を十分に捉えられるかに依存する。$k$が小さすぎるとグラフが疎になり、大きすぎると「ショートカット」（多様体上では遠いが空間的に近い点同士）を繋いでしまう。適切な$k$の選択は、データの密度と多様体の曲率に依存する。

### LLE：局所的な線形性から大域的な構造へ

LLEは、Isomapとは対照的なアプローチを取る。測地距離を明示的に計算する代わりに、**局所的な線形近似**を積み重ねることで、大域的な非線形構造を捉える。
...
2. **局所的な重みの計算**：各点を近傍点の線形結合で表現する重み $w_{ij}$ を求める：
   $$\min_{\{w_{ij}\}} \sum_i \left\| \mathbf{x}_i - \sum_j w_{ij} \mathbf{x}_j \right\|^2$$
   ただし、$\sum_j w_{ij} = 1$ という制約のもと（重みの総和は1）
...
LLEは「局所的な関係性」を保存する。各点が近傍点からどのように「構成」されているかという情報を、低次元空間でも維持する。これにより、大域的には非線形な構造であっても、局所的な線形性を利用して展開できる。

スイスロールの例では、各点はその周辺の点の線形結合として表現できる。この「周辺との関係」を保ったまま2次元に展開すると、ロールがほどけた平面が得られる。

### IsomapとLLEの対比

| 観点 | Isomap | LLE |
| --- | --- | --- |
| **保存する量** | 測地距離（大域的） | 局所的な線形関係 |
| **計算の流れ** | 距離→MDS | 重み→固有値問題 |
| **スケーラビリティ** | $O(n^2)$〜$O(n^3)$（全点対距離） | $O(kn^2)$（疎行列の固有値問題） |
| **理論的背景** | 微分幾何（測地線） | 線形代数（固有空間） |

### 何が革命的だったか

IsomapとLLEが2000年にScience誌に掲載されたことの意義は、単に新しい手法を提案したことではない。それは、**多様体学習（manifold learning）** という新しいパラダイムを確立したことにある。

**パラダイムシフトの三要素：**

1. **仮定の明示化**：データが低次元多様体上に分布しているという仮定を、明示的に述べた
2. **幾何的視点の導入**：「距離を保つ」「局所構造を保つ」という幾何学的な目的関数を設計した
3. **非線形の受容**：線形性という制約を外し、曲がった空間を直接扱う方法を示した

> [!IMPORTANT]
> **深層学習への伏線：** IsomapとLLEは、後のオートエンコーダや変分オートエンコーダ（VAE）の理論的基礎の一つとなった。特に「高次元データが低次元の潜在多様体から生成される」という発想は、現代の生成モデルに直接受け継がれている。第9回で扱うVAEは、この多様体学習の確率的な定式化と見なせる。

### 限界と後続研究

IsomapとLLEは画期的だったが、実用上の限界もあった。

**1. パラメータ選択の困難**

近傍数$k$の選択は、データ依存で自明ではない。$k$が小さすぎるとグラフが断片化し、大きすぎると多様体構造が破壊される。

**2. 計算コスト**

どちらも$O(n^2)$以上の計算量を要し、大規模データには適用困難だった。

**3. Out-of-sample 問題**

新しいデータ点の埋め込みを求める明示的な方法がなかった（訓練データ全体に依存）。

これらの限界は、後の手法で部分的に解決されていく：

- **Laplacian Eigenmaps**（Belkin & Niyogi, 2003）：グラフラプラシアンを用いた定式化
- **t-SNE**（van der Maaten & Hinton, 2008）：確率分布のマッチングによる可視化（第7回で詳述）
- **Autoencoders**（Hinton & Salakhutdinov, 2006）：ニューラルネットによるパラメトリックな埋め込み（第9回）

### 「曲がった空間」への扉

IsomapとLLEは、機械学習における「幾何学的思考」の復権を象徴している。

それ以前の手法——PCA、SVM、LDA——は、ユークリッド空間という「平らな舞台」を暗黙に仮定していた。IsomapとLLEは、**データ自体が舞台の形を教えてくれる**という発想を持ち込んだ。

この「データの幾何」への注目は、後の深層学習時代においてさらに発展する。第2部「曲がった空間での戦い」では、この系譜を辿っていく。
