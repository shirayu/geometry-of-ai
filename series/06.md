...
初期のTransformer（Vaswani et al., 2017）は、正弦波による**絶対位置埋め込み**を使った。しかし、現代の大規模言語モデル（Llama, Qwen, Mistral等）では、**RoPE（Rotary Position Embedding）**が標準的に使われている。

### RoPEの核心：回転による相対位置

RoPE（Su et al., 2024）の核心的アイデアは、位置を**回転**として表現することである。
...
第4回で見たように、Softmaxには温度パラメータがある。Attentionの文脈では、スケーリング係数 $\sqrt{d_k}$ がこの役割を果たす。

$$\text{softmax}\left(\frac{\mathbf{Q} \mathbf{K}^\top}{\sqrt{d_k}}\right)$$

$\sqrt{d_k}$ が小さいと、Attention分布は鋭くなり（低温）、特定のKeyに集中する。$\sqrt{d_k}$ が大きいと、分布は平坦になり（高温）、多くのKeyに分散する。
...
RoPEは、位置情報を「回転」として表現する設計であり、本講義のテーマ（角度、回転、球面）と深く整合する。絶対位置ではなく相対位置に基づくAttentionを可能にし、現代のLLMの標準的な位置埋め込みとなっている。
...
- Su, J., Ahmed, M., Lu, Y., Pan, S., Bo, W., & Liu, Y. (2024). RoFormer: Enhanced Transformer with Rotary Position Embedding. *Neurocomputing*, 568, 127063. arXiv: [arXiv:2104.09864](https://arxiv.org/abs/2104.09864) (2021).
    - RoPEの原論文。回転による相対位置表現を提案。
...
- Loshchilov, I., Hsieh, C.-P., Sun, S., & Ginsburg, B. (2024). nGPT: Normalized Transformer with Representation Learning on the Hypersphere. *arXiv:2410.01131*.
    - すべての表現を単位球面上に制約するTransformer設計。
