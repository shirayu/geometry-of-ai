...
$$
となり、結果が**相対位置 $(n - m)$ のみに依存する**ことである。これは、ベクトルの「絶対的な位置」ではなく「ペア間の距離（オフセット）」をモデルが直接扱えることを意味する。

### 2次元での具体例

RoPEの仕組みを、まずは**単一の周波数を持つ2次元部分空間**の場合で具体的に見てみよう。 位置 $m$ での回転行列は以下のように定義される：

$$
R_m = \begin{pmatrix} \cos(m\theta) & -\sin(m\theta) \\ \sin(m\theta) & \cos(m\theta) \end{pmatrix}
$$
ここで、基本回転角 $\theta = 0.5$（約28度）と仮定し、Queryベクトル $\mathbf{q} = (1, 0)$ の変化を追ってみる。（※実際には次元ごとに異なる $\theta$ を使うが、ここでは直観のために固定する）。
...
2. **相対的な距離の抽出**: 内積をとると絶対位置 $m, n$ は消え、差分 $n-m$ だけが残る。これにより、文章中のどこに現れても、同じ距離関係にある単語ペアは同じように処理できる。
3. **長距離減衰の傾向**: 複数の周波数が組み合わさることで、距離が離れると位相がズレて内積が打ち消し合いやすくなる（ランダムウォークに近い振る舞いをする）。これにより、**結果として**遠くの単語への注目が平均的に弱まる傾向（Long-term decay）が生まれることがある。*補足*: これは強制的な減衰（ALiBi等）とは異なり、モデルが学習次第で遠くの情報を維持することも可能な、柔軟な特性である。
