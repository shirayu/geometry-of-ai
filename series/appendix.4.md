...
重要なのは、**逆向きのKLダイバージェンス $D_{\text{KL}}(p_{\theta+d\theta} \| p_\theta)$ も、2次の項は同じフィッシャー情報行列になる**点である。つまり、フィッシャー情報行列は、分布の「局所的な隔たりの2次形式」を表している。
...
しかし、学習の軌道は本来、**パラメータの表し方に左右されるべきではない**。自然勾配法は、フィッシャー情報行列を使って勾配を補正することで、**どのようなパラメータ化を選んでも、分布空間での同じ経路を辿る**ことを保証する。
