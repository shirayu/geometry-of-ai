...
| **レベル2：表現ベクトル** | $\mathbf{h} \in \mathbb{R}^d$ | 高次元空間内の向きと長さ | 向き（角度）と大きさ（ノルム） |
| **レベル3：確率分布** | $p(x; \theta)$ | 不確実性を含む「関数」 | 統計的構造全体（平均、分散、形状） |

**重要なのは第3の階層である。** 統計多様体上の「点」$\theta$ は、実は確率分布 $p(x; \theta)$ という **関数そのもの** を指している。つまり：

- **一般的な直感：** 点 = 位置を表すスカラや座標
- **情報幾何学：** 点 = 確率分布という**無限次元の情報構造**

たとえば、正規分布 $N(\mu, \sigma^2)$ という「点」は、単に2つの数値 $(\mu, \sigma^2)$ だけではなく、**全ての可能な $x$ に対する確率密度関数 $p(x)$ という関数の全体**を表している。

> [!TIP]
> **比喩的理解：点は「プラネタリウム」**<br>
> 夜空を見上げると、星が点として見える。しかし、その一つ一つの「点」は、実際には巨大な太陽系であり、惑星や衛星や無数の構造を内包している。情報幾何学における「点」も同様である。パラメータ空間上の一つの「点」は、その内部に**確率分布という宇宙**を抱えている。
>
> スカラという「皮」を剥ぐと、その中から情報という「形（関数）」が現れる。この視点転換が、情報幾何学の出発点である。
...
この「情報密度」という概念は、**「点」の内部構造と深く関わっている**。

思い出してほしい。統計多様体上の「点」$\theta$ は、単なるスカラではなく、確率分布 $p(x; \theta)$ という関数そのものだった。フィッシャー情報行列は、この「点の内部にある分布の構造」が、パラメータの微小変化に対してどれだけ敏感に反応するかを測定している。

| 概念 | 数学的定義 | 直感的イメージ |
| --- | --- | --- |
| **点** $\theta$ | パラメータベクトル | 座標（位置） |
| **点の内部** $p(x; \theta)$ | 確率分布（関数） | 不確実性の全体像 |
| **フィッシャー情報** $G(\theta)$ | 分布の感度（計量） | 「点の内部がどれだけ揺れやすいか」 |

つまり、フィッシャー情報行列が大きい場所では、**点の内部にある分布構造が脆く、パラメータの微小な揺れが分布の大きな変化を引き起こす**。逆に、フィッシャー情報が小さい場所では、**点の内部の分布構造が頑健で、パラメータが多少動いても分布はほとんど変わらない**。

> [!TIP]
> **点の「重み」と「密度」の違い**<br>
> - **点の重み（スカラ的な量）**：データ点の個数や、ベクトルのノルム（大きさ）
> - **点の密度（関数的な構造）**：確率分布の集中度や、パラメータ変化への感度
>
> フィッシャー情報は後者を測る。点という「容器」の中に、どれだけ「変化しやすい情報」が詰まっているかを定量化する。
...
- **「この『点』の内部には、どんな構造が隠されているのか」**

最後の問いは、情報幾何学の核心である。数値的な「点」の背後に、確率分布という豊かな構造が広がっている。スカラという「皮」を剥いだ先に、情報という「形」が現れる。この多層的な視点こそ、AIの本質を理解する鍵となる。
