...
| **コサイン類似度** | ベクトルの方向の近さ | 対称、長さを無視 | 正規化された埋め込み、Attention |
...
> [!NOTE]
> **実装上の注意**：Poincaré球モデルでは、すべての点が $\|\mathbf{u}\| < 1$ を満たす必要がある（開球内）。実装時には数値安定性のため、境界 $\|\mathbf{u}\| = 1$ に近づきすぎないようクリッピング（例: $\|\mathbf{u}\| < 1 - \epsilon$ で $\epsilon = 10^{-5}$）を行うことが推奨される。また、arcosh の引数が1以上であることを保証するため、分母のゼロ除算回避も必要。
...
# Fisher情報行列を単位行列で代用しているため、実際には単なる前処理付きSGDである。
# 真の自然勾配法の実装ではない。

def natural_gradient_step(model, loss, lr=0.01, damping=1e-4):
    """自然勾配法の"形式"を示す疑似コード（実用不可）
    
    注意: 本実装はFisher情報を単位行列で代用しており、
    自然勾配の本質（モデル依存の計量）が失われている。
...
> [!CAUTION]
> 上記は**自然勾配法そのものではなく、その"形式"を示す教育的疑似コード**である。Fisher情報行列を単位行列で代用しているため、モデル依存の計量構造（自然勾配の本質）が失われている。真の自然勾配法を実装するには、K-FAC（Martens & Grosse, 2015）などの専用手法、または少なくとも対角Fisher近似（モデル出力の分布から計算）が必要である。
...
- **Attention**：QueryとKeyの内積（コサイン類似度）が高い = 方向が近い = その方向が「重要」
- **MoE**：入力とExpertのゲートベクトルの内積が高い = その部分空間が「重要」

ただし、**厳密にはAttentionは内積幾何に基づく類似度計算であり、Fisher情報（統計モデルの計量）とは別概念**である。ここでの「情報密度」は、教育的な比喩として捉えるべきである。
...
| --- | --- | --- | --- | --- |
| **距離・内積** | 対称 | 2点間の隔たり | 配置・類似度判定（Attention） | 内積幾何に基づく |
...
> [!IMPORTANT]
> **比喩と厳密性の区別**：本Appendixでは「AttentionやMoEが情報密度の高い場所に計算資源を投じる」という表現を用いたが、これは**直感的理解を助けるための比喩的対応づけ**である。厳密には、AttentionはQuery-Keyの内積類似度、MoEは入力とゲートベクトルの内積に基づいており、Fisher情報（統計モデルのパラメータ空間の計量）とは別物である。両者の関係は、「重要度・感度が高い方向への資源配分」という抽象レベルでの類似性であり、数学的同一性ではない。
