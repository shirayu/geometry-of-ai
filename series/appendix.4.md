...
> [!IMPORTANT]
> **次元の注意：** 統計多様体自体は通常**有限次元**（パラメータ数に等しい）である。正規分布族は2次元、多項分布（$k$クラス）は$k-1$次元の多様体となる。分布そのものは関数なので背後に豊かな構造があるが、多様体として扱うときは $\theta$ の次元数だけの有限次元空間である。
...
> フィッシャー情報行列は、KLダイバージェンス $D_{\text{KL}}(p_\theta \| p_{\theta+d\theta})$ の2次近似の係数として現れる。つまり、局所的な分布の「距離」（曲がり具合）を与えている。
...
>
> また、フィッシャー情報行列の値は**座標（パラメータ化）の選び方に依存する**。だからこそ計量テンソルとして扱い、座標によらない幾何学的性質を抽出する必要がある。
>
