...
| **レベル2：表現ベクトル** | $\mathbf{h} \in \mathbb{R}^d$ | **第7回で拡張した対象**。<br>従来の点埋め込みは $\kappa \to \infty$ （確信度無限大）の極限。<br>第7回では、これを有限の $\kappa$ を持つ分布に拡張した。 | 望遠鏡で見る前の星<br>（点 or 霧） | 点： $\kappa = \infty$ <br>分布： $\kappa$ 有限 |
