...
CLIPの損失関数は、**InfoNCE損失** の一種である：

$$\mathcal{L} = -\frac{1}{N}\sum_{i=1}^{N}\log \frac{\exp(\text{sim}(\mathbf{t}_i, \mathbf{i}_i)/\tau)}{\sum_{j=1}^{N} \exp(\text{sim}(\mathbf{t}_i, \mathbf{i}_j)/\tau)}$$
...
| 負例を遠ざける | $\cos\theta \to -1$ （角度を大きく） | 斥力 |

> [!NOTE]
> **「引力・斥力」は比喩：** この物理的メタファーは直感的な理解を助けるが、実際の最適化は勾配降下法で行われており、力学シミュレーションではない。また、負例を「できるだけ遠ざける」わけではなく、Softmax内での相対的な順位付けが重要である。
...
CLIPでは $\tau$ は学習可能なパラメータとして扱われることが多い。適切な温度は、正例と負例の区別の「粒度」を制御する。

## 統一多様体の問題：「無理やり同じドームに押し込む」歪み

### モダリティ固有の構造の喪失

CLIPのアプローチには、根本的な問題がある。異なるモダリティを同じ空間に押し込むことで、**各モダリティに固有の構造が失われる**可能性があるのだ。

**画像の空間的隣接関係**：画像では、「猫の顔」と「猫の尻尾」は空間的に離れているが、同じ「猫」という概念に属する。しかし、テキストでは「顔」と「尻尾」は別の単語であり、その空間的関係は表現されない。統一空間では、この画像固有の空間構造が薄められる。

**テキストの構文構造**：テキストでは、主語-動詞-目的語の関係や、修飾関係といった構文構造が意味を担う。画像にはこのような階層的な文法構造は存在しない。統一空間では、この構文的な情報が失われる可能性がある。

| 失われうる構造 | 説明 | 影響 |
| --- | --- | --- |
| 画像の空間的関係 | ピクセル間の隣接関係、物体の配置 | 位置に基づく推論が困難に |
| テキストの構文構造 | 文法的な依存関係、係り受け | 複雑な文の理解が困難に |
| 音声の時間的連続性 | 音の遷移パターン、リズム | 長期的な依存関係が失われる |
...
完全に同じ空間に押し込むのではなく、各モダリティが独自の多様体を持ちつつ、それらの間の**写像（morphism）** を学習する：

$$f: \mathcal{M}_\text{text} \to \mathcal{M}_\text{image}, \quad g: \mathcal{M}_\text{image} \to \mathcal{M}_\text{text}$$

この写像が構造を保存する（準同型である）なら、各多様体の固有の構造を維持しながら、モダリティ間の変換が可能になる。
...
print(f"Off-diagonal (negative pairs) mean: {(sim_matrix.sum() - sim_matrix.diag().sum()).item() / (batch_size * (batch_size - 1)):.4f}")
```
