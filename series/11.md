...
$$\mathcal{L}_{\text{t→i}} = -\frac{1}{N}\sum_{i=1}^{N}\log \frac{\exp(\text{sim}(\mathbf{t}_i, \mathbf{i}_i)/\tau)}{\sum_{j=1}^{N} \exp(\text{sim}(\mathbf{t}_i, \mathbf{i}_j)/\tau)}$$
...
CLIPでは $\tau$ は学習可能なパラメータとして扱われる。実装上は、 $1/\tau$ に相当する **logit_scale** をログ空間で学習する形が典型的である（`logit_scale = nn.Parameter(torch.log(torch.tensor(1/0.07)))`のように初期化）。適切な温度は、正例と負例の区別の「粒度」を制御する。
...
| 取り出しにくくなりうる構造 | 説明 | 影響 |
| --- | --- | --- |
| 画像の空間的関係 | ピクセル間の隣接関係、物体の配置 | 位置に基づく推論が困難になりうる |
| テキストの構文構造 | 文法的な依存関係、係り受け | 複雑な文の理解が困難になりうる |
| 音声の時間的連続性 | 音の遷移パターン、リズム | 長期的な依存関係が保持されにくい |
