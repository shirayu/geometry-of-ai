...
> [!NOTE]
> **条件依存性：** この傾向は常に成り立つわけではなく、手法（SGNS/CBOW）、サブサンプリングの有無、負例分布、正則化、学習率スケジュール等によって変わりうる。以下の議論は「典型的に観察される傾向」として理解されたい。
...
> [!NOTE]
> **例外の存在：** タスクによっては、昔からコサイン類似度を前提として、事実上の正規化相当の運用をしていた例もある（情報検索など）。ここでの記述は「代表的な深層学習の実践」についてである。
...
理想的には、「近い点」と「遠い点」を明確に区別したい。しかし、特定の条件下では、高次元で最遠点と最近点の距離比が1に近づく現象が起きる：
...
> [!NOTE]
> **球面符号との関係：** 「内積が $\epsilon$ 以下のベクトルをいくつ配置できるか」は、球面符号（spherical code）の問題として研究されている。ランダム構成では、$d$ 次元で内積 $\epsilon$ 以下のベクトルを $\exp(c \cdot d)$ 個程度配置できることが知られている（$c$ は $\epsilon$ に依存する定数）。ただし、この「指数的」というスケーリングは、$\epsilon$ の取り方、誤差の定義、ランダム構成か最適配置かによって係数や成立条件が変わる。詳細は Conway & Sloane (1999) を参照。
...
カプセルネットワークは、理論的には美しいアイデアだが、実用上の課題が多いと指摘されている：
